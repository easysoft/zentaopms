...
class xuanxuanIm extends imModel
{
    public function getExtensionList($userID)
    {
        ...
        if(common::hasPriv('bug', 'create'))
        {
            if(empty($products))
            {
                $actions->createBug = array('title' => $this->lang->im->createBug, 'url' => $baseURL . helper::createLink('product', 'showerrornone', '', 'xhtml'), 'height' => "600px", 'width' => "800px");
            }
            else
            {
                $actions->createBug = array('title' => $this->lang->im->createBug, 'url' => $baseURL . str_replace('/x.php', '/index.php', helper::createLink('bug', 'create', "product=$productID", 'xhtml')), 'height' => "600px", 'width' => "800px");
            }
        }
        if(common::hasPriv('story', 'create'))
        {
            if(empty($products))
            {
                $actions->createStory = array('title' => $this->lang->im->createStory, 'url' => $baseURL . helper::createLink('product', 'showerrornone', '', 'xhtml'), 'height' => "600px", 'width' => "800px");
            }
            else
            {
                $actions->createStory = array('title' => $this->lang->im->createStory, 'url' => $baseURL . str_replace('/x.php', '/index.php', helper::createLink('story', 'create', "product=$productID", 'xhtml')), 'height' => "600px", 'width' => "800px");
            }
        }
