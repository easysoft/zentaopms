...
class biModel extends model
{
    ...
    public function isSqlBuilderAction()
    {
        ...
        $sqlBuilder = zget($_POST, 'sqlBuilder', '');

        return !empty($action) && !empty($sqlBuilder);
    }
    ...
    public function sqlBuilderAction()
    {
        $action = zget($_POST, 'action', '');
        $sqlBuilder = zget($_POST, 'sqlBuilder', '{}');
        $sqlBuilder = json_decode($sqlBuilder, true);
        $builder = new sqlBuilder($sqlBuilder);
        ...
        $builder->build($this->config->db->prefix, $parser);
    ...
    public function sqlBuilderStepAction($builder)
    {
        ...
        $checkList = array('checkFrom' => 'table', 'checkJoins' => 'table', 'checkSelects' => 'field', 'checkWheres' => 'where', 'checkQuerys' => 'query');
     ...
     * @return void
     */
    public function sqlBuilderTableAction($builder): void
    {
     ...
     * @return void
     */
    public function sqlBuilderFuncAction($builder): void
    {
     ...
     * @return void
     */
    public function sqlBuilderFieldAction($builder): void
    {
     ...
     * @return void
     */
    public function sqlBuilderGroupAction($builder): void
    {
        $builder->setAggFunc();
        $builder->processFuncs();
    }

    public function sqlBuilderQueryAction($builder): void
    {
    ...
    private function setTableDescList($builder)
    {
        ...
        foreach($joins as $join)
        {
            ...
            $builder->addTableDesc($table, $this->getTableDescList($table));
        }
    }
     ...
     * @return array
     */
    private function getTableDescList(string $table): array
    {
