...
title=biModel->getTableAndFields();
timeout=0
cid=1
...
- 执行biModel模块的getTableAndFields方法，参数是'SELECT u.id, p.name FROM zt_user u LEFT JOIN zt_project p ON u.id = p.createdBy' 属性tables @2
...
r($biModel->getTableAndFields('SELECT u.id, p.name FROM zt_user u LEFT JOIN zt_project p ON u.id = p.createdBy')) && p('tables') && e('2');
