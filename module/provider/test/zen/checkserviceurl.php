...
- 步骤1：缺少服务类型时直接通过校验 @1
- 步骤2：缺少服务地址时直接通过校验 @1
- 步骤3：Subversion 非法服务地址格式返回错误第url条的0属性 @『服务器地址』应当为合法的URL。
- 步骤4：GitLab 非法服务地址格式返回错误第url条的0属性 @『服务器地址』应当为合法的URL。
...
$subversionProvider      = (object)array('type' => 'Subversion', 'url' => 'not-a-valid-url', 'token' => '');
...
r($providerZen->checkServiceUrlTest($subversionProvider)) && p('url:0') && e('『服务器地址』应当为合法的URL。');   // 步骤3：Subversion 非法服务地址格式返回错误
