...
r($testcaseModel->getByList($caseIdList[1])) && p('1:module;2:module') && e('1,1'); // 批量修改用例所属模块后模块为 1。
r($testcase->batchChangeCaseModuleTest($caseIdList[1], 2)) && p() && e('1');       // 批量修改用例所属模块为 2 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:module;2:module') && e('2,2'); // 批量修改用例所属模块后模块为 2。
r($testcase->batchChangeCaseModuleTest($caseIdList[1], 3)) && p() && e('1');       // 批量修改用例所属模块为 3 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:module;2:module') && e('3,3'); // 批量修改用例所属模块后模块为 3。
