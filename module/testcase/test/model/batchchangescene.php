...
r($testcaseModel->getByList($caseIdList[1])) && p('1:scene;2:scene') && e('1,1'); // 批量修改用例所属场景后场景为 1。
r($testcase->batchChangeSceneTest($caseIdList[1], 2)) && p() && e('1');          // 批量修改用例所属场景为 2 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:scene;2:scene') && e('2,2'); // 批量修改用例所属场景后场景为 2。
r($testcase->batchChangeSceneTest($caseIdList[1], 3)) && p() && e('1');          // 批量修改用例所属场景为 3 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:scene;2:scene') && e('3,3'); // 批量修改用例所属场景后场景为 3。
