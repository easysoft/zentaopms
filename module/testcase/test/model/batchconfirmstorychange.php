...
r($testcaseModel->getByList($caseIdList[1])) && p('1:story,storyVersion;2:story,storyVersion') && e('2,1,2,1'); // 批量确认需求变动前需求 2 版本为 1。
...
r($testcaseModel->getByList($caseIdList[1])) && p('1:story,storyVersion;2:story,storyVersion') && e('2,2,2,2'); // 批量确认需求变动后需求 2 版本为 2。

$tester->dao->update(TABLE_STORY)->set('version')->eq(3)->where('id')->eq(2)->exec();                          // 更新需求 2 版本号为 3。
r($testcase->batchConfirmStoryChangeTest($caseIdList[1])) && p() && e('1');                                    // 批量确认需求变动成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:story,storyVersion;2:story,storyVersion') && e('2,3,2,3'); // 批量确认需求变动后需求 2 版本为 3。

$tester->dao->update(TABLE_STORY)->set('version')->eq(4)->where('id')->eq(2)->exec();                          // 更新需求 2 版本号为 4。
r($testcase->batchConfirmStoryChangeTest($caseIdList[1])) && p() && e('1');                                    // 批量确认需求变动成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:story,storyVersion;2:story,storyVersion') && e('2,4,2,4'); // 批量确认需求变动后需求 2 版本为 4。

$actions = $tester->dao->select('objectType,objectID,action,extra')->from(TABLE_ACTION)->orderBy('id_desc')->limit(6)->fetchAll();
