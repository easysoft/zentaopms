...
r($testcaseModel->getByList($caseIdList[1])) && p('1:branch;2:branch') && e('1,1'); // 批量修改用例所属分支后分支为 1。
r($testcase->batchChangeCaseBranchTest($caseIdList[1], 2)) && p() && e('1');       // 批量修改用例所属分支为 2 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:branch;2:branch') && e('2,2'); // 批量修改用例所属分支后分支为 2。
r($testcase->batchChangeCaseBranchTest($caseIdList[1], 3)) && p() && e('1');       // 批量修改用例所属分支为 3 成功，返回 true。
r($testcaseModel->getByList($caseIdList[1])) && p('1:branch;2:branch') && e('3,3'); // 批量修改用例所属分支后分支为 3。
