...
class personnelModelTest extends baseTest
{
    ...
    {
        $objects = $this->instance->getAccessiblePersonnel($programID, $deptID, $browseType, $queryID);
    ...
    {
        su($account);
        $canView = $this->instance->canViewProgram($programID, $account);
    ...
    {
        $objects = $this->instance->getInvest($programID);
    ...
    public function getRiskInvestTest(int $programID = 0): string|array
    {
        ...
        $projects = $tester->dao->select('id,model,type,parent,path,name')->from(TABLE_PROJECT)
            ...
            ->orderBy('id_desc')
            ->fetchAll('id');
        $accountPairs = $this->instance->getInvolvedProjects($projects);
        arsort($accountPairs);

        $objects = $this->instance->getRiskInvest($accountPairs, $projects);
    ...
    public function getIssueInvestTest(int $programID = 0): string|array
    {
        ...
        $objects = $this->instance->getIssueInvest($accountPairs, $projects);
    ...
    public function getInvolvedProjectsTest(array $projects): array
    {
        ...
        $objects = $this->instance->getInvolvedProjects($projectID);
    ...
    public function getInvolvedExecutionsTest(array $projects): array
    {
        ...
        $objects = $this->instance->getInvolvedExecutions($project);
    ...
    public function getProjectTaskInvestTest(array $projects, array $accounts): array
    {
        global $tester;
        $project = $tester->dao->select('id')->from(TABLE_PROJECT)->where('id')->in($projects)->fetchAll('id');
        $objects = $this->instance->getProjectTaskInvest($project, $accounts);
    ...
    public function getUserHoursTest(int $projectID, array $accounts): array
    {
        ...
        $objects = $this->instance->getUserHours($userTasks);
    ...
    {
        $objects = $this->instance->getWhitelist($objectID, $objectType, $orderBy, $pager);
    ...
    {
        $objects = $this->instance->getWhitelistAccount($objectID, $objectType);
    ...
    {
        $this->instance->updateWhitelist($users, $objectType, $objectID, $type, $source, $updateType);
    ...
    {
        $this->instance->deleteProductWhitelist($productID, $account);
    ...
    {
        $this->instance->deleteProgramWhitelist($programID, $account);
    ...
    {
        $this->instance->deleteProjectWhitelist($objectID, $account);
    ...
    {
        $this->instance->deleteExecutionWhitelist($executionID, $account);
    ...
    public function createMemberLinkTest(int $programID, int $deptID): int|array
    {
        $dept = new stdclass();
        $dept->id = $deptID;
        $link = $this->instance->createMemberLink($dept, $programID);
        if(dao::isError()) return dao::getError();
    ...
    {
        $acl = $this->instance->dao->select('*')->from(TABLE_ACL)->where('id')->eq($id)->fetch();
        if(!$acl) return false;

        $this->instance->unbindWhitelist($acl);
        if(dao::isError()) return false;

        $objectTable  = $acl->objectType == 'product' ? TABLE_PRODUCT : TABLE_PROJECT;
        return $this->instance->dao->select('whitelist')->from($objectTable)->where('id')->eq($acl->objectID)->fetch('whitelist');
    }
    ...
    public function updateParentWhitelistTest(string $objectType, int $objectID, array $accounts, string $source = 'sync', string $updateType = 'replace', array $deletedAccounts = array(), string $objectTable = ''): string|array
    {
        ...
        $result = $method->invoke($this->instance, $objectType, $objectID, $accounts, $source, $updateType, $deletedAccounts, $objectTable);
    ...
    public function setSelectObjectTipsTest(int $objectID, string $objectType, string $module): array
    {
        ...
        {
            $execution = $this->instance->dao->select('*')->from(TABLE_EXECUTION)->where('id')->eq($objectID)->fetch();
