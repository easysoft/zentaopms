...
<?php
/**
title=测试 devopsspaceModel::update();
timeout=0
cid=0
...
$oldSpace         = $devopsspaceModel->getById(1);

$space = new stdClass();
$space->name  = 'update space1';
$space->owner = 'test12';

r($devopsspaceModel->update($oldSpace, $space)) && p('0:old;0:new') && e('space1,update space1'); //正常修改

$oldSpace = $devopsspaceModel->getById(2);
r($devopsspaceModel->update($oldSpace, $space)) && p()         && e('0'); //空间名重复
r($tester->dao->getError())                     && p('name:0') && e('『name』已经有『update space1』这条记录了。如果您确定该记录已删除，请到后台-系统设置-回收站还原。');//空间名重复信息
$space->name = '';
r($devopsspaceModel->update($oldSpace, $space)) && p()         && e('0'); //空间名为空
r($tester->dao->getError())                     && p('name:0') && e('『name』不能为空。');//空间名为空信息
$space->name  = 'test2';
$space->owner = '';
r($devopsspaceModel->update($oldSpace, $space)) && p()          && e('0');//owner为空
