...
class commonModel extends model
{
   ...
   /**
    ...
    /**
     * Print top bar for ZIN demo.
     *
     * @static
     * @access public
     * @return array
     */
    public static function printUserBarZin()
    {
        global $lang, $app;
        ...
        if(isset($app->user))
        {
            ...
            if(!$isGuest)
            {
                $menus[] = array
                (
                    'url'    => helper::createLink('my', 'profile', '', '', true),
                    'avatar' => $app->user->avatar,
                    'text'   => empty($app->user->realname) ? $app->user->account : $app->user->realname,
                    'role'   => isset($lang->user->roleList[$app->user->role]) ? $lang->user->roleList[$app->user->role] : '',
                );
                $menus[] = array('text' => 'divider');
                ...
                if($app->config->vision === 'rnd')
                {
                    if(!commonModel::isTutorialMode())
                    {
                        $menus[] = array
                        (
                            'url'  => helper::createLink('tutorial', 'start'),
                            'icon' => 'guide',
                    ...
                    $menus[] = array
                    (
                        'url'  => helper::createLink('my', 'preference', 'showTip=false', '', true),
                        'icon' => 'controls',
                ...
                if(common::hasPriv('my', 'changePassword'))
                {
                    $menus[] = array
                    (
                        'url'  => helper::createLink('my', 'changepassword', '', '', true),
                        'icon' => 'cog-outline',
            ...
            foreach($app->lang->themes as $key => $value)
            {
                $item = array
                (
                    'url'  => "javascript:selectTheme(\"$key\")",
                    'text' => $value,
                    'attr' => array('data-value' => $key),
                );
                if($app->cookie->theme == $key) $item['attr']['class'] = 'selected';
                $items[] = $item;
            }
            $menus[] = array
            (
            ...
            foreach($app->config->langs as $key => $value)
            {
                $item = array
                (
                    'url'  => "javascript:selectLang(\"$key\")",
                    'text' => $value,
                    'attr' => array('data-value' => $key),
                );
                if($app->cookie->lang == $key) $item['attr']['class'] = 'selected';
                $items[] = $item;
            }
            $menus[] = array
            (
            ...
            if($isGuest)
            {
                $menus[] = array
                (
                    'url'  => helper::createLink('user', 'login'),
                    'text' => $lang->login,
                );
            }
            else
            {
                $menus[] = array
                (
                    'url'  => helper::createLink('user', 'logout'),
                    'icon' => 'exit',
                    'text' => $lang->logout,
                );
            }
            ...
            if(common::hasPriv($module, $createMethod))
            {
                ...
                switch($objectType)
                {
                    case 'doc':
                        ...
                        $isOnlyBody   = true;
                        $attr         = array('class'=>'iframe', 'data-width'=>'700px');
                        break;
                    case 'project':
                        if($config->vision == 'lite')
                        {
                    ...
                    case 'kanbanspace':
                        $isOnlyBody = true;
                        $attr       = array('class'=>'iframe', 'data-width'=>'75%');
                        break;
                    case 'kanban':
                        $isOnlyBody = true;
                        $attr       = array('class'=>'iframe', 'data-width'=>'75%');
                ...
                $menus[] = array(
                    'url'  => helper::createLink($module, $createMethod, $params, '', $isOnlyBody),
                    'icon' => $objectIcon,
                    'text' => $lang->createObjects[$objectType],
                    'attr' => $attr
                );
            }
        }
