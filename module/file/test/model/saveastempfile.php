...
r($fileTest->saveAsTempFileTest($emptyFile) === '') && p() && e('1'); // 测试传入空realPath的文件对象
r($fileTest->saveAsTempFileTest($validFile)) && p() && e('/tmp/test.txt'); // 测试fs存储类型下的有效文件路径

// 设置文件存储类型为s3
$fileTest->objectModel->config->file->storageType = 's3';
r($fileTest->saveAsTempFileTest($validFile) === '') && p() && e('1'); // 测试s3存储类型下的文件处理

// 重新设置为fs进行更多测试
$fileTest->objectModel->config->file->storageType = 'fs';
r($fileTest->saveAsTempFileTest($nullPathFile) === '') && p() && e('1'); // 测试空字符串realPath的处理

// 测试其他存储类型
$fileTest->objectModel->config->file->storageType = 'oss';
r($fileTest->saveAsTempFileTest($validFile) === '') && p() && e('1'); // 测试其他存储类型下的文件处理
