...
class codescanModel extends model
{
     ...
     * @return int|false
     */
    public function createTrigger(int $repoID, int $planID, object $formData): int|false
    {
        $this->loadModel('gitfox');
        $url = "/scan/repos/{$repoID}/scanplans/{$planID}/triggers";
        foreach($formData->solutions as &$solutionID) $solutionID = (int)$solutionID;

        $result = $this->gitfox->request($url, 'POST', $formData);
        if(dao::isError()) return false;

        $this->createTriggerConditions($repoID, $planID, $result->id, $formData->conditions);
        return (int)$result->id;
    }

    /**
     * 通过API获取触发器详情。
     * Api get scan trigger.
     *
     * @param  int   $planID
     ...
     * @return array|object
     */
    public function getScanTrigger(int $serviceRepoID, int $planID, int $triggerID): array|object
    {
        $url    = "/scan/repos/{$serviceRepoID}/scanplans/{$planID}/triggers/{$triggerID}";
        $result = $this->loadModel('gitfox')->request($url, 'GET');
        return $result ?: array();
    }
     ...
     * @return bool
     */
    public function editTrigger(int $serviceRepoID, int $planID, int $triggerID, object $formData): bool
    {
        $url    = "/scan/repos/{$serviceRepoID}/scanplans/{$planID}/triggers/{$triggerID}";
        $result = $this->loadModel('gitfox')->request($url, 'PATCH', $formData);
        if(dao::isError()) return false;
        $this->createTriggerConditions($serviceRepoID, $planID, $result->id, $formData->conditions);
        return !dao::isError();
    }

    /**
     * 通过API获取扫描计划触发器列表。
     * Api get scan plan triggers.
     *
     * @param  int   $planID
     * @param  int   $repoID
     * @param  array $params
     ...
     * @return array|object
     */
    public function getScanPlanTriggers(int $planID, int $repoID, array $params): array|object
    {
        $url    = "/scan/repos/{$repoID}/scanplans/{$planID}/triggers";
     ...
     * @return array|object
     */
    public function getTriggerConditions(int $repoID, int $planID, int $triggerID): array|object
    {
        $url    = "/scan/repos/{$repoID}/scanplans/{$planID}/triggers/{$triggerID}/conditions";
        $result = $this->loadModel('gitfox')->request($url, 'GET');
        return $result ?: array();
    }
    ...
    /**
     * 获取触发器最新扫描结果
     * Get trigger latest scan.
     *
     * @param  object $latestScan
     * @access public
     * @return string
     */
    public function getTriggerLatestScan(object $latestScan): string
    {
        $latestScanResult = '';
        if(!empty($latestScan->latestScanTime))
        {
            $currentTime = time();
            $timeDiff    = $currentTime - $latestScan->latestScanTime;
            ...
            if($timeDiff < 3600)
            {
                for($min = 1; $min <= 59; $min++)
                {
                    if($timeDiff < $min * 60)
                    {
                        $latestScanResult = sprintf($this->lang->codescan->withinMinutes, $min);
                        break;
                    }
                }
            }
            elseif($timeDiff < 86400 && $timeDiff > 3600)
            {
                for($hours = 1; $hours <= 23; $hours++)
                {
                    if($timeDiff < $hours * 3600)
                    {
                        $latestScanResult = sprintf($this->lang->codescan->withinHours, $hours);
                        break;
                    }
                }
            }
            ...
            {
                $latestScanResult = date("Y-m-d", $latestScan->latestScanTime);
            }
        }

        if(empty($latestScan->latestExecStatus) || empty($latestScan->result)) return $latestScanResult;

        if(in_array($latestScan->latestExecStatus, array('wait', 'running', 'success')) && $latestScan->result == 'pass')
        {
            $latestScanResult = "<span class='label success-pale mr-1'>{$this->lang->codescan->latestScanResultList['pass']}</span>" . $latestScanResult;
        }
        elseif(in_array($latestScan->latestExecStatus, array('wait', 'running', 'success')) && $latestScan->result == 'fail')
        {
            $latestScanResult = "<span class='label danger-pale mr-1'>{$this->lang->codescan->latestScanResultList['failed']}</span>" . $latestScanResult;
        }
        else
        {
            $latestScanResult = "<span class='label warning-pale mr-1'>{$this->lang->codescan->latestExecStatusList['failed']}</span>" . $latestScanResult;
        }
     ...
     * @return array|object
     */
    public function getTrigger(int $triggerID): array|object
    {
        $apiRoot = $this->loadModel('gitfox')->getApiRoot();
        $url     = sprintf($apiRoot->url, "/scan/triggers/{$triggerID}");
     ...
     * @return void
     */
    public function deleteTrigger(int $repoID, int $planID, int $triggerID)
    {
        $url    = "/scan/repos/{$repoID}/scanplans/{$planID}/triggers/{$triggerID}";
        $result = $this->loadModel('gitfox')->request($url, 'DELETE');
        return !dao::isError();
    }
     ...
     * @return bool
     */
    public function createTriggerConditions(int $repoID, int $planID, int $triggerID, array $conditions): bool
    {
        if(empty($conditions)) return true;

        $apiRoot = $this->loadModel('gitfox')->getApiRoot();
        $url     = sprintf($apiRoot->url, "/repos/{$repoID}/scanplans/{$planID}/triggers/{$triggerID}/conditions/new");
    ...
    /**
     * 批量删除触发器结果通过标准。
     * Batch delete trigger conditions.
     *
     * @param  int    $repoID
     * @param  int    $planID
     * @param  int    $triggerID
     ...
     * @return bool
     */
    public function deleteTriggerConditions(int $repoID, int $planID, int $triggerID, array $conditions): bool
    {
        $apiRoot = $this->loadModel('gitfox')->getApiRoot();
        $url     = sprintf($apiRoot->url, "/repos/{$repoID}/scanplans/{$planID}/triggers/{$triggerID}/conditions/delete");

        $data = new stdClass();
        $data->condition_ids = $conditions;
    ...
    public function changeIssueState(int|array $issueIdList, string $status, string $solution = null, string $solutionDate = null, int $ignoreDate = 0): bool
    {
        ...
        $repoPair = array_column($this->loadModel('repo')->getGitFoxRepos(), 'id', 'serviceProject');
