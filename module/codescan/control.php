...
class codescan extends control
{
    ...
    public function task(int $repoID = 0, int $serviceRepoID = 0, int $planID = 0, string $type = 'all', string $queryID = '', string $orderBy = 'id_desc', int $recPerPage = 20, int $pageID = 1)
    {
        ...
        $taskList = $this->codescan->getScanTasks(!empty($repo) ? (int)$repo->id : 0, $planID, (array)$params);
     ...
     * @return void
     */
    public function confirmIssue(int $issueID = 0)
    {
        if(empty($issueID) && $this->post->issueIdList) $issueID = $this->post->issueIdList;

        $result = $this->codescan->changeIssueState($issueID, 'todo');
        if(!$result) $this->codescanZen->responseError($this->lang->codescan->notice->changeStateFailed);

        $this->sendSuccess(array('message' => $this->lang->saveSuccess, 'load' => true));
    }
     ...
     * @return void
     */
    public function activeIssue(int $issueID = 0)
    {
        if(empty($issueID) && $this->post->issueIdList) $issueID = $this->post->issueIdList;
        $bugList = $this->codescan->getLinkedBugList($issueID, 'closed');
        $linkBugIssueID   = array_keys(array_column($bugList, 'id', 'issueKey'));
        $unlinkBugIssueID = array_diff(is_array($issueID) ? $issueID : array($issueID), $linkBugIssueID);

        if(!empty($linkBugIssueID))
        {
            $result = $this->codescan->changeIssueState($linkBugIssueID, 'solving', '', '');
            if(!$result) $this->codescanZen->responseError($this->lang->codescan->notice->changeStateFailed);
        }

        if(!empty($unlinkBugIssueID))
        {
            $result = $this->codescan->changeIssueState(array_values($unlinkBugIssueID), 'wait', '', '');
            if(!$result) $this->codescanZen->responseError($this->lang->codescan->notice->changeStateFailed);
        }
        ...
        if(!empty($bugList))
        {
            ...
            {
                $_POST['id'][$bugID]          = $bugID;
                $_POST['status'][$bugID]      = $bug->status;
                $_POST['openedBuild'][$bugID] = $bug->openedBuild ? $bug->openedBuild : 'trunk';
                $_POST['assignedTo'][$bugID]  = $bug->resolvedBy;
            }

            $this->session->set('bugList', true);
            return $this->fetch('bug', 'batchActivate', 'productID=0');
        }
     ...
     * @return void
     */
    public function ignoreIssue(int $issueID = 0)
    {
        if($_POST)
        {
            if(empty($issueID) && !empty($_COOKIE['issueIdList'])) $issueID = explode('|', $_COOKIE['issueIdList']);
            if(!$this->post->ignoreDate) $this->sendError(array('ignoreDate' => sprintf($this->lang->error->notempty, $this->lang->codescan->ignoreDate)));

            $ignoreDate = $this->post->ignoreDate == '-1' ? -1 : strtotime(" + {$this->post->ignoreDate}");
            $resolved   = $this->post->ignoreDate == '-1' ? 'willnotfix' : 'postponed';
            $result     = $this->codescan->changeIssueState($issueID, 'closed', $resolved, helper::now(), $ignoreDate);
            if(!$result) $this->codescanZen->responseError($this->lang->codescan->notice->changeStateFailed);

            $this->sendSuccess(array('message' => $this->lang->saveSuccess, 'load' => true));
        }

        $this->view->title = $this->lang->codescan->ignoreIssue;
        $this->display();
    }
