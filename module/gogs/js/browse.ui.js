...
window.renderCell = function(result, {col, row})
{
    if(col.name === 'name')
    {
        ...
        if(canBrowseProject) result[0] = {html:'<a href="' + browseProjectLink + '&onlybody=yes' + '" data-toggle="modal" data-width="40%">' + row.data.name + '</a>', style:{flexDirection:"column"}};
...
window.confirmDelete = function(gogsID)
{
    ...
    {
        if(res) $.ajaxSubmit({url: $.createLink('gogs', 'delete', 'gogsID=' + gogsID)});
    });
}
