...
$help->bug->module         = '所属模块|设定Bug所属的模块。';
...
$help->bug->severity       = '严重程度|Bug的严重程度，但和优先级不能等同。';
...
$help->bug->steps          = '重现步骤|Bug的重现步骤，也是bug最为重要的信息，一定要将完整的重现步骤写清楚。如果有抓图，一定提供抓图。';
$help->bug->status         = 'bug状态|Bug当前所处的状态，禅道里面Bug总共为Active, Resolved, Closed三个状态。';
$help->bug->mailto         = '抄送给|当前bug所有的操作都会抄送给该列表，抄送给可以是多个人。';
$help->bug->openedby       = '由谁创建|Bug的创建者。';
$help->bug->openedbuild    = '影响版本|Bug影响的版本，可以选择多个。';
...
$help->build->name      = '名称编号|Build的编号，团队应当建立自己的配置管理规范';
...
$help->company->guest    = '匿名登录|是否运行匿名登录。如果允许的话，需要在组织视图中，添加一个guest分组，并为改组分配相应的权限。';
...
$help->testtask->build          = 'build|需要测试哪个Build，非常重要。测试人员测试的都应该是固定的东西，不应该是时刻变化的系统。';
