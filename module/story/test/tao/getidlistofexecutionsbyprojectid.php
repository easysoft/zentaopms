...
$projectstory->project->range('21-25');
...
r(count($storyModel->getIdListOfExecutionsByProjectID('linkedexecution', 0)))    && p() && e('10'); //请求类型是 linkedexecution，不传入项目。
r(count($storyModel->getIdListOfExecutionsByProjectID('linkedexecution', 11)))   && p() && e('10'); //请求类型是 linkedexecution，项目下有执行。
r(count($storyModel->getIdListOfExecutionsByProjectID('linkedexecution', 12)))   && p() && e('0');  //请求类型是 linkedexecution，项目下无执行。
r(count($storyModel->getIdListOfExecutionsByProjectID('unlinkedexecution', 0)))  && p() && e('10'); //请求类型是 unlinkedexecution，不传入项目。
r(count($storyModel->getIdListOfExecutionsByProjectID('unlinkedexecution', 11))) && p() && e('10'); //请求类型是 unlinkedexecution，项目下有执行。
