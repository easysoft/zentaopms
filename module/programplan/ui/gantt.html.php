...
if($app->rawModule == 'programplan' && !$isFromDoc)
{
    ...
    foreach($versions as $version)
    {
        ...
        if($version->reviewType == 'gantt' && $version->status != 'tmpGantt')
        {
            ...
            $item['actions'] = array();
            if(hasPriv('programplan', 'rollbackGanttVersion')) $item['actions'][] = array('icon' => 'undo',  'hint' => $lang->programplan->rollbackGanttVersion, 'url' => createLink('programplan', 'rollbackGanttVersion', "versionID={$version->id}"), 'data-confirm' => $lang->programplan->rollbackTip, 'className' => 'ajax-submit');
