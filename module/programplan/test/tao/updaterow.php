...
r((array)$rawPlan)  && p('name,attribute,version') && e('执行3-1,``,1');  // 检查未编辑前的数据。
...
r((array)$plan)  && p('name,version') && e('修改后的阶段,2');  // 测试修改plan的 name 和 version 值。
r(!empty($spec)) && p()               && e('1');               // 测试查看项目下的spec信息。

$plan = $programplan->updateRowTest($planID, $projectID, array('attribute' => 'devel'));
r((array)$plan)  && p('name,attribute,version') && e('修改后的阶段,devel,2');  // 测试修改plan的 attribute 值。
