...
class programplanModelTest extends baseTest
{
    ...
    {
        $object = $this->instance->getByID($planID);
    ...
    {
        $objects = $this->instance->getStage($executionID, $productID, $browseType, $orderBy);
    ...
    {
        $objects = $this->instance->getPlans($executionID, $productID, $orderBy);
    ...
    {
        $objects = $this->instance->getPairs($executionID, $productID, $type);
    ...
    public function getTotalPercentTest(int $stageID, bool $parent = false): int|float
    {
        ...
        $result = $this->instance->getTotalPercent($stage, $parent);
    ...
    {
        $plans   = $this->instance->getByList($planIDList);
        $objects = $this->instance->processPlans($plans);
    ...
    {
        $plan   = $this->instance->getByID($planID);
        $object = $this->instance->processPlan($plan);
    ...
    {
        $count = $this->instance->getDuration($begin, $end);
    ...
    public function createTest($param = array(), $projectID = 0, $productID = 0, $parentID = 0)
    {
        ...
        $objects = $this->instance->create($plans, $projectID, $productID, $parentID);

        unset($_POST);

        if(dao::isError()) return dao::getError();

        $objects = $this->instance->dao->select('*')->from(TABLE_PROJECT)->where('parent')->eq($plans[0]->parent)->andWhere('type')->eq('stage')->fetchAll();
        return $objects;
    }
    ...
    {
        $this->instance->setTreePath($planID);
    ...
    public function updateTest($planID, $projectID, $param = array())
    {
        ...
        $this->instance->update($planID, $projectID, $newPlan);

        if(dao::isError()) return dao::getError();
        return $this->instance->dao->select('*')->from(TABLE_PROJECT)->where('id')->eq($planID)->fetch();
    }
    ...
    public function updateRowTest($planID, $projectID, $param = array()): array|object
    {
        ...
        $this->instance->updateRow($planID, $projectID, $newPlan);
    ...
    {
        $result = $this->instance->isCreateTask($planID);
    ...
    {
        $objects = $this->instance->getParentStageList($executionID, $planID, $productID);
    ...
    {
        $objects = $this->instance->getParentChildrenTypes($parentID);
    ...
    {
        $objects = $this->instance->isTopStage($planID);
    ...
    {
        $this->instance->updateSubStageAttr($planID, $attribute);
        if(dao::isError()) return false;
    ...
    {
        $objects = $this->instance->getSelfAndChildrenList($executionIdList);
    ...
    {
        $objects = $this->instance->getSiblings($executionIdList);
    ...
    {
        $result = $this->instance->computeProgress($stageID, $action, $isParent);
    ...
    {
        $gantt = $this->instance->getDataForGantt($executionID, $productID, $baselineID, $selectCustom, $returnJson);
    ...
    {
        $tasks = $this->instance->getGanttTasks($projectID, $plans, $browseType, $queryID);
    ...
    {
        $this->instance->syncParentData($executionID, $parentID);
        return $this->instance->dao->select('*')->from(TABLE_TASK)->where('execution')->eq($executionID)->fetchAll();
    }
    ...
    {
        $result = $this->instance->checkLeafStage($stageID);
    ...
    {
        $result = $this->instance->getStageAttribute($stageID);
    ...
    {
        $result = $this->instance::isClickable($plan, $action);
