$(function()
{
    $('.productstatistic-block.block-sm .nav.nav-tabs').find('.nav-item.nav-switch').removeClass('active');
    $('.productstatistic-block.block-sm .nav.nav-tabs').find('.nav-item.nav-switch').each(function()
    {
...
$('.productstatistic-block.block-sm .nav.nav-tabs').on('show', function(event, info)
{
