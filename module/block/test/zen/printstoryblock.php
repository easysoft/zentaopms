...
- 测试步骤5:测试显示需求数量限制为0时应该显示所有需求属性storiesCount @19
...
r($blockTest->printStoryBlockTest($block1)) && p('storiesCount') && e('5'); // 测试步骤1:测试正常显示指派给我的需求类型assignedTo,限制5条
r($blockTest->printStoryBlockTest($block2)) && p('storiesCount') && e('3'); // 测试步骤2:测试显示由我创建的需求类型openedBy,限制3条
r($blockTest->printStoryBlockTest($block3)) && p('storiesCount') && e('10'); // 测试步骤3:测试显示我评审的需求类型reviewedBy,限制10条
r($blockTest->printStoryBlockTest($block4)) && p('storiesCount') && e('8'); // 测试步骤4:测试显示需求类型closedBy,限制8条
r($blockTest->printStoryBlockTest($block5)) && p('storiesCount') && e('19'); // 测试步骤5:测试显示需求数量限制为0时应该显示所有需求