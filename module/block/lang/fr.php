...
$lang->block->flowchart['project'] = array('Project Manager', "Add {$lang->projectCommon}s and " . $lang->execution->common . 's', 'Maintain Teams', 'Link Stories', 'Create Tasks', 'Track');
...
$lang->block->tooltips['ev']                = "1) The task status is done, and the estimated work hours are accumulated.<br/>
2) The task status is closed and the reason for closing is done, and the estimated work hours areaccumulated.<br/>
3) The task status is in doing, suspended, and the estimated work hours are accumulated * progress of task.<br/>
Conditions: If there is a waterfall {$lang->projectCommon}, filter parent tasks, tasks with 0 spent hours, deleted tasks, canceled tasks, tasks linked with deleted {$lang->execution->common}, and deleted {$lang->projectCommon}. Progress of task = Hours Spent on task / (Hours Spent on task + Hours Remaining on task).";
$lang->block->tooltips['pv']                = "If the task end date ≤ the end date of the week, the estimated hours are accrued.<br/>
If the estimated start date of the task ≤ the end date of the week and the estimated end date ＞the end date of the week, then Cumulative Estimated Hours = (Estimated hours for task ÷ Number of days in the task's work period) x Number of days from the estimated start of the task to the end date of the week.<br/>
Conditions: Filter parent tasks, deleted tasks, canceled tasks, deleted executed tasks, and deleted {$lang->projectCommon}. If the task doesn’t have an estimated start date, it will default to the planned start date of the phase the task belongs to. If the task does not have an estimated end date, it will default to the planned finish date of the phase the task belongs to. Time will only be counted for the working days set in the Admin.";
