...
- 测试获取不存在插件包的文件列表 @0
- 测试空插件代号的情况 @0
- 测试包含db和doc目录的插件包testpkg2(验证排除功能) @2
- 测试特殊字符插件代号的处理 @0
...
r(count($extensionTest->getFilesFromPackageTest('testpkg1'))) && p() && e('5'); // 测试获取存在插件包testpkg1的文件列表
...
r(count($extensionTest->getFilesFromPackageTest('testpkg2'))) && p() && e('2'); // 测试包含db和doc目录的插件包testpkg2(验证排除功能)
