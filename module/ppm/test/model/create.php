...
- 执行ppmModel模块的createTest方法，参数是$apiCreatePPM  @6402
- 执行ppmModel模块的createTest方法，参数是$noReviewerPPM  @6403
- 执行ppmModel模块的createTest方法，参数是$crossRepoBranchPPM  @6404
- 执行ppmModel模块的createTest方法，参数是$approvalFlowPPM  @6405
- 执行instance模块的fetchByID方法，参数是6401 
 - 属性status @opened
 - 属性title @Create PPM 6401
- 执行$ppmModel->instance->dao->select('count(*)')->from(TABLE_PPM)->where('repoID')->eq(6401)->fetch('count(*)' @5
...
r($ppmModel->createTest($apiCreatePPM)) && p() && e('6402');
r($ppmModel->createTest($noReviewerPPM)) && p() && e('6403');
r($ppmModel->createTest($crossRepoBranchPPM)) && p() && e('6404');
r($ppmModel->createTest($approvalFlowPPM)) && p() && e('6405');
r($ppmModel->instance->fetchByID(6401)) && p('status,title') && e('opened,Create PPM 6401');
r((int)$ppmModel->instance->dao->select('count(*)')->from(TABLE_PPM)->where('repoID')->eq(6401)->fetch('count(*)')) && p() && e('5');