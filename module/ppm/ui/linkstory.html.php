...
/**
 * The linkstory file of ppm module of ZenTaoPMS.
...
jsVar('sortLink', createLink($moduleName, 'linkStory', "id=$id&repoID=$repoID&browseType=$browseType&param=$param&orderBy={orderBy}&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}"));
...
$footToolbar = array(
    ...
    (
        array(
            'text'      => $lang->ppm->linkStory,
            'className' => 'batch-btn ajax-btn',
            'data-app'  => $app->tab,
            'data-url'  => createLink($moduleName, 'linkStory', "id=$id&repoID=$repoID&browseType=$browseType&param=$param&orderBy=$orderBy")
        )
    ),
...
(
    ...
    set::footer(array('checkbox', 'toolbar', array('html' => html::a(inlink('link', "id={$id}&type=story"), $lang->goback, '', "class='btn size-sm'")), 'flex', 'pager')),
