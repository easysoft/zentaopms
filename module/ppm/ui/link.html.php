...
/**
 * The link view file of ppm module of ZenTaoPMS.
...
jsVar('sortLink', createLink($moduleName, 'link', "id={$ppm->id}&type={type}&orderBy={orderBy}"));
...
$storyCols['actions']['list']['unlink']['url']          = $this->createLink($moduleName, 'unlink', "id={$ppm->id}&type=story&linkID={id}&confirm=yes");
...
$bugCols['actions']['list']['unlink']['url']          = $this->createLink($moduleName, 'unlink', "id={$ppm->id}&type=bug&linkID={id}&confirm=yes");
...
$taskCols = $config->ppm->taskDtable->fieldList;
$taskCols['assignedTo']['currentUser'] = $app->user->account;

$taskCols['actions'] = $actionMenu;
$taskCols['actions']['list']['unlink']['data-confirm'] = $lang->ppm->confirmUnlinkTask;
$taskCols['actions']['list']['unlink']['url']          = $this->createLink($moduleName, 'unlink', "id={$ppm->id}&type=task&linkID={id}&confirm=yes");
$taskCols['actions']['list']['unlink']['hint']         = $lang->ppm->unlinkTask;
...
if(common::hasPriv($moduleName, 'linkStory'))
{
    $linkStoryBtn = btn(set(array(
        ...
        'icon'     => 'link',
        'data-url' => createLink($moduleName, 'linkStory', "id={$ppm->id}&repoID={$ppm->repoID}&browseType=&param={$param}&orderBy={$orderBy}"),
...
if(common::hasPriv($moduleName, 'linkBug'))
{
    $linkBugBtn = btn(set(array(
        ...
        'icon'     => 'link',
        'data-url' => createLink($moduleName, 'linkBug', "id={$ppm->id}&repoID={$ppm->repoID}&browseType=&param={$param}&orderBy={$orderBy}"),
        'class'    => 'link mr-actions',
        'type'     => 'primary',
        'onclick'  => 'showLink(this)'
    )));
...
if(common::hasPriv($moduleName, 'linkTask'))
{
    $linkTaskBtn = btn(set(array(
        'text'     => $lang->ppm->linkTask,
        'icon'     => 'link',
        'data-url' => createLink($moduleName, 'linkTask', "id={$ppm->id}&repoID={$ppm->repoID}&browseType=&param={$param}&browseType=unclosed&orderBy={$orderBy}"),
        'class'    => 'link mr-actions',
        'type'     => 'primary',
        'onclick'  => 'showLink(this)'
    )));
