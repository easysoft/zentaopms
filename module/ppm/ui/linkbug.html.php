...
/**
 * The linkbug file of ppm module of ZenTaoPMS.
...
jsVar('sortLink', createLink($moduleName, 'linkBug', "id=$id&repoID=$repoID&browseType=$browseType&param=$param&orderBy={orderBy}&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}"));
...
$footToolbar = array(
    ...
    (
        array(
            ...
            'data-app'  => $app->tab,
            'data-url'  => helper::createLink($moduleName, 'linkBug', "id=$id&repoID=$repoID&browseType=$browseType&param=$param&orderBy=$orderBy")
        )
    ),
...
(
    ...
    set::footer(array('checkbox', 'toolbar', array('html' => html::a(inlink('link', "id=$id&type=bug"), $lang->goback, '', "class='btn size-sm'")), 'flex', 'pager')),
