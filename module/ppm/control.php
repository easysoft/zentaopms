...
class ppm extends control
{
    ...
    public function create(int $repoID = 0, int $objectID = 0, string $sourceBranch = '', string $targetBranch = '')
    {
        ...
        if($mergeCheckMessage)
        {
            ...
            if(!empty($conflictFiles)) $message .= $this->lang->ppm->checkConflicts;
            if(!$checkSourceBranch)    $message .= $this->lang->ppm->checkSourceBranch;
            if(!$checkTargetBranch)    $message .= $this->lang->ppm->checkTargetBranch;
        }
    ...
    public function view(int $id, string $type = 'basic', string $param = '', int $recTotal = 0, int $recPerPage = 20, int $pageID = 0)
    {
        ...
        $this->view->reviewResult      = $this->ppm->getReviewResult($reviewers, empty($flow) ? array() : $flow);
        $this->view->repo              = $repo;
