...
))) && p() && e(' where tt.`status` = \'active\'');                                                               // 步骤2：正常单个过滤条件
r($pivotTest->getConnectSQLTest(array(
    'name' => array('field' => 'name', 'operator' => '=', 'value' => "'test'"),
    'score' => array('field' => 'score', 'operator' => '>', 'value' => '100')
))) && p() && e(' where tt.`name` = \'test\' and tt.`score` > 100');                                              // 步骤3：多个过滤条件组合
r($pivotTest->getConnectSQLTest(array(
    0 => array('from' => 'query', 'field' => 'name')
))) && p() && e('0');                                                                                              // 步骤4：包含from键的过滤条件
r($pivotTest->getConnectSQLTest(array(
...
))) && p() && e(' where tt.`title` = \'test\'s data\' and tt.`count` >= 0');                                     // 步骤6：特殊字符和边界值处理
r($pivotTest->getConnectSQLTest(array(
...
))) && p() && e(' where tt.`category` IN (\'bug\',\'task\',\'story\') and tt.`deleted` = \'0\' and tt.`created` > \'2024-01-01\''); // 步骤7：复杂数据类型的过滤条件