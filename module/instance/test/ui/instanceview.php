...
$type = 'GitFox';
r($tester->instanceView($type)) && p('message,status') && e('检查GitFox应用成功,SUCCESS'); //检查iGitFox应用页面是否可以正常展示
$type = 'GitLab';
r($tester->instanceView($type)) && p('message,status') && e('检查GitLab应用成功,SUCCESS'); //检查iGitLab应用页面是否可以正常展示
$type = 'Gogs';
