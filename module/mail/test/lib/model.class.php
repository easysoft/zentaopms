...
class mailModelTest extends baseTest
{
    ...
    public function addQueueTest($toList = '', $subject = '', $body = '', $ccList = '', $includeMe = false)
    {
        ...
        $result = $this->instance->addQueue($toList, $subject, $body, $ccList, $includeMe);
        if(dao::isError()) return dao::getError();
    ...
    {
        // 调用真实的autoDetect方法
        $result = $this->instance->autoDetect($email);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->isClickable($item, $method);
        if(dao::isError()) return dao::getError();
