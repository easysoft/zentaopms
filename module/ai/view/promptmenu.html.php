...
?>
  ...
  ?>
    <?php
      ...
      if(count($prompts) > 1)
      {
        $btnName = sprintf($this->lang->ai->promptMenu->dropdownTitle, isset($this->lang->ai->moduleList[$module]['common']) ? $this->lang->ai->moduleList[$module]['common'] : '');
        $html .= '<div class="prompts dropdown' . ((isset($menuOptions->class) ? ' ' . $menuOptions->class : '') . (isset($menuOptions->dropdownClass) ? ' ' . $menuOptions->dropdownClass : '')) . '"><button class="btn btn-link' . (isset($menuOptions->buttonClass) ? ' ' . $menuOptions->buttonClass : '') . '" type="button" data-toggle="dropdown">' . $btnName . ' <i class="icon-caret-down"></i></button><ul class="dropdown-menu pull-right">';
