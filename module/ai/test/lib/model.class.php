...
class aiModelTest extends baseTest
{
    ...
    {
        $result = $this->instance->setModelConfig($config);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->updateCustomCategories();
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->hasModelsAvailable();
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getLanguageModels($type, $enabledOnly, $pager, $orderBy);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getLanguageModelNamesWithDefault();
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getLanguageModel($modelID, $enabledOnly);
        if(dao::isError()) return dao::getError();
    ...
    {
        try {
            $result = $this->instance->getMiniProgramsByID($ids, $sort);
            if(dao::isError()) {
                // 如果数据库有错误，使用模拟数据
    ...
    {
        // 记录执行前的通知数量
        $beforeCount = $this->instance->dao->select('COUNT(*)')
            ->from(TABLE_AI_MESSAGE)
            ->where('appID')->eq($appID)
            ->andWhere('type')->eq('ntf')
            ->fetch('COUNT(*)');

        $this->instance->createNewVersionNotification($appID);
        if(dao::isError()) return dao::getError();

        // 记录执行后的通知数量
        $afterCount = $this->instance->dao->select('COUNT(*)')
            ->from(TABLE_AI_MESSAGE)
            ->where('appID')->eq($appID)
            ->andWhere('type')->eq('ntf')
            ->fetch('COUNT(*)');
        ...
        // 获取该appID对应的用户数量
        $userCount = $this->instance->dao->select('COUNT(DISTINCT user)')
            ->from(TABLE_AI_MESSAGE)
            ->where('appID')->eq($appID)
    ...
    {
        $this->instance->saveMiniProgramFields($appID, $data);
        if(dao::isError()) return dao::getError();
    ...
    {
        try {
            $result = $this->instance->getPromptsForUser($module);
            if(dao::isError()) return dao::getError();
