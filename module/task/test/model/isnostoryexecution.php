...
r($task->isNoStoryExecutionTest($executionIdList[1]))   && p() && e('1'); // 测试执行为短期执行 阶段类型request为的执行是否有需求列表
r($task->isNoStoryExecutionTest($executionIdList[2]))   && p() && e('1'); // 测试执行为短期执行 阶段类型为review的执行是否有需求列表
r($task->isNoStoryExecutionTest($executionIdList[3]))   && p() && e('0'); // 测试执行为长期执行 阶段类型为mix的执行是否有需求列表
r($task->isNoStoryExecutionTest($executionIdList[4]))   && p() && e('1'); // 测试执行为长期执行 阶段类型request为的执行是否有需求列表
r($task->isNoStoryExecutionTest($executionIdList[5]))   && p() && e('1'); // 测试执行为长期执行 阶段类型为review的执行是否有需求列表
