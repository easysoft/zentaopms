...
r($task->isClickableTest($task14, 'close'))          && p('1') && e("1"); //计算任务为子任务 状态为doing 能否进行close操作
r($task->isClickableTest($task15, 'batchcreate'))    && p('2') && e("1"); //计算任务为子任务 状态为done 能否进行batchcreate操作
r($task->isClickableTest($task16, 'recordworkhour')) && p('2') && e("2"); //计算任务为子任务 状态为pause 能否进行recordworkhour操作
