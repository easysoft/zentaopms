...
$task->instance->dao->update(TABLE_TASK)->set("path = concat(',', id, ',')")->exec();
$task->instance->dao->update(TABLE_TASK)->set("path = concat(',', parent, ',', id, ',')")->where('parent')->gt('0')->exec();
$task->instance->dao->update(TABLE_TASK)->set('isParent')->eq('1')->where('parent')->eq('-1')->exec();
$task->instance->dao->update(TABLE_TASK)->set('parent')->eq('0')->where('parent')->eq('-1')->exec();
...
$task->instance->dao->update(TABLE_ACTION)->set('extra')->eq('')->where('objectType')->eq('task')->andWhere('objectID')->eq(7)->exec();
r($task->updateChildrenStatusTest(6, 'doing')) && p('id,status') && e('7,pause'); //子任务的状态为pause，父任务的状态为doing，不是自动暂停任务,不更新子任务

$task->instance->dao->update(TABLE_ACTION)->set('extra')->eq('auto')->where('objectType')->eq('task')->andWhere('objectID')->eq(7)->andWhere('action')->eq('paused')->exec();
