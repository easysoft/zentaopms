...
class startTaskTester extends tester
{
    ...
    public function start($id, $assignedTo, $consumed, $left)
    {
        ...
        if((empty($consumed) || $consumed == '0') && (empty($left) || $left == '0'))
        {
            ...
            if(is_object($form->dom->modalText) and $form->dom->modalText->getText() != $this->lang->task->noticeTaskStart) return $this->failed('总计消耗和预计剩余都为空或0时提示错误');
        ...
        elseif($consumed != '0' && (empty($left) || $left == '0'))
        {
            ...
            if(is_object($form->dom->modalText) and $form->dom->modalText->getText() != $this->lang->task->confirmFinish) return $this->failed('预计剩余为空或0时提示错误');
