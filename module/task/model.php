...
class taskModel extends model
{
    ...
    public function getUserTaskPairs($account, $status = 'all', $skipExecutionIDList = array(), $appendTaskID = 0)
    {
        ...
        $stmt = $this->dao->select('t1.id, t1.name, t2.name as execution')
            ...
            ->leftJoin(TABLE_TASKTEAM)->alias('t3')->on("t3.task = t1.id and t3.account = '{$account}'")
            ->where("(t1.assignedTo = '{$account}' or (t1.mode = 'multi' and t3.`account` = '{$account}' and t1.status != 'closed' and t3.status != 'done') )")
            ->andWhere('t1.deleted')->eq(0)
            ->andWhere('t2.deleted')->eq(0)
            ->beginIF($this->config->vision)->andWhere('t1.vision')->eq($this->config->vision)->fi()
            ->beginIF($status != 'all')->andWhere('t1.status')->in($status)->fi()
