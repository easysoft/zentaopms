...
class testtask extends control
{
    ...
    public function view(int $testtaskID)
    {
        ...
        $this->testtaskZen->setDropMenu($testtask->product, $testtask);
        ...
        if(!isset($products[$testtask->product]))
        {
            $product = $this->loadModel('product')->getByID($testtask->product);
        ...
        $this->view->productID  = $testtask->product;
        $this->display();
    }
    ...
    public function cases(int $testtaskID, string $browseType = 'all', int $param = 0, string $orderBy = 'id_desc', int $recTotal = 0, int $recPerPage = 20, int $pageID = 1)
    {
        ...
        /* 检查是否有权限访问测试单所属产品。*/
        /* Check if user have permission to access the product to which the testtask belongs. */
        $this->testtaskZen->setMenu($testtask->product, (string)$testtask->branch, $testtask->project, $testtask->execution, $testtask);
        $productID = $this->loadModel('product')->checkAccess($testtask->product, $this->products);
    ...
    public function report(int $productID, int $testtaskID, string $browseType, int $branchID, int $moduleID = 0, string $chartType = 'pie')
    {
        ...
        if(!empty($_POST))
        {
            ...
            {
                if(!$this->session->testtaskQuery) $this->session->set('testtaskQuery', ' 1 = 1');
                if($moduleID)
                {
                    $query = $this->loadModel('search')->getQuery($moduleID);
                    if($query)
                    {
            ...
            foreach($this->post->charts as $chart)
            {
                ...
                $chartData   = $this->testtask->$chartFunc($testtaskID, $caseQuery, $browseType == 'bymodule' ? $moduleID : 0);
        ...
        /* 如果测试单所属产品在产品键值对中不存在，将其加入。*/
        /* Prepare the product key-value pairs. */
        if(!isset($this->products[$task->product]))
        {
