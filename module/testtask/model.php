...
class testtaskModel extends model
{
    ...
    public function getProductTasks($productID, $branch = 'all', $orderBy = 'id_desc', $pager = null, $scopeAndStatus = array(), $beginTime = 0, $endTime = 0)
    {
        ...
        return $this->dao->select("t1.*, IF(t2.shadow = 1, t5.name, t2.name) AS productName, IF(t5.id IS NOT NULL, CONCAT(t5.name, ' / ', t3.name), t3.name) AS executionName, t4.name AS buildName, t4.branch AS branch")
