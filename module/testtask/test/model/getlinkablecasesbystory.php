...
r($cases) && p('0:id,title,pri,type,auto,status,storyTitle') && e('8,这个是测试用例8,4,feature,no,investigate,软件需求2'); // 查看可关联的用例 8 的详细信息。
r($cases) && p('1:id,title,pri,type,auto,status,storyTitle') && e('7,这个是测试用例7,3,other,no,blocked,用户需求1');       // 查看可关联的用例 7 的详细信息。
r($cases) && p('2:id,title,pri,type,auto,status,storyTitle') && e('6,这个是测试用例6,2,interface,no,normal,软件需求2');    // 查看可关联的用例 6 的详细信息。

$cases = $testtask->getLinkableCasesByStory(1, $task5);
r(count($cases)) && p() && e(12); // 产品 1 测试单 5 可关联的用例数为 12。
r($cases) && p('0:id,title,pri,type,auto,status,storyTitle') && e('19,这个是测试用例19,3,security,no,blocked,用户需求1');        // 查看可关联的用例 19 的详细信息。
r($cases) && p('1:id,title,pri,type,auto,status,storyTitle') && e('18,这个是测试用例18,2,install,no,normal,软件需求2');          // 查看可关联的用例 18 的详细信息。
r($cases) && p('2:id,title,pri,type,auto,status,storyTitle') && e('16,这个是测试用例16,4,performance,no,investigate,软件需求2'); // 查看可关联的用例 16 的详细信息。
...
r($cases) && p('0:id,title,pri,type,auto,status,storyTitle') && e('18,这个是测试用例18,2,install,no,normal,软件需求2');          // 查看可关联的用例 18 的详细信息。
r($cases) && p('1:id,title,pri,type,auto,status,storyTitle') && e('16,这个是测试用例16,4,performance,no,investigate,软件需求2'); // 查看可关联的用例 16 的详细信息。
r($cases) && p('2:id,title,pri,type,auto,status,storyTitle') && e('14,这个是测试用例14,2,other,no,normal,软件需求2');            // 查看可关联的用例 14 的详细信息。
