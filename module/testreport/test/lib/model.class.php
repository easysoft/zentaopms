...
class testreportModelTest extends baseTest
{
    ...
    public function createTest(array $param): array|object
    {
        ...
        $reportID = $this->instance->create($testreport);
        if(dao::isError()) return dao::getError();
        $object = $this->instance->getByID($reportID);
        return $object;
    }
    ...
    {
        $report = $oldReport = $this->instance->getByID($reportID);
        ...
        $this->instance->update($report, $oldReport);
    ...
    {
        $objects = $this->instance->getCaseIdList($reportID);
   ...
   /**
    ...
    public function getResultSummaryTest(int $taskID, int $reportID): array|string
    {
        ...
        $resultSummary = $this->instance->getResultSummary($tasks, $cases, $report->begin, $report->end);
    ...
    {
        $objects = $this->instance->getPairs($productID, $appendID);
        if(dao::isError()) return dao::getError();
        return implode(',', array_keys($objects));
    }
