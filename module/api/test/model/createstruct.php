...
- 测试正常创建数据结构 @1
- 测试结构名称为空时创建数据结构 @0
- 测试结构名称为空的错误提示信息第name条的0属性 @『结构名称』不能为空。
- 测试params字段为空时创建数据结构 @0
- 测试params字段为空的错误提示信息第params条的0属性 @『参数』不能为空。
- 测试创建复杂数据结构 @1
...
r(dao::getError()) && p('name:0') && e('『结构名称』不能为空。'); // 测试结构名称为空的错误提示信息

// 测试步骤3：params字段为空时创建数据结构
$emptyParamsData = new stdclass();
$emptyParamsData->lib  = 1;
$emptyParamsData->name = '测试结构';
$emptyParamsData->params = '';

r($tester->api->createStruct($emptyParamsData)) && p() && e(0); // 测试params字段为空时创建数据结构
r(dao::getError()) && p('params:0') && e('『参数』不能为空。'); // 测试params字段为空的错误提示信息
