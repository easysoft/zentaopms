...
class bugModelTest extends baseTest
{
    ...
    {
        $array = $this->instance->getBySonarqubeID($sonarqubeID);
    ...
    public function updateObject($bugID, $param = array())
    {
        ...
        $change = $this->instance->update($bug, 'Edit');
    ...
    public function closeObject(int $bugID, array $output = array())
    {
        ...
        {
            $bug = $this->instance->fetchBugInfo($bugID);
            return $bug;
        }
    }
    ...
    public function processBuildForBugsTest(array $bugIDList): array
    {
        ...
        $array = $this->instance->processBuildForBugs($bugs);
    ...
    public function getUserBugsTest(string $account, string $type = 'assignedTo', int $limit = 0, int $executionID = 0, int $queryID = 0, string $rawMethod = 'work', string|bool $query = false): array|int
    {
        ...
        $array = $this->instance->getUserBugs($account, $type, 'id_desc', $limit, null, $executionID, $queryID);
    ...
    {
        $array = $this->instance->getUserBugPairs($account, $appendProduct, $limit, $skipProductIdList, $skipExecutionIdList, $appendBugID);
    ...
    {
        $array = $this->instance->getProductLeftBugs($buildIdList, $productID, $branch, $linkedBugs);
    ...
    {
        $array = $this->instance->getProductBugPairs($productID, $branch);
    ...
    {
        $array = $this->instance->getProductMemberPairs($productID);
    ...
    {
        $array = $this->instance->getReleaseBugs(array($buildID), $productID);
    ...
    {
        $array = $this->instance->getStoryBugs($storyID);
    ...
    {
        $array = $this->instance->getCaseBugs($runID, $caseID, $version);
    ...
    {
        $array = $this->instance->getStoryBugCounts($storyIDList, $executionID);
    ...
    {
        $array = $this->instance->getBugInfoFromResult($resultID, $caseID);
    ...
    public function getToAndCcListTest(int $bugID): array|string
    {
        ...
        $array = $this->instance->getToAndCcList($bug);
    ...
    {
        $array = $this->instance->getBugQuery($bugQuery);
    ...
    {
        $dates = $this->instance->getStatistic($productID);
        $returns = array();
        $today   = date('m/d', time());
    ...
    public function getLinkedExecutionByIdListTest(string $buildIdList): array|string
    {
        ...
        $array       = $this->instance->getLinkedExecutionByIdList($buildIdList);
    ...
    public function getRelatedObjectsTest(string $object, string $pairs = ''): object
    {
        ...
        $result = $this->instance->getRelatedObjects($object, $pairs);
    ...
    public function getBatchResolveVarsTest(array $bugIDList): array|int
    {
        ...
        $stmt = $tester->dao->query($this->instance->loadModel('tree')->buildMenuQuery($productID, 'bug'));
        $modules = array();
    ...
    public function getProductsForCreateTest(object $bug): object|array
    {
        ...
        try {
            ...
            // 模拟获取产品列表
            $products = $this->instance->loadModel('product')->getPairs('noclosed', 0, '', 'all');
            ...
            {
                $products = array();
                $linkedProducts = $this->instance->loadModel('product')->getOrderedProducts('normal', 40, $projectID);
                foreach($linkedProducts as $product) $products[$product->id] = $product->name;
            }
            elseif($app->tab == 'execution' && $executionID)
            {
                $products = array();
                $linkedProducts = $this->instance->loadModel('product')->getProducts($executionID);
                foreach($linkedProducts as $product) $products[$product->id] = $product->name;
    ...
    public function getBranchesForCreateTest(object $bug): object|array|string
    {
        ...
        try {
            ...
            if($app->tab == 'execution' || $app->tab == 'project')
            {
                ...
                {
                    $productBranches = $this->instance->loadModel('execution')->getBranchByProduct(array($productID), $objectID, 'noclosed|withMain');
    ...
    public function assignUsersForBatchEditTest($bugsType = 'normal', $tabType = 'product')
    {
        ...
        // 总是返回基础用户列表
        $userModel = $this->instance->loadModel('user');
