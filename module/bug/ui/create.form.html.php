...
$items['module'] = array();
$items['module']['control'] = array('type' => 'inputGroup', 'id' => 'moduleBox');
$items['module']['items']   = array();
$items['module']['items'][] = array('type' => 'picker', 'name' => 'module', 'items' => $moduleOptionMenu, 'value' => $bug->moduleID);
$items['module']['items'][] = count($moduleOptionMenu) == 1 ? array('type' => 'btn', 'text' => $lang->tree->manage, 'url' => createLink('tree', 'browse', "rootID=$bug->productID&view=bug&currentModuleID=0&branch={$bug->branch}"), 'data-toggle' => 'modal', 'data-size' => 'lg') : null;
$items['module']['items'][] = count($moduleOptionMenu) == 1 ? array('type' => 'btn', 'icon' => 'refresh text-primary', 'hint' =>  $lang->bug->loadAll, 'id' => 'refreshModule') : null;
