...
class projectModelTest extends baseTest
{
    ...
    {
        $result = $this->instance->getAclListByObjectType($objectType);
    ...
    {
        $result = $this->instance->getListByAcl($acl, $idList);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getTeamListByType($type);
        if(dao::isError()) return dao::getError();
    ...
    public function leftJoinInvolvedTableTest($stmt = null)
    {
        ...
        $result = $this->instance->leftJoinInvolvedTable($stmt);
        if(dao::isError()) return dao::getError();
    ...
    public function appendInvolvedConditionTest($stmt = null)
    {
        ...
        {
            $stmt = $tester->dao->select('*')->from(TABLE_PROJECT)->alias('t1');
            $stmt = $this->instance->leftJoinInvolvedTable($stmt);
        }

        $result = $this->instance->appendInvolvedCondition($stmt);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getExecutionProductGroup($executionIDs);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->addTeamMembers($projectID, $project, $members);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->checkBranchAndProduct($parent, $products, $branch);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->checkDates($projectID, $project);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->updateUserView($projectID, $acl);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->unlinkStoryByType($projectID, $storyType);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->setNoMultipleMenu($projectID);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->formatDataForList($project, $PMList);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->recordFirstEnd($projectID);
        if(dao::isError()) return dao::getError();
    ...
    {
        $this->instance->setMenuByModel($model);
    ...
    {
        $members            = $this->instance->getTeamMembers($projectID);
        $oldProjectProducts = $this->instance->dao->select('*')->from(TABLE_PROJECTPRODUCT)->where('project')->eq($projectID)->fetchGroup('product', 'branch');
        if(!empty($branches)) $_POST['branch'] = $branches;
        if(!empty($plans)) $_POST['branch'] = $plans;

        $this->instance->linkProducts($projectID, $products, $oldProjectProducts, $members);

        return $this->instance->dao->select('*')->from(TABLE_PROJECTPRODUCT)->where('project')->eq($projectID)->fetchAll();
    }
    ...
    {
        $oldProject = $this->instance->getByID($projectID);
        $newProject = clone $oldProject;
        $newProject->name = '更新' . $oldProject->name;
        $newProject->acl  = 'open';

        $this->instance->updateShadowProduct($newProject, $oldProject);
        $products = $this->instance->dao->select('t2.*')->from(TABLE_PROJECTPRODUCT)->alias('t1')
    ...
    {
        $this->instance->addPlans($projectID, $plans);

        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_PROJECTSTORY)->where('project')->eq($projectID)->orderBy('order_asc')->fetchAll();
    }
    ...
    {
        $this->instance->updatePlans($projectID, $plans);

        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_PROJECTSTORY)->where('project')->eq($projectID)->fetchAll();
    }
    ...
    public function linkOtherProductsTest(int $projectID, array $productIdList): bool
    {
        ...
        return $this->instance->linkOtherProducts($projectID, $members);
    }
    ...
    {
        $this->instance->batchUpdate($data);

        if(dao::isError()) return array('message' => dao::getError());

        return $this->instance->getByIdList(array_keys($data));
    }
    ...
    {
        $project = $this->instance->getByID($projectID);
        $actions = $this->instance->buildActionList($project);
    ...
    public function updateProductsTest(int $projectID, array $products = array(), array $otherProducts = array()): array
    {
        ...
        $this->instance->updateProducts($projectID, $products);
    ...
    {
        $this->instance->update($project, $oldProject);

        if(dao::isError()) return dao::getError();

        return $this->instance->getByID($oldProject->id);
    }
    ...
    {
        $projectName = $this->instance->dao->select('name')->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetch('name');
        $output      = $this->instance->getSwitcher($projectID, $module, $method);
    ...
    {
        $this->instance->updateProductStage($projectID, $postProductData);
        $linkInfo = $this->instance->dao->select('*')->from(TABLE_PROJECTPRODUCT)->fetchAll();
    ...
    public function updateWhitelistTest(int $projectID, string $whitelist): string
    {
        ...
        $oldProject = $this->instance->getByID($projectID);
        $this->instance->updateWhitelist($project, $oldProject);
        if(dao::isError()) return '';
        $whitelist = $this->instance->dao->select('whitelist')->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetch('whitelist');
    ...
    public function getDisabledProductsTest(int $projectID): array
    {
        ...
        $linkedProducts = $this->instance->dao->select('product')->from(TABLE_PROJECTPRODUCT)->where('project')->eq($projectID)->fetchPairs();

        return $this->instance->getDisabledProducts($project, $linkedProducts);
    }
    ...
    {
        $this->instance->manageMembers($projectID, $members);
        if(dao::isError()) return dao::getError();
        return $this->instance->getTeamMembers($projectID);
    }
    ...
    {
        $projectID = $this->instance->create($project, $postData);

        if(dao::isError()) return array('message' => dao::getError());

        return $this->instance->getById($projectID);
    }
    ...
    public function buildProjectBuildSearchFormTest(int $projectID, int $productID, string $type)
    {
        ...
        $result = $this->instance->buildProjectBuildSearchForm(array(), 0, $projectID, $productID, $type);
    ...
    {
        $this->instance->app->rawModule = 'project';
        $this->instance->app->rawMethod = 'index';

        $this->instance->setMenu($projectID);
    ...
    {
        $oldProjects = $this->instance->getByIdList($projectIdList);
        $newProjects = array();
        ...
        return $this->instance->buildBatchUpdateProjects($newProjects, $oldProjects);
    }
    ...
    public function setMenuByProductTest($hasProduct, $model)
    {
        global $lang;
        $this->instance->setMenuByModel($model);
        $this->instance->setMenuByProduct(11, $hasProduct, $model);
    ...
    public function getLinkedProductsCountTest(int $projectID, array $products)
    {
        ...
        return $this->instance->getLinkedProductsCount($project, $rawdata);
    }
    ...
    {
        $this->instance->doUpdate($projectID, $project);
        if(dao::isError())
        {
            return dao::getError();
        }
    ...
    {
        $this->instance->doCreate($project);
        if(dao::isError())
        {
            return dao::getError();
        }
    ...
    public function createDocLibTest(int $projectID, int $programID): object|bool
    {
        ...
        $this->instance->createDocLib($projectID, $project, $program);

        return $this->instance->dao->select('*')->from(TABLE_DOCLIB)->where('type')->eq('project')->andWhere('project')->eq($projectID)->fetch();
    }
    ...
    public function updateTasksStartAndEndDateTest(int $projectID, string $type = 'expand')
    {
        ...
        $tasks = $this->instance->dao->select('id,status,estStarted,deadline')->from(TABLE_TASK)
            ->where('deleted')->eq(0)
        ...
        $this->instance->updateTasksStartAndEndDate($tasks, $oldProject, $project);

        $changes = array();
        if(isset($oldTask))
        {
            $task = $this->instance->dao->select('id,status,estStarted,deadline')->from(TABLE_TASK) ->where('id')->eq($oldTask->id)->fetch();
    ...
    {
        $this->instance->addProjectAdmin($projectID);

        global $app;
        return $this->instance->dao->select('*')->from(TABLE_PROJECTADMIN)->where('account')->eq($app->user->account)->fetch();
    }
