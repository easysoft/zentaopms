    ...
    public function create()
    {
        ...
        if($project->parent)
        {
            ...
            if($program)
            {
                ...
                if($project->begin < $program->begin) dao::$errors['begin'] = sprintf($this->lang->project->beginGreateChild, $project->name, $program->name, $program->begin);

                /* When parent set end then child project end cannot greater than parent. */
                if($program->end != '0000-00-00' and $project->end > $program->end) dao::$errors['end'] = sprintf($this->lang->project->endLetterChild, $project->name, $program->name, $program->end);
    ...
    public function batchUpdate()
    {
        ...
        foreach($data->projectIdList as $projectID)
        {
            ...
            if($projects[$projectID]->parent)
            {
                ...
                {
                    /* Child project begin cannot less than parent. */
                    if($projects[$projectID]->begin < $parentProject->begin)
                    {
                    ...
                    /* When parent set end then child project end cannot greater than parent. */
                    if($parentProject->end != '0000-00-00' and $projects[$projectID]->end > $parentProject->end)
                    {
