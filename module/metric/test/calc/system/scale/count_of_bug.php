...
zendata('bug')->loadYaml('bug_severity', true, 4)->gen(356, true, false);
$calc = $metric->calcMetric(__FILE__);
r($calc->getResult()) && p('0:value') && e('92'); // 测试356条数据Bug数。

zendata('bug')->loadYaml('bug_severity', true, 4)->gen(652, true, false);
$calc = $metric->calcMetric(__FILE__);
r($calc->getResult()) && p('0:value') && e('164'); // 测试652条数据Bug数。

zendata('bug')->loadYaml('bug_severity', true, 4)->gen(1265, true, false);
$calc = $metric->calcMetric(__FILE__);
