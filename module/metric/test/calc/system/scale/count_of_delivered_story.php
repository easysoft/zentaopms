...
zendata('story')->loadYaml('story_stage_closedreason', true, 4)->gen(839, true, false);
$calc = $metric->calcMetric(__FILE__);
r($calc->getResult()) && p('0:value') && e('48'); // 测试839条数据全局已交付研发需求数。

zendata('story')->loadYaml('story_stage_closedreason', true, 4)->gen(500, true, false);
$calc = $metric->calcMetric(__FILE__);
r($calc->getResult()) && p('0:value') && e('32'); // 测试500条数据全局已关闭研发需求数。

zendata('story')->loadYaml('story_stage_closedreason', true, 4)->gen(1252, true, false);
$calc = $metric->calcMetric(__FILE__);
