 ...
 * 对象：execution
 * 目的：rate
 * 度量名称：按全局统计的年度完成执行中执行的按期关闭率
 * 单位：%
 * 描述：按全局统计的年度完成执行中执行的按期关闭率是指某年度按预定计划时间关闭的执行数量与某年度关闭执行执行数量之比。这个度量项可以帮助团队评估某年度执行按期关闭的能力和效果，并作为执行管理的绩效指标之一。较高的执行按期关闭率表示团队能够按时完成执行和项目。
 * 定义：复用：
 *       按全局统计的年度关闭执行数
 *       按全局统计的年度完成执行中按期完成执行数
 *       公式：
 *       按全局统计的年度完成执行中执行的按期关闭率=按全局统计的年度完成执行中按期完成执行数/按全局统计的年度关闭执行数
 * 度量库：
 * 收集方式：realtime
 *
