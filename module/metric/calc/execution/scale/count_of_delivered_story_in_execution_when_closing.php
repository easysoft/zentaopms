 ...
 * 单位：个
 * 描述：按执行统计的执行关闭时已交付研发需求数表示执行关闭时需求阶段为已发布或状态为已关闭且关闭原因为已完成的研发需求的数量。该度量项反映了执行结束时能够交付给用户的研发需求的数量，可以用于评估执行团队的研发需求交付能力。
...
class count_of_delivered_story_in_execution_when_closing extends baseCalc
{
    ...
    public $fieldList = array('t4.id as execution', 't1.id', 't4.closedDate as executionClosed', 't1.closedDate as storyClosedDate', 't1.releasedDate as storyReleasedDate', 't1.closedReason');
    ...
    public function calculate($row)
    {
        ...
        if($condition1 || $condition2) $this->result[$row->execution] ++;
    }
