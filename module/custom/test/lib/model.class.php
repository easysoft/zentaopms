...
class customModelTest extends baseTest
{
    ...
    {
        $objects = $this->instance->setItem($path, $value);
        ...
        if($level > 1)
        {
            ...
            $objects = $this->instance->dao->select('*')->from(TABLE_LANG)->where('`lang`')->eq($lang)->andWhere('`module`')->eq($module)->andWhere('`key`')->eq($key)->fetch();
        }
    ...
    {
        $items = $this->instance->getItems($paramString);
    ...
    {
        $result = $this->instance->deleteItems($paramString);
    ...
    public function prepareSQLTest(string $paramString, string $method = 'select'): array|int
    {
        ...
        if($method == 'delete')
        {
            $objects = $this->instance->prepareSQL($params, $method)->exec();
        }
        else
        {
            $objects = $this->instance->prepareSQL($params, $method)->orderBy('lang,id')->fetchAll('key');
        }
    ...
    {
        $objects = $this->instance->getRequiredFields($moduleConfig);
    ...
    public function getFormFieldsTest($moduleName, $method = ''): array
    {
        ...
        $fields = $this->instance->getFormFields($moduleName, $method);
        if(dao::isError()) return dao::getError();
    ...
    {
        $URSRPairs = $this->instance->getURSRPairs();
        if(dao::isError()) return dao::getError();
    ...
    {
        // 先清理已有数据
        $this->instance->dao->delete()->from(TABLE_LANG)
    ...
    public function getURSRPairsTestWithData(): array
    {
        ...
        {
            $this->instance->dao->insert(TABLE_LANG)->data($data)->exec();
        }
    ...
    {
        $URPairs = $this->instance->getURPairs();
        if(dao::isError()) return dao::getError();
    ...
    {
        $SRPairs = $this->instance->getSRPairs();
        if(dao::isError()) return dao::getError();
    ...
    {
        $URSRList = $this->instance->getURSRList();
    ...
    public function saveRequiredFieldsTest(string $moduleName, array $requiredFields, string $fieldsType): array|object
    {
        ...
        $this->instance->saveRequiredFields($moduleName, $requiredFields);
        $objects = $this->instance->dao->select('`value`')->from(TABLE_CONFIG)
        ...
        $this->instance->loadModel('setting')->deleteItems("owner=system&module={$moduleName}&key=requiredFields&vision={$app->config->vision}");
        return $objects;
    }
    ...
    {
        try {
            $result = $this->instance->setConcept($sprintConcept);
            if(dao::isError()) return dao::getError();
            return $result ? 1 : 0;
        } catch(Exception $e) {
    ...
    {
        $objects = $this->instance->setURAndSR($data);
    ...
    {
        $this->instance->updateURAndSR($key, '', $data);
        if(dao::isError()) return dao::getError();

        $concept = $this->instance->getURSRConcept($key);
    ...
    {
        $this->instance->disableFeaturesByMode($mode);

        if(dao::isError()) return dao::getError();
        $result = $this->instance->loadModel('setting')->getItem('owner=system&module=common&key=disabledFeatures');
    ...
    public function disableFeaturesByModeTestWithURAndSR(string $mode): string
    {
        ...
        $disabledFeatures = $this->instance->loadModel('setting')->getItem('owner=system&module=common&key=disabledFeatures');
        $URAndSR = $this->instance->loadModel('setting')->getItem('owner=system&module=custom&key=URAndSR');
        $enableER = $this->instance->loadModel('setting')->getItem('owner=system&module=custom&key=enableER');
    ...
    {
        $this->instance->loadModel('setting')->setItem('system.common.disabledFeatures', $disabledFeatures);
        $this->instance->processMeasrecordCron();
        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('status')->from(TABLE_CRON)->where('command')->like('%methodName=execCrontabQueue')->fetch('status');
    }
    ...
    {
        $count = $this->instance->hasProductURData();
    ...
    {
        $count = $this->instance->hasWaterfallData();
    ...
    {
        $count = $this->instance->hasWaterfallplusData();
    ...
    public function hasAssetlibDataTest(string $edition): int|array
    {
        ...
        $count = $this->instance->hasAssetlibData();
    ...
    {
        list($projectGroup, $programPM, $stakeholders) = $this->instance->getDataForUpdateProjectAcl();
        return ${$key};
    }
    ...
    {
        $this->instance->processProjectAcl();

        if(dao::isError()) return dao::getError();
        return $this->instance->loadModel('project')->getByID($projectID);
    }
    ...
    public function computeFeaturesTest(string $edition): array
    {
        ...
        $this->instance->config->edition = $edition;
        $features = $this->instance->computeFeatures();
    ...
    public function getCustomLangTest(): array|false
    {
        ...
        $allCustomLang = $this->instance->getCustomLang();
    ...
    public function getAllLangTest(): array|false
    {
        ...
        $this->instance->config->vision = 'rnd';
        $processedLang = $this->instance->getCustomLang();
    ...
    {
        $result = $this->instance->hasProductERData();
        if(dao::isError()) return dao::getError();
    ...
    public function assignFieldListForSetTest(string $module = 'story', string $field = 'priList', string $lang = '', string $currentLang = ''): string
    {
        ...
        try {
            ...
            if($lang == 'all') {
                ...
                $items = $this->instance->getItems("lang=all&module={$module}&section={$field}&vision=rnd");
                return 'all';
            }
            else
            {
                ...
                $dbFields = $this->instance->getItems("lang={$lang}&module={$module}&section={$field}&vision=rnd");

                if(empty($dbFields)) {
                    $dbFields = $this->instance->getItems("lang=" . ($lang == $currentLang ? 'all' : $currentLang) . "&module={$module}&section={$field}");
                }
    ...
    public function checkInvalidKeysTest(array $keys = array(), string $module = 'story', string $field = 'priList'): string|bool
    {
        ...
        try {
            ...
            // 获取现有的自定义配置项
            $oldCustoms = $this->instance->getItems("lang=zh-cn&module={$module}&section={$field}");
    ...
    public function checkEmptyKeysTest(array $keys = array(), array $values = array(), array $systems = array(), string $module = 'story', string $field = 'priList', string $lang = 'zh-cn'): string|bool
    {
        ...
        try {
            ...
            foreach($keys as $index => $key)
            {
                ...
                // 模拟调用 setItem 方法
                if($this->instance) {
                    $this->instance->setItem("{$lang}.{$module}.{$field}.{$key}.{$system}", $value);
                }
    ...
    public function setStoryReviewTest(string $module = 'story', array $data = array()): bool|array
    {
        ...
        // 模拟保存配置到数据库
        $settingModel = $this->instance->loadModel('setting');
        $settingModel->setItems("system.{$module}@{$this->instance->config->vision}", $data);
    ...
    public function setTestcaseReviewTest(array $data = array()): bool|array
    {
        ...
        // 模拟保存配置到数据库
        $this->instance->loadModel('setting')->setItems("system.testcase", $data);
