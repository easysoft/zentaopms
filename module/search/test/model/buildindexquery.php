...
- 构建未删除项目集的sql语句 @SELECT t1.* FROM `zt_project` AS t1  WHERE (t1.`isTpl` = '0' OR t1.`isTpl` IS NULL) AND 1=1  AND  `type`  = 'program'
...
- 构建查询所有项目的sql语句 @SELECT t1.* FROM `zt_project` AS t1  WHERE (t1.`isTpl` = '0' OR t1.`isTpl` IS NULL) AND 1=1  AND  `type`  = 'project'
...
r($search->buildIndexQuery('program')->get())        && p() && e("SELECT t1.* FROM `zt_project` AS t1  WHERE (t1.`isTpl` = '0' OR t1.`isTpl` IS NULL) AND 1=1  AND  `type`  = 'program'"); //构建未删除项目集的sql语句
...
r($search->buildIndexQuery('project', false)->get()) && p() && e("SELECT t1.* FROM `zt_project` AS t1  WHERE (t1.`isTpl` = '0' OR t1.`isTpl` IS NULL) AND 1=1  AND  `type`  = 'project'"); //构建查询所有项目的sql语句