...
function createBugMenu(options)
{
    ...
    if(priv.canConfirmBug && (bug.$col.type == 'unconfirmed' || bug.$col.type == 'fixing')) items.push({label: bugLang.confirmBug, icon: 'ok', url: createLink('bug', 'confirmbug', 'bugID=' + bug.id, '', 'true'), className: 'iframe', attrs: {'data-toggle': 'modal', 'data-width': '80%'}});
