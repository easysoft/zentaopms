...
class webhookModelTest extends baseTest
{
    ...
    {
        $objects = $this->instance->getByType($type);
    ...
    {
        $objects = $this->instance->getBindAccount($webhookID, $webhookType, $openID);
    ...
    {
        $objects = $this->instance->getList($orderBy, $pager);
    ...
    {
        $objects = $this->instance->getLogList($id, $orderBy, $pager);
    ...
    {
        global $tester;
        $objects = $this->instance->getDataList();
    ...
    {
        $objectID = $this->instance->create($webhook);
    ...
    {
        $this->instance->update($id, $webhook);
    ...
    public function bindTest($webhookID, $userList)
    {
        ...
        $result = $this->instance->bind($webhookID);
    ...
    {
        $objects = $this->instance->send($objectType, $objectID, $actionType, $actionID, $actor);
    ...
    public function buildDataTest($objectType, $objectID, $actionType, $actionID)
    {
        ...
        {
            $data = $this->instance->buildData($objectType, $objectID, $actionType, $actionID, $webhook);
            if($data !== false)
            {
                $result = $data;
                break;
    ...
    {
        $objects = $this->instance->getViewLink($objectType, $objectID);
    ...
    {
        $objects = $this->instance->getDingdingData($title, $text, $mobile);
        if(dao::isError()) return dao::getError();
    ...
    {
        $objects = $this->instance->getWeixinData($text, $mobile);
    ...
    {
        $objects = $this->instance->getFeishuData($title, $text);
    ...
    {
        try {
            $result = $this->instance->getOpenIdList($webhookID, $actionID, $toList);
            if(dao::isError()) return dao::getError();
            return $result;
        } catch (Exception $e) {
            return false;
    ...
    {
        $result = $this->instance->saveLog($webhook, $actionID, $data, $result);

        if(dao::isError()) return dao::getError();

        if($result)
        {
            return $this->instance->dao->select('*')->from(TABLE_LOG)->where('objectID')->eq($webhook->id)->andWhere('objectType')->eq('webhook')->andWhere('action')->eq($actionID)->orderBy('id_desc')->limit(1)->fetch();
        }
    ...
    {
        $this->instance->setSentStatus($idList, $status, $time);

        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_NOTIFY)->where('id')->in($idList)->fetchAll('id');
    }
    ...
    public function getDataByTypeTest($type, $title, $text, $mobile, $email, $objectType, $objectID)
    {
        ...
        $result = $this->instance->getDataByType($webhook, $action, $title, $text, $mobile, $email, $objectType, $objectID);
    ...
    {
        $result = $this->instance->fetchHook($webhook, $sendData, $actionID, $appendUser);
    ...
    {
        $result = $this->instance->sendToUser($webhook, $sendData, $actionID, $appendUser);
