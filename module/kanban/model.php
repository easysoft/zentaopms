...
class kanbanModel extends model
{
     ...
     * @return array
     */
    public function getExecutionKanban(int $executionID, string $browseType = 'all', string $groupBy = 'default', string $searchValue = '', string $orderBy = 'id_asc')
    {
    ...
    public function buildExecutionGroup(object $lane, array $columns, array $objectGroup, string $searchValue = '', array $storyCardMenu = array(), array $bugCardMenu = array(), array $taskCardMenu = array()): array
    {
        ...
        foreach($columns[$lane->id] as $columnID => $column)
        {
            ...
            $columnData[$column->id]['cards']  = empty($column->cards) ? 0 : count($cardIdList);
    ...
    public function getKanban4Group($executionID, $browseType, $groupBy, $searchValue = '', $orderBy = 'id_asc')
    {
        ...
        foreach($lanes as $laneID => $lane)
        {
            ...
            foreach($columns as $column)
            {
                ...
                $columnData['group']      = 0;
                $columnData['cards']      = empty($column->cards) ? 0 : count($cardIdList);
                ...
                foreach($objects as $object)
                {
                    ...
                    $cardData['id']         = $object->id;
                    $cardData['lane']       = $laneData['id'];
                    $cardData['column']     = $column->column;
                    $cardData['pri']        = $object->pri ? $object->pri : '';
                    $cardData['group']      = $laneData['id'];
                    ...
                    if($lane->type == 'story') $cardData['actionList'] = $storyCardMenu[$object->id];
                    if($lane->type == 'bug')   $cardData['actionList'] = $bugCardMenu[$object->id];
                    if($lane->type == 'task')  $cardData['actionList'] = $taskCardMenu[$object->id];
        ...
        switch ($objecType)
        {
            ...
            case 'bug':
                ...
                foreach($objects as $bug)
                {
                    ...
                    if(common::hasPriv('bug', 'edit')     && $this->bug->isClickable($bug, 'edit'))     $menu[] = array('label' => $this->lang->bug->edit, 'icon' => 'edit', 'url' => helper::createLink('bug', 'edit', "bugID=$bug->id", '', true), 'size' => '95%');
                    if(common::hasPriv('bug', 'confirm')  && $this->bug->isClickable($bug, 'confirm'))  $menu[] = array('label' => $this->lang->bug->confirm, 'icon' => 'ok', 'url' => helper::createLink('bug', 'confirm', "bugID=$bug->id&extra=&from=taskkanban", '', true));
                    if(common::hasPriv('bug', 'resolve')  && $this->bug->isClickable($bug, 'resolve'))  $menu[] = array('label' => $this->lang->bug->resolve, 'icon' => 'checked', 'url' => helper::createLink('bug', 'resolve', "bugID=$bug->id&extra=&from=taskkanban", '', true));
                    if(common::hasPriv('bug', 'close')    && $this->bug->isClickable($bug, 'close'))    $menu[] = array('label' => $this->lang->bug->close, 'icon' => 'off', 'url' => helper::createLink('bug', 'close', "bugID=$bug->id&extra=&from=taskkanban", '', true));
                    if(common::hasPriv('bug', 'create')   && $this->bug->isClickable($bug, 'create'))   $menu[] = array('label' => $this->lang->bug->copy, 'icon' => 'copy', 'url' => helper::createLink('bug', 'create', "productID=$bug->product&branch=$bug->branch&extras=bugID=$bug->id", '', true), 'size' => '95%');
                    if(common::hasPriv('bug', 'activate') && $this->bug->isClickable($bug, 'activate')) $menu[] = array('label' => $this->lang->bug->activate, 'icon' => 'magic', 'url' => helper::createLink('bug', 'activate', "bugID=$bug->id", '', true));
                    if(common::hasPriv('story', 'create') && $bug->status != 'closed')                  $menu[] = array('label' => $this->lang->bug->toStory, 'icon' => 'lightbulb', 'url' => helper::createLink('story', 'create', "product=$bug->product&branch=$bug->branch&module=0&story=0&execution=0&bugID=$bug->id", '', true), 'size' => '95%');
            ...
            case 'task':
                ...
                foreach($objects as $task)
                {
                    ...
                    if(common::hasPriv('task', 'recordWorkhour') and $this->task->isClickable($task, 'recordWorkhour'))            $menu[] = array('label' => $this->lang->task->recordWorkhour, 'icon' => 'time', 'url' => helper::createLink('task', 'recordWorkhour', "taskID=$task->id&from=taskkanban", '', true));
