      ...
      <tbody>
        ...
        <tr data-id='<?php echo $task->id;?>' data-status='<?php echo $task->status?>' data-estimate='<?php echo $task->estimate?>' data-consumed='<?php echo $task->consumed?>' data-left='<?php echo $task->left?>'>
          ...
          <td class='c-name <?php if(!empty($task->children)) echo 'has-child';?>' title='<?php echo $task->name?>'>
            ...
            <?php if($task->parent > 0) echo '<span class="label label-badge label-light">' . $this->lang->task->parentAB . '</span> ' .  html::a($this->createLink('task', 'view', "taskID=$task->parent", '', '', $task->project),$task->parentName) .'<span class="label label-badge label-light">' . $this->lang->task->childrenAB . '</span> ';?>
