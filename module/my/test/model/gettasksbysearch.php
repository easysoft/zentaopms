...
r(implode(',', $tasks1)) && p() && e('37,33,25,21,13,9,1'); // 测试通过搜索获取用户 admin 的任务id
r(count($tasks1))        && p() && e('7');                  // 测试通过搜索获取用户 admin 的任务数量
r(implode(',', $tasks2)) && p() && e('37,33,25,21,13');     // 测试通过搜索获取用户 admin 的任务id 限制5
r(count($tasks2))        && p() && e('5');                  // 测试通过搜索获取用户 admin 的任务数量 限制5
r(implode(',', $tasks3)) && p() && e('10,2');               // 测试通过搜索获取用户 user1 的任务id
r(count($tasks3))        && p() && e('2');                  // 测试通过搜索获取用户 user1 的任务数量
r(implode(',', $tasks4)) && p() && e('10,2');               // 测试通过搜索获取用户 user1 的任务id 限制5
r(count($tasks4))        && p() && e('2');                  // 测试通过搜索获取用户 user1 的任务数量 限制5
