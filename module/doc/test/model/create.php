...
r($docTester->createTest($createProjectDoc))   && p('2:title')           && e('新建项目文档');            // 创建项目文档
r($docTester->createTest($createExecutionDoc)) && p('3:title')           && e('新建执行文档');            // 创建执行文档
r($docTester->createTest($createCustomDoc))    && p('4:type')            && e('text,');                   // 创建自定义文档
r($docTester->createTest($privateDoc))         && p('5:acl')             && e('private');                 // 创建私有文档
r($docTester->createTest($customDoc))          && p('6:acl-groups', '-') && e('custom-1,2,3');            // 创建自定义文档
r($docTester->createTest($mineDoc))            && p('7:title')           && e('新建我的文档');            // 创建我的文档
r($docTester->createTest($draftDoc))           && p('8:title,status')    && e('新建我的草稿文档, draft'); // 创建草稿文档
r($docTester->createTest($noLib))              && p('lib')               && e('『所属库』不能为空。');    // 不输入文档库
r($docTester->createTest($noTitle))            && p('title:0')           && e('『文档标题』不能为空。');  // 不输入标题
