...
title=测试 docModel->createSeperateDocs();
timeout=0
cid=1
...
- 创建草稿文档
 ...
 - 第8条的status属性 @ draft
- 不输入文档库属性lib @『所属库』不能为空。
- 不上传文件属性files @『上传文件』不能为空。
...
r($docTester->createSeperateDocsTest($createProductDoc))   && p('1:title')           && e('新建产品文档');            // 创建产品文档
r($docTester->createSeperateDocsTest($createProjectDoc))   && p('2:title')           && e('新建项目文档');            // 创建项目文档
r($docTester->createSeperateDocsTest($createExecutionDoc)) && p('3:title')           && e('新建执行文档');            // 创建执行文档
r($docTester->createSeperateDocsTest($createCustomDoc))    && p('4:type')            && e('text,');                   // 创建自定义文档
r($docTester->createSeperateDocsTest($privateDoc))         && p('5:acl')             && e('private');                 // 创建私有文档
r($docTester->createSeperateDocsTest($customDoc))          && p('6:acl-groups', '-') && e('custom-1,2,3');            // 创建自定义文档
r($docTester->createSeperateDocsTest($mineDoc))            && p('7:title')           && e('新建我的文档');            // 创建我的文档
r($docTester->createSeperateDocsTest($draftDoc))           && p('8:title,status')    && e('新建我的草稿文档, draft'); // 创建草稿文档
r($docTester->createSeperateDocsTest($noLib))              && p('lib')               && e('『所属库』不能为空。');    // 不输入文档库
r($docTester->createSeperateDocsTest($noTitle))            && p('files')             && e('『上传文件』不能为空。');  // 不上传文件
