...
- 查询所有者是admin并且所属模块是common、值是URSR的第一条数据 @0
- 查询所有者是admin并且所属模块是my、附加条件是block、值是initVersion的第一条数据 @0
- 查询不存在的界面配置 @0
- 查询不存在的所有者配置 @0
- 查询不存在的模块配置 @0
- 查询不存在的附加条件配置 @0
...
r($setting->getItemTest($items[3]))  && p() && e('0'); // 查询所属模块是custom的第一条数据
...
r($setting->getItemTest($items[7]))  && p() && e('0'); // 查询研发界面中所属模块是custom的第一条数据
r($setting->getItemTest($items[8]))  && p() && e('1'); // 查询研发界面中附加条件是global的第一条数据
r($setting->getItemTest($items[9]))  && p() && e('1'); // 查询研发界面中值是CRProduct的第一条数据
r($setting->getItemTest($items[10])) && p() && e('0'); // 查询所有者是系统并且所属模块是custom的第一条数据
...
r($setting->getItemTest($items[16])) && p() && e('0'); // 查询所有者是admin并且所属模块是common、值是URSR的第一条数据
r($setting->getItemTest($items[17])) && p() && e('0'); // 查询所有者是admin并且所属模块是my、附加条件是block、值是initVersion的第一条数据
