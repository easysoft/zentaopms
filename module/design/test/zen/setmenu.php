...
global $app;
global $lang;
$lang->design = $lang->design ?? new stdclass();
...
$module->id->range('1-4');
$module->root->range('1');
$module->extra->range('design');
$module->gen(4);

$deliverable = zenData('deliverable');
$deliverable->id->range('1-4');
$deliverable->workflowGroup->range('1');
$deliverable->module->range('1-4');
$deliverable->name->range('概要设计,详细设计,数据库设计,接口设计');
$deliverable->status->range('enabled');
$deliverable->deleted->range('0');
$deliverable->gen(4);
...
$designTest->instance->lang->design->typeList   = $lang->design->typeList;
$designTest->instance->lang->design->plusTypeList = $lang->design->plusTypeList;

r($designTest->setMenuTest(999, 0, '')) && p('count') && e('0'); // 测试项目不存在的情况
r($designTest->setMenuTest(1, 1, '')) && p('count,hasAll,hasHlds,hasDds') && e('5,1,0,0'); // 测试 waterfall 项目且 type 为空的情况
r($designTest->setMenuTest(2, 1, 'hlds')) && p('count,hasAll,hasHlds') && e('5,1,0'); // 测试 waterfall 项目且 type 为 HLDS 的情况
r($designTest->setMenuTest(3, 1, '')) && p('count,hasAll') && e('5,1'); // 测试 ipd 项目的情况
r($designTest->setMenuTest(4, 1, '')) && p('count,hasAll,hasHlds') && e('5,1,0'); // 测试 waterfallplus 项目的情况
