...
class todoTao extends todoModel
{
    ...
    protected function getListBy(string $type, string $account, array|string $status, string $begin, string $end, int $limit, string $orderBy, object $pager = null): array
    {
        return $this->dao->select('*')->from(TABLE_TODO)
            ...
            ->markRight(1)
            ->beginIF($begin && $type != 'future')->andWhere('date')->ge($begin)->fi()
            ->beginIF($end   && $type != 'future')->andWhere('date')->le($end)->fi()
            ...
            ->beginIF($type == 'future')
            ->andWhere('(date')->eq(FUTURE_TIME)
            ->orWhere('(begin')->eq('2400')
            ->andWhere('end')->eq('2400')
            ->markRight(2)
            ->fi()
            ->orderBy($orderBy)
