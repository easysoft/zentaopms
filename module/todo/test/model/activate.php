...
function initData()
{
    zenData('todo')->loadYaml('activate')->gen(5);
}
...
r($todo->activateTest($todoIDList[3])) && p('status') && e('wait'); // 激活一个状态为wait的todo
r($todo->activateTest($todoIDList[4])) && p('status') && e('wait'); // 激活一个状态为doing的todo
