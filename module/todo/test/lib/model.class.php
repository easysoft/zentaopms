...
class todoModelTest extends baseTest
{
    ...
    {
        $objectID = $this->instance->create($todoData);
    ...
    public function batchCreateTest($datas)
    {
        ...
        $todoIDList = $this->instance->batchCreate($todos);
    ...
    public function batchUpdateTest(array $todos, int $todoID)
    {
        ...
        $changes = $this->instance->batchUpdate($todos, $todoIDList);
    ...
    {
        $this->instance->start($todoID);
        $object = $this->instance->getByID($todoID);
    ...
    {
        return $this->instance->batchFinish($todoIDList);
    }
    ...
    {
        $objects = $this->instance->getList($type, $account, $status, $limit, $pager, $orderBy);
    ...
    {
        $objects = $this->instance->getByList($todoIDList);
        $result = '';
    ...
    {
        $object = $this->instance->isClickable($todo, $action);
    ...
    {
        $todoList = $this->instance->getValidCycleList();
        $this->instance->createByCycle($todoList);
    ...
    {
        $count = $this->instance->getCount($account);
    ...
    {
        return $this->instance->getByExportList($orderBy, $queryCondition, $checkedItem);
    }
    ...
    {
        $pri = $this->instance->getPriByTodoType($todoType, $objectID);
    ...
    public function getCycleListTest(bool $initCycle = true): array
    {
        ...
        if($initCycle) $this->instance->createBycycle($todoList);

        $cycleList = $this->instance->getCycleList($todoList);
    ...
    public function closeTodoTest(int $todoID): object
    {
        ...
        $isClosed = $this->instance->closeTodo($todoID);
    ...
    public function getValidsOfBatchCreateTest(array $todos, int $loop, string $assignedTo): int|object
    {
        ...
        $todo  = $this->instance->getValidsOfBatchCreate($todos, $loop, $assignedTo);
    ...
    public function getCycleTodoDateTest(string $configType, int $todoId = 0, string $lastCycleDate = '', string $today = ''): mixed
    {
        ...
        $date = $this->instance->getCycleTodoDate($todo, $lastCycle, $today);
    ...
    public function getCycleTodoDateTestEdgeCase(string $caseType): string
    {
        ...
        switch($caseType)
        {
            case 'valid_empty_result':
                ...
                $date = $this->instance->getCycleTodoDate($todo, '', $today);
            ...
            case 'invalid_type':
                ...
                $date = $this->instance->getCycleTodoDate($todo, '', date('Y-m-d'));
                return empty($date) ? '0' : '1';
    ...
    {
        $result = $this->instance->insert($todo);
    ...
    public function dateRangeTest(string $type): array
    {
        ...
        $result = $method->invoke($this->instance, $type);
        if(dao::isError()) return dao::getError();
    ...
    {
        $todo = $this->instance->getByID($todoID);
