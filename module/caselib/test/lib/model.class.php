...
class caselibModelTest extends baseTest
{
    ...
    {
        $object = $this->instance->getById($libID, $setImgSize);
    ...
    {
        $this->instance->update($lib);

        if(dao::isError()) return dao::getError();

        return $this->instance->getByID($lib->id);
    }
    ...
    {
        $objects = $this->instance->delete($libID, $table);

        if(dao::isError()) return dao::getError();

        return $this->instance->getByID($libID);
    }
    ...
    {
        $objects = $this->instance->getLibraries();
    ...
    {
        $objects = $this->instance->getList('all', $orderBy, $pager);
    ...
    public function createTest(array $params = array()): array|object
    {
        ...
        $libID = $this->instance->create($lib);
    ...
    {
        $objects = $this->instance->getLibCases($libID, $browseType, 0, $moduleID, $sort);
    ...
    {
        $objects = $this->instance->createFromImport($libID);
    ...
    {
        $link = $this->instance->getLibLink($module, $method);
    ...
    {
        $cases = $this->instance->initImportedCase($data);
    ...
    {
        $caseID = $this->instance->insertImportedCase($key, $caseData, $data, $forceNotReview);
        if(dao::isError()) return dao::getError();
    ...
    public function updateImportedCaseTest(int $key, object $caseData, object $data, bool $forceNotReview): array|object|false
    {
        ...
        $this->instance->updateImportedCase($key, $caseData, $data, $forceNotReview, $oldCase);
        if(dao::isError()) return dao::getError();
    ...
    public function getCasesToExportTest(string $browseType, string $exportType, string $orderBy, string $checkedItem = '', int $limit = 0): array|string
    {
        ...
        $this->instance->getLibCases(201, $browseType, 0, $moduleID, $orderBy);
        $this->instance->loadModel('common')->saveQueryCondition($this->instance->dao->get(), 'testcase', true);

        if($exportType == 'selected') $_COOKIE['checkedItem']= $checkedItem;
        $objects = $this->instance->getCasesToExport($exportType, $orderBy, $limit);
    ...
    public function setLibMenuTest(array $libraries = array(), int $libID = 0): bool
    {
        ...
        ob_start();
        $result = $this->instance->setLibMenu($libraries, $libID);
        $output = ob_get_clean();
    ...
    public function getPairsTest(string $type = 'all', string $orderBy = 'id_desc', $pager = null): array
    {
        ...
        $result = $this->instance->getPairs($type, $orderBy, $pager);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->buildSearchConfig($libID);
        if(dao::isError()) return dao::getError();
