...
class mrModelTest extends baseTest
{
    ...
    {
        // 检查MR是否存在
        $oldMR = $this->instance->fetchByID($MRID);
    ...
    {
        $projects = $this->instance->getGitlabProjects($hostID, $projectIdList);
    ...
    {
        $this->instance->insertMR($MR);
        if(dao::isError()) return dao::getError();

        return $this->instance->fetchByID($this->instance->dao->lastInsertID());
    }
    ...
    {
        $this->instance->execJob($MRID, $jobID);
        if(dao::isError()) return dao::getError();

        return $this->instance->fetchByID($MRID);
    }
    ...
    {
        $this->instance->createMRLinkedAction($MRID, 'createmr', '2023-12-12 12:12:12');
        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_ACTION)->fetchAll();
    }
    ...
    public function apiCreateMrTester(int $hostID, string $project, object $params): array|object|null
    {
        ...
        $result = $this->instance->apiCreateMR($hostID, $params);
        if(!empty($result->message) && preg_match('/ID(\d+)/', $result->message, $matches))
        {
            $this->instance->apiDeleteMR($hostID, $project, (int)$matches[1]);
            $result = $this->instance->apiCreateMR($hostID, $params);
        }
        if(empty($result->iid) && empty($result->id)) return $result;

        $this->instance->apiDeleteMR($hostID, $project, empty($result->iid) ? $result->id : $result->iid);
        return $result;
    }
    ...
    {
        $result = $this->instance->apiGetMRCommits($hostID, $project, $mriid);
    ...
    {
        $result = $this->instance->apiUpdateMR($oldMR, $newMR);
        if(empty($result->iid) && empty($result->id)) return $result;

        $this->instance->apiUpdateMR($oldMR, $oldMR);
    ...
    public function apiDeleteMrTester(int $hostID, string $project, object $params): array|object|null
    {
        ...
        $this->instance->apiDeleteMR($hostID, $project, $mrID);
        return $this->instance->apiGetSingleMR($hostID, $mrID);
    }
    ...
    {
        $result = $this->instance->apiDeleteMR($hostID, $projectID, $MRID);
        if(dao::isError()) return dao::getError();
    ...
    {
        $this->instance->apiCloseMR($hostID, $project, $mriid);
        $this->instance->apiReopenMR($hostID, $project, $mriid);
        return $this->instance->apiGetSingleMR($hostID, $mriid);
    }
    ...
    {
        try {
            $result = $this->instance->apiReopenMR($hostID, $projectID, $MRID);
            if(dao::isError()) return dao::getError();
            return $result;
        } catch (Exception $e) {
            return null;
    ...
    {
        $MR = $this->instance->fetchByID($MRID);
        if(!$MR) return array('result' => 'fail', 'message' => 'MR not found');

        $result = $this->instance->approve($MR, $action, $comment);

        // 重置状态以便后续测试
        if($result['result'] == 'success')
        {
            $this->instance->dao->update(TABLE_MR)->set('status')->eq($MR->status)->set('approvalStatus')->eq('')->where('id')->eq($MRID)->exec();
        }
    ...
    public function approveHistoryTester(int $MRID): array|object
    {
        ...
        $this->instance->approve($MR, 'reject', $comment);

        // 查询审批历史记录
        $approval = $this->instance->dao->select('*')->from(TABLE_MRAPPROVAL)
            ->where('mrID')->eq($MRID)
        ...
        if($approval) {
            // 清理测试数据
            $this->instance->dao->delete()->from(TABLE_MRAPPROVAL)->where('id')->eq($approval->id)->exec();
            return $approval;
        }
    ...
    public function closeTester(int $MRID): array
    {
        ...
        // 更新MR状态为closed用于测试
        $this->instance->dao->update(TABLE_MR)->set('status')->eq('closed')->where('id')->eq($MRID)->exec();
        ...
        {
            $this->instance->dao->update(TABLE_MR)->set('status')->eq($originalStatus)->where('id')->eq($MRID)->exec();
        }
    ...
    public function reopenTester(int $MRID): array
    {
        ...
        $result = $this->instance->reopen($MR);
        if($MR->status != 'opened') $this->instance->close($MR);
        return $result;
    }
    ...
    {
        $result = $this->instance->link($MRID, $type, array(1, 2));
        if(!$result) return $result;

        return $this->instance->dao->select('*')->from(TABLE_RELATION)->orderBy('id_desc')->fetch();
    }
    ...
    {
        $result = $this->instance->unlink($MRID, $type, 1);
        if(!$result) return $result;

        return $this->instance->dao->select('*')->from(TABLE_ACTION)->orderBy('id_desc')->fetch();
    }
    ...
    {
        $MR = $this->instance->fetchByID($MRID);
        $result = $this->instance->linkObjects($MR);
        if(!$result) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_RELATION)->orderBy('id_desc')->fetchAll('id');
    }
    ...
    {
        $MR = $this->instance->fetchByID($MRID);
        return $this->instance->getMRProduct($MR);
    }
    ...
    {
        $MR = $this->instance->fetchByID($MRID);
        $this->instance->logMergedAction($MR);
        if(dao::isError()) return dao::getError();
    ...
    {
        $this->instance->deleteByID($MRID);
        if(dao::isError()) return dao::getError();

        return $this->instance->dao->select('*')->from(TABLE_ACTION)->orderBy('id_desc')->fetchAll('id');
    }
    ...
    public function apiAcceptMrTester(int $MRID): object|null
    {
        $MR = $this->instance->fetchByID($MRID);
        if(!$MR) return null;
        return $this->instance->apiAcceptMR($MR);
    }
    ...
    public function getAllProjectsTest($repo)
    {
        ...
        if(isset($repo->serviceHost) && isset($repo->serviceProject))
        {
            ...
            return $this->instance->{$methodName}((int)$repo->serviceHost, $projectList);
        }
    ...
    {
        $result = $this->instance->apiCreateMRTodo($hostID, $projectID, $MRID);
        if(dao::isError()) return dao::getError();
    ...
    {
        try {
            $result = $this->instance->apiGetDiffVersions($hostID, $projectID, $MRID);
            if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->apiGetSingleDiffVersion($hostID, $projectID, $MRID, $versionID);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->apiGetSingleMR($repoID, $MRID);
        if(dao::isError()) return dao::getError();
    ...
    {
        try {
            $result = $this->instance->getDiffs($MR);
            if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->getPairs($repoID);
        if(dao::isError()) return dao::getError();
