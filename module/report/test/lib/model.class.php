...
class reportModelTest extends baseTest
{
    ...
    {
        $objects = $this->instance->computePercent($datas);
    ...
    public function createSingleJSONTest(int $executionID): bool|array
    {
        ...
        $objects = $this->instance->createSingleJSON($sets, $dateList[0]);
    ...
    {
        $objects = $this->instance->convertFormat($dateList, $format);
    ...
    {
        $objects = $this->instance->getUserBugs();
    ...
    {
        $objects = $this->instance->getUserTasks();
    ...
    {
        $objects = $this->instance->getUserTodos();
    ...
    {
        $objects = $this->instance->getUserTestTasks();
    ...
    {
        $count = $this->instance->getUserYearLogins($accounts, date('Y'));
    ...
    {
        $count = $this->instance->getUserYearActions($accounts, date('Y'));
    ...
    {
        $count = $this->instance->getUserYearContributionCount($accounts, $year);
        if(dao::isError()) return dao::getError();
    ...
    {
        $objects = $this->instance->getUserYearContributions($accounts, date('Y'));
    ...
    {
        $objects = $this->instance->getUserYearTodos($accounts, date('Y'));
    ...
    {
        $object = $this->instance->getUserYearEfforts($accounts, date('Y'));
    ...
    {
        $objects = $this->instance->getUserYearProducts($accounts, date('Y'));
    ...
    public function getUserYearExecutionsTest(array $accounts): string|array
    {
        ...
        $objects = $this->instance->getUserYearExecutions($accounts, $year);
    ...
    {
        $objects = $this->instance->getAllTimeStatusStat();
    ...
    {
        $objects = $this->instance->getYearObjectStat($accounts, date('Y'), $objectType);
    ...
    {
        $objects = $this->instance->getYearCaseStat($accounts, date('Y'));
    ...
    {
        $objects = $this->instance->getYearMonths($year);
    ...
    {
        $return = $this->instance->getStatusOverview($objectType, $statusStat);
    ...
    {
        $objects = $this->instance->getProjectStatusOverview($accounts);
    ...
    {
        $objects = $this->instance->getOutput4API($accounts, date('Y'));
    ...
    {
        $objects = $this->instance->getProjectExecutions();
    ...
    {
        $result = $this->instance->getContributionCountTips($mode);
        if(dao::isError()) return dao::getError();
    ...
    public function getReminderTest(): mixed
    {
        ...
        if($tester->config->report->dailyreminder->bug) $bugs = $this->instance->getUserBugs();
        if($tester->config->report->dailyreminder->task) $tasks = $this->instance->getUserTasks();
        if($tester->config->report->dailyreminder->todo) $todos = $this->instance->getUserTodos();
        if($tester->config->report->dailyreminder->testTask) $testTasks = $this->instance->getUserTestTasks();
    ...
    public function assignAnnualReportTest(string $year, string $dept, string $account): mixed
    {
        ...
        try {
            ...
            // 测试getYearMonths方法
            if(method_exists($this->instance, 'getYearMonths')) {
                $months = $this->instance->getYearMonths($year ?: date('Y'));
                $testResult['monthsCount'] = count($months);
            }

            // 测试用户和部门相关方法
            if(method_exists($this->instance, 'getUserYearContributions')) {
                $accounts = array('admin');
                $contributions = $this->instance->getUserYearContributions($accounts, $year ?: date('Y'));
    ...
    public function assignAnnualDataTest(string $year, string|int $dept, string $account, array $accounts, int $userCount): mixed
    {
        ...
        try {
            ...
            {
                $data['logins'] = $this->instance->getUserYearLogins($accounts, $year);
            }

            $deptEmpty = (int)$dept && empty($accounts);

            // 核心数据获取
            $data['actions'] = $deptEmpty ? 0 : $this->instance->getUserYearActions($accounts, $year);
            $data['todos'] = $deptEmpty ? (object)array('count' => 0, 'undone' => 0, 'done' => 0) : $this->instance->getUserYearTodos($accounts, $year);
            $data['contributions'] = $deptEmpty ? array() : $this->instance->getUserYearContributions($accounts, $year);
            $data['executionStat'] = $deptEmpty ? array() : $this->instance->getUserYearExecutions($accounts, $year);
            $data['productStat'] = $deptEmpty ? array() : $this->instance->getUserYearProducts($accounts, $year);
            $data['storyStat'] = $deptEmpty ? array('statusStat' => array(), 'actionStat' => array()) : $this->instance->getYearObjectStat($accounts, $year, 'story');
            $data['taskStat'] = $deptEmpty ? array('statusStat' => array(), 'actionStat' => array()) : $this->instance->getYearObjectStat($accounts, $year, 'task');
            $data['bugStat'] = $deptEmpty ? array('statusStat' => array(), 'actionStat' => array()) : $this->instance->getYearObjectStat($accounts, $year, 'bug');
            $data['caseStat'] = $deptEmpty ? array('resultStat' => array(), 'actionStat' => array()) : $this->instance->getYearCaseStat($accounts, $year);

            $yearEfforts = $this->instance->getUserYearEfforts($accounts, $year);
            $data['consumed'] = $deptEmpty ? 0 : $yearEfforts->consumed;

            if(empty($dept) && empty($account)) $data['statusStat'] = $this->instance->getAllTimeStatusStat();
