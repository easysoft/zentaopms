...
- 步骤1:测试有效的code和accessToken,返回失败(因为无效的凭证)属性result @fail
...
r($ssoTest->getFeishuUserTokenTest('test_code_001', 'test_token_001')) && p('result') && e('fail'); // 步骤1:测试有效的code和accessToken,返回失败(因为无效的凭证)
...
r($ssoTest->getFeishuUserTokenTest('invalid_code_123', 'invalid_token_456')) && p('result') && e('fail'); // 步骤4:测试另一组无效凭证,返回失败信息
r($ssoTest->getFeishuUserTokenTest('another_invalid_code', 'another_invalid_token')) && p('result') && e('fail'); // 步骤5:测试第三组无效凭证,返回失败信息