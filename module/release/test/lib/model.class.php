...
class releaseModelTest extends baseTest
{
    protected $moduleName = 'release';
    ...
    {
        $objects = $this->instance->getByID($releaseID, $setImgSize);
    ...
    public function createTest(array $data, bool $isSync)
    {
        ...
        $objectID = $this->instance->create($release, $isSync);
    ...
    {
        $release = $this->instance->getById($releaseID);
        return $this->instance->getNotifyPersons($release);
    }
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->linkStory($releaseID, $stories);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->unlinkStory($releaseID, $storyID);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->batchUnlinkStory($releaseID, $storyIdList);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->linkBug($releaseID, $type, $bugs);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->unlinkBug($releaseID, $bugID, $type);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->batchUnlinkBug($releaseID, $type, $bugIdList);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->changeStatus($releaseID, $status);
    ...
    {
        $oldRelease = $this->instance->getByID($releaseID);
        $this->instance->changeStatus($releaseID, $status, $releasedDate);
    ...
    public function getToAndCcListTest($releaseData): false|array
    {
        ...
        {
            $release = $this->instance->getByID($releaseData);
            if(!$release) return false;
        }
        return $this->instance->getToAndCcList($release);
    }
    ...
    public function processReleaseForCreateTest(int $releaseID, bool $isSync): object|false
    {
        ...
        return $this->instance->processReleaseForCreate($release, $isSync);
    }
    ...
    {
        // 记录删除前的状态
        $beforeRelease = $this->instance->dao->select('*')->from(TABLE_RELEASE)->where('id')->eq($releaseID)->fetch();
        ...
        $this->instance->dao->update(TABLE_RELEASE)->set('deleted')->eq('1')->where('id')->eq($releaseID)->exec();
        ...
        {
            $this->instance->dao->update(TABLE_BUILD)->set('deleted')->eq('1')->where('id')->eq($beforeRelease->shadow)->exec();
        }

        // 删除相关的空构建（execution为空且创建时间相同）
        $builds = $this->instance->dao->select('*')->from(TABLE_BUILD)->where('id')->in($beforeRelease->build)->fetchAll();
        foreach($builds as $build)
        {
            if(empty($build->execution) && $build->date == $beforeRelease->createdDate)
            {
                $this->instance->dao->update(TABLE_BUILD)->set('deleted')->eq('1')->where('id')->eq($build->id)->exec();
            }
        }

        // 检查删除后的状态
        $afterRelease = $this->instance->dao->select('*')->from(TABLE_RELEASE)->where('id')->eq($releaseID)->fetch();
    ...
    public function buildActionListTest(int $releaseID): array
    {
        ...
        return $this->instance->buildActionList($release);
    }
    ...
    {
        $release = $this->instance->getByID($releaseID);
        return $this->instance->isClickable($release, $action);
    }
    ...
    {
        $this->instance->dao->update(TABLE_STORY)->set('stage')->eq($stage)->where('id')->eq($storyID)->exec();
        $this->instance->setStoriesStage(1);
        if(dao::isError()) return dao::getError();
    ...
    {
        $releases = $this->instance->getList($productID, 0, $type);
        return $this->instance->processReleaseListData($releases, $childReleases, $addActionsAndBuildLink);
    }
    ...
    {
        $release = $this->instance->getByID($releaseID);
        return $this->instance->processReleaseBuilds($release, $addActionsAndBuildLink);
    }
    ...
    {
        $result = $this->instance->getPageSummary($releases, $type);
        if(dao::isError()) return dao::getError();
    ...
    public function sendmailTest(int $releaseID): string
    {
        ...
        try
        {
            $this->instance->sendmail($releaseID);
    ...
    public function getNotifyListTest(int $releaseID): false|array
    {
        ...
        $result = $this->instance->getNotifyList($release);
        if(dao::isError()) return dao::getError();
    ...
    public function sendMail2FeedbackTest(object $release, string $subject): string
    {
        ...
        if($stories) {
            $storyEmails = $this->instance->dao->select('notifyEmail')->from(TABLE_STORY)
                ->where('id')->in($stories)
        ...
        if($bugs) {
            $bugEmails = $this->instance->dao->select('notifyEmail')->from(TABLE_BUG)
                ->where('id')->in($bugs)
                ->andWhere('notifyEmail')->ne('')
                ->fetchPairs();
        ...
        try {
            ...
            $this->instance->sendMail2Feedback($release, $subject);
    ...
    public function processRelatedTest(int $releaseID, object $release): array
    {
        try {
            // 记录操作前的关联数据数量
            $beforeCount = $this->instance->dao->select('COUNT(*) as count')->from(TABLE_RELEASERELATED)
                ->where('release')->eq($releaseID)
                ->fetch('count');

            $this->instance->processRelated($releaseID, $release);

            if(dao::isError()) return dao::getError();

            // 记录操作后的关联数据数量
            $afterCount = $this->instance->dao->select('COUNT(*) as count')->from(TABLE_RELEASERELATED)
                ->where('release')->eq($releaseID)
                ->fetch('count');

            // 获取所有关联数据
            $relatedData = $this->instance->dao->select('*')->from(TABLE_RELEASERELATED)
                ->where('release')->eq($releaseID)
                ->fetchAll();
    ...
    {
        $result = $this->instance->updateRelated($releaseID, $objectType, $objectIdList);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->deleteRelated($releaseID, $objectType, $objectIdList);
        if(dao::isError()) return dao::getError();
    ...
    {
        // 直接查询父需求ID列表
        $parentIdList = $this->instance->dao->select('id')->from(TABLE_STORY)
    ...
    {
        $result = $this->instance->getBugList($bugIdList, $orderBy, $pager, $type);
        if(dao::isError()) return dao::getError();
