...
- 测试传入产品ID列表属性1 @2
- 测试传入不存在产品ID列表 @0
- 测试传入包含空数据的产品ID列表属性11 @3
- 测试传入包含不存在数据的产品ID列表属性12 @3
...
r($tester->product->getCaseCountByProductIdList($productIdList[1])) && p(1)  && e('2'); // 测试传入产品ID列表
r($tester->product->getCaseCountByProductIdList($productIdList[2])) && p(0)  && e('0'); // 测试传入不存在产品ID列表
r($tester->product->getCaseCountByProductIdList($productIdList[3])) && p(11) && e('3'); // 测试传入包含空数据的产品ID列表
r($tester->product->getCaseCountByProductIdList($productIdList[4])) && p(12) && e('3'); // 测试传入包含不存在数据的产品ID列表
