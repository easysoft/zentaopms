...
class productModelTest extends baseTest
{
    ...
    public function createObject(array $param = array(), string $lineName = ''): object|array
    {
        ...
        $createFields['PMT']            = '';
        $createFields['createdBy']      = $this->instance->app->user->account;
        $createFields['createdDate']    = helper::now();
        $createFields['createdVersion'] = $this->instance->config->version;

        $data = new stdclass();
        foreach($createFields as $field => $defaultValue) $data->$field = zget($param, $field, $defaultValue);

        $objectID = $this->instance->create($data, $lineName);
    ...
    {
        $project = $this->instance->getLatestProject($productID);
    ...
    {
        return $this->instance->getList($programID);
    }
    ...
    {
        return $this->instance->getList($programID, 'noclosed');
    }
    ...
    {
        return $this->instance->getList($programID, 'closed');
    }
    ...
    {
        return $this->instance->getList($programID, 'involved');
    }
    ...
    {
        return $this->instance->getList($programID, 'all', 0, $line);
    }
    ...
    {
        $this->instance->dao->update(TABLE_PRODUCT)->set('deleted')->eq(1)->orderBy('id_desc')->limit(5)->exec(); /* 将最后五个产品设置为影子产品。 */
        $this->instance->dao->update(TABLE_PRODUCT)->set('shadow')->eq(1)->where('id')->ge(18)->andWhere('id')->lt(23)->exec(); /* 将第18到22个产品设置为影子产品。*/
        $pairs = $this->instance->getPairs($mode, $programID, $append, $shadow);
        return $pairs;
    }
    ...
    {
        $this->instance->dao->update(TABLE_PRODUCT)->set('deleted')->eq(1)->orderBy('id_desc')->limit(5)->exec();
        $this->instance->dao->update(TABLE_PRODUCT)->set('shadow')->eq(1)->where('id')->ge(18)->andWhere('id')->lt(23)->exec();
        $pairs = $this->instance->fetchPairs($mode, $programID, $append, $shadow);
        return $pairs;
    }
    ...
    {
        $projects = $this->instance->getProjectListByProduct($productID, 'all');
    ...
    {
        $projects = $this->instance->getProjectListByProduct($productID, $browseType);
        if($projects == array()) return '没有数据';
        return $projects;
    }
    ...
    {
        $projects = $this->instance->getProjectPairsByProduct($productID, 0, 0);
        if($projects == array()) return '没有数据';
        return $projects;
    }
    ...
    {
        $project = $this->instance->getProjectPairsByProduct(10086, 0, $projectID);
    ...
    {
        $product = $this->instance->dao->select('*')->from(TABLE_PRODUCT)->where('id')->eq($productID)->fetch();
        return $this->instance->isClickable($product, $status);
    }
    ...
    {
        return $this->instance->checkPriv($productID);
    }
    ...
    {
        $object = $this->instance->getById($productID);
    ...
    {
        $products = $this->instance->getByIdList($productIDList);
    ...
    {
        $objects = $this->instance->getPairsByProjectModel($model);
    ...
    {
        $this->instance->app->user->admin = true;
        $objects = $this->instance->getProducts($projectID, $status);
    ...
    {
        $object = $this->instance->getProductIDByProject($projectID);
    ...
    {
        $products = $this->instance->getOrderedProducts($status, $num, $projectID, $shadow);
        if(dao::isError()) return dao::getError();
    ...
    {
        $objects = $this->instance->getMultiBranchPairs($programID);
    ...
    {
        return $this->instance->batchUpdate($products);
    }
    ...
    {
        $this->instance->doUpdate($product, $productID, $programID);

        if(dao::isError()) return dao::getError();
        return $this->instance->getById($productID);
    }
    ...
    {
        $this->instance->manageLine($lines);
        ...
        {
            return $this->instance->dao->select('id,root,name')->from(TABLE_MODULE)->where('type')->eq('line')->orderby('id desc')->fetchAll('id');
        }
    }
    ...
    {
        $this->instance->syncProgramToProduct($programID, $lineID);
        ...
        else
        {
            ...
            {
                return $this->instance->dao->select('id,program')->from(TABLE_PRODUCT)->where('line')->eq($lineID)->orderby('id desc')->fetch();
            }
            elseif($checkType == 'action')
            {
                $products = $this->instance->dao->select('id')->from(TABLE_PRODUCT)->where('line')->eq($lineID)->fetchPairs('id');
                return $this->instance->dao->select('*')->from(TABLE_ACTION)->where('objectID')->eq(current($products))->orderby('id desc')->fetch();
            }
        }
    }
    ...
    {
        $objects = $this->instance->batchGetStoryStage($stories);
    ...
    {
        $this->instance->loadModel('program')->refreshStats(true);
        $objects = $this->instance->getProjectStatsByProduct($productID, $browseType, $branch, $involved, $orderBy, $pager);
    ...
    public function buildExecutionPairsTest(string $mode = '', bool $withBranch = false): array
    {
        ...
        $executions = $this->instance->dao->select('t2.id,t2.name,t2.project,t2.grade,t2.path,t2.parent,t2.attribute,t2.multiple,t3.name as projectName')->from(TABLE_PROJECTPRODUCT)->alias('t1')
        ...
        $objects = $this->instance->buildExecutionPairs($executions, $mode, $withBranch);
        if(dao::isError())
        {
            return dao::getError();
        }
    ...
    public function processRoadmapTest($productID, $branch = '0')
    {
        ...
        $objects = $this->instance->processRoadmap($roadmapGroups, $branch);
    ...
    {
        $objects = $this->instance->getStatByID($productID);
    ...
    {
        $this->instance->refreshStats(true);
        $objects = $this->instance->getStats($productIdList);
    ...
    {
        $this->instance->config->product->showAllProjects = true;
        $objects = $this->instance->getStats4Kanban();
    ...
    {
        $objects = $this->instance->getLinePairs($programID);
    ...
    {
        $productStats = $this->instance->getStats($productIdList);
        $objects      = $this->instance->statisticProgram($productStats);
    ...
    public function statisticDataTest($product)
    {
        if($product->line)
        {
            ...
            $data = $this->instance->statisticData('line', $productStructure, $product);
        }
        ...
        if($product->program)
        {
            ...
            $data = $this->instance->statisticData('program', $productStructure, $product);
        }
    ...
    {
        $records = $this->instance->getPagerProductsWithProgramIn($productIDs, $pager);
    ...
    {
        $lineID = $this->instance->createLine($programID, $lineName);
    ...
    public function concatProductLineTest(array $productIdList): array
    {
        ...
        $products = $this->instance->dao->select('*')->from(TABLE_PRODUCT)->where('id')->in($productIdList)->fetchAll();
        return $this->instance->concatProductLine($products);
    }
    ...
    {
        $libID = $this->instance->createMainLib($productID);
    ...
    {
        /* Reset data. */
        $this->instance->lang->product->moreSelects['willclose'] = 'willcose';
        $this->instance->lang->product->menu->settings['link'] = "Settings|product|view|productID=%s";
        $this->instance->lang->product->menu->settings['subMenu']->branch = array('link' => "@branch@|branch|manage|product=%s", 'subModule' => 'branch');

        $this->instance->setMenu($productID, $branch, $extra);

        $hasBranch      = (int) isset($this->instance->lang->product->menu->settings['subMenu']->branch);
        $requirement    = (int)!isset($this->instance->lang->product->moreSelects['willclose']);
        $idReplaced     = (int)(strpos($this->instance->lang->product->menu->settings['link'], '%s') === false);
        $branchReplaced = (int)($hasBranch and strpos($this->instance->lang->product->menu->settings['subMenu']->branch['link'], '@branch@') === false);
    ...
    {
        $this->instance->updateOrder($sortedIdList);

        $products = $this->instance->dao->select('id')->from(TABLE_PRODUCT)->orderBy('`order`')->fetchAll('id');
    ...
    {
        $this->instance->deleteLine($lineID);
        return $this->instance->dao->select('*')->from(TABLE_MODULE)->where('id')->eq($lineID)->fetch();
    }
    ...
    {
        $this->instance->deleteByID($productID);
        return $this->instance->getByID($productID);
    }
    ...
    public function statisticProductDataTest(string $type, array $productIdList, int $productID)
    {
        ...
        $programStructure = $this->instance->statisticProgram($productStats);

        return $this->instance->statisticProductData($type, $programStructure, $product);
    }
    ...
    {
        $this->instance->buildProductSearchForm($queryID, 'searchUrl');
    ...
    {
        $product = $this->instance->getByID($productID);
        if(empty($product)) return 0;

        $products = $this->instance->loadModel('product')->getProducts($productID);
        $this->instance->buildSearchForm($productID, $products, $queryID, 'searchStory', 'story');
    ...
    {
        $product = $this->instance->getStats(array($productID));
        ...
        return $this->instance->formatDataForList($product[$productID], array());
    }
    ...
    {
        $stories = $this->instance->getStories($productID, 'all', 'unclosed', 0, 0, $type);

        return str_replace('%', '%%', $this->instance->summary($stories, $type));
    }
    ...
    {
        $productList = $this->instance->getByIdList($productIdList);

        return $this->instance->getProjectProductList($productList);
    }
    ...
    {
        $planList = $this->instance->loadModel('productplan')->getList($productID, $branch);
        return $this->instance->filterOrderedAndParentPlans($planList);
    }
    ...
    {
        $releases = $this->instance->loadModel('release')->getList($productID, $branch);
        return $this->instance->getRoadmapOfReleases(array(), $releases, $branch, $count);
    }
    ...
    public function getRoadmapOfPlansTest(int $productID, string $branch, int $count): array
    {
        $roadmap  = array();
        $planList = $this->instance->loadModel('productplan')->getList($productID, $branch);
        list($orderedPlans, $parentPlans) = $this->instance->filterOrderedAndParentPlans($planList);

        return $this->instance->getRoadmapOfPlans($orderedPlans, $parentPlans, $branch, $count);
    }
    ...
    {
        $oldProduct = $this->instance->getByID($productID);
    ...
    {
        $productStories = $this->instance->getStoryStats($productIdList);
        if(dao::isError()) return dao::getError();
    ...
    {
        $productBugs = $this->instance->getBugStats($productIdList);
        if(dao::isError()) return dao::getError();
    ...
    {
        $products = $this->instance->getProductStats($productIdList);
        if(dao::isError()) return dao::getError();
    ...
    public function refreshStatsTest(): array
    {
        ...
        return $this->instance->getProducts();
    }
    ...
    {
        $result = $this->instance->buildSearchConfig($productID, $storyType);
        if(dao::isError()) return dao::getError();
    ...
    {
        $result = $this->instance->filterNoCasesStory($storyIDList);
        if(dao::isError()) return dao::getError();
    ...
    public function setMenu4AllTest(): array
    {
        ...
        $products = $this->instance->getPairs();
    ...
    public function setTrackMenuTest(int $productID, string $branch, int $projectID): array
    {
        ...
        try {
            ...
            if($projectID <= 0) {
                ...
                $product = $this->instance->getByID($productID);
                if($product) {
                    $checkedProductID = $this->instance->checkAccess($productID, array($productID => $product->name));
    ...
    public function setShowErrorNoneMenu4QATest(string $activeMenu): array
    {
        ...
        $this->instance->view = new stdClass();
        $this->instance->view->moduleName = 'qa';
    ...
    public function setShowErrorNoneMenu4ExecutionTest(string $activeMenu, int $executionID): array
    {
        ...
        try {
            ...
            $execution = $this->instance->dao->select('*')->from(TABLE_PROJECT)->where('id')->eq($executionID)->andWhere('type')->in('sprint,stage,kanban')->fetch();
    ...
    public function setSelectFormOptionsTest(int $programID, array $fields): array
    {
        ...
        $users = $this->instance->loadModel('user')->getPairs('nodeleted|noclosed');
        ...
        foreach($fields as $field => $attr)
        {
            ...
            if(!isset($fields[$field]['title'])) $fields[$field]['title'] = zget($this->instance->lang->product, $field, $field);
        }

        // 设置下拉菜单内容
        if(isset($fields['groups']))  $fields['groups']['options']  = $this->instance->loadModel('group')->getPairs();
        if(isset($fields['program'])) $fields['program']['options'] = $this->instance->loadModel('program')->getTopPairs('noclosed');
        if(isset($fields['line']))    $fields['line']['options']    = $this->instance->getLinePairs($programID, true);

        if($this->instance->config->edition != 'open' && isset($fields['workflowGroup']))
        {
            $groupPairs = $this->instance->loadModel('workflowGroup')->getPairs('product', 'scrum', 1, 'normal', '0');
            $fields['workflowGroup']['options'] = $this->instance->workflowGroup->appendBuildinLabel($groupPairs);
        }
    ...
    public function getFormFields4EditTest(object $product): array
    {
        ...
        if(isset($fields['program']) && !isset($fields['program']['options'][$programID]) && $programID) {
            $program = $this->instance->dao->select('id,name')->from(TABLE_PROGRAM)->where('id')->eq($programID)->fetch();
            if($program) {
    ...
    public function getProductLinesTest(array $programIdList = array(), string $check = 'count'): mixed
    {
        ...
        $productLines = $this->instance->getLines($programIdList);
    ...
    public function getExportFieldsTest(string $systemMode = 'normal', bool $hasExtendFields = false, bool $hasHeaderGroup = false): array
    {
        ...
        try {
            ...
            $zenObject = new class($this->instance) {
                private $objectModel;

                public function __construct($objectModel) {
                    $this->instance = $objectModel;
                    global $config;
                    $this->config = $config;
                }
    ...
    public function responseAfterBatchEditTest(int $programID, string $appTab = 'program'): array
    {
        ...
        $extendedProductZen = new class($this->instance, $appTab) extends productZen {
            private $objectModel;
            public $app;
            public $lang;
            private $testAppTab;
    ...
    public function getModuleTreeTest(int $projectID, int $productID, string $branch, int $param, string $storyType, string $browseType, bool $tutorialMode = false, string $rawModule = 'product'): mixed
    {
        ...
        try {
            ...
            // 步骤4：产品检查 - 验证产品ID是否有效
            if ($productID > 0) {
                $product = $this->instance->dao->select('*')->from(TABLE_PRODUCT)->where('id')->eq($productID)->andWhere('deleted')->eq(0)->fetch();
                if (!$product) {
            ...
            if ($productID > 0) {
                // 正常情况返回树形菜单字符串
                $modules = $this->instance->dao->select('*')->from(TABLE_MODULE)
    ...
    public function getBranchOptionsTest(array $projectProducts, int $projectID)
    {
        ...
        try {
            ...
            foreach ($branchProducts as $product) {
                ...
                // 模拟获取分支列表
                $branches = $this->instance->dao->select('*')->from(TABLE_BRANCH)
                ...
                if ($projectID > 0) {
                    // 根据项目ID过滤分支
                    $projectBranches = $this->instance->dao->select('branch')->from(TABLE_PROJECTPRODUCT)
