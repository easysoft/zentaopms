...
- 步骤2：使用session中jiraRelation的正常转换 @wait
- 步骤3：未找到匹配状态的情况 @wait
- 步骤4：空参数的边界值测试 @wait
- 步骤5：relations为空数组但session有数据的情况 @wait
...
r($convertTest->convertStageTest('in-progress', '1', array())) && p() && e('wait'); // 步骤2：使用session中jiraRelation的正常转换
...
r($convertTest->convertStageTest('done', '2', array())) && p() && e('wait'); // 步骤5：relations为空数组但session有数据的情况
