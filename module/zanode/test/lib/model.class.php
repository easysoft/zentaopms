...
class zanodeModelTest extends baseTest
{
    ...
    public function constructTest(): object
    {
        ...
        $result->objectModel = is_a($this->instance, 'zanodeModel');
        return $result;
    }
    ...
    public function setAutomationSettingTest(object $object): object|array
    {
        ...
        $return = $this->instance->getAutomationByID($resultID);
        return $return;
        return $this->instance->getAutomationByID($resultID);
    }
    ...
    public function createTest(object $data): object|array
    {
        ...
        return $this->instance->dao->select('*')->from(TABLE_ZAHOST)->where('id')->eq($nodeID)->fetch();
    }
    ...
    {
        // 记录创建前的action数量
        $beforeActionCount = $this->instance->dao->select('COUNT(*) as count')->from(TABLE_ACTION)->fetch();

        $nodeID = $this->create($data);
        if(dao::isError()) return dao::getError();

        // 检查创建后的action数量
        $afterActionCount = $this->instance->dao->select('COUNT(*) as count')->from(TABLE_ACTION)->fetch();

        // 查找新创建的action记录
        $actionRecord = $this->instance->dao->select('*')->from(TABLE_ACTION)
    ...
    public function createSnapshotTest(int $nodeID, string $nodeIP, int $hzap, string $token, array $data): object|string
    {
        ...
        $createdSnapshot = $this->instance->dao->select('*')->from(TABLE_IMAGE)->where('id')->eq($snapshotID)->fetch();
        if($createdSnapshot)
        {
    ...
    public function createDefaultSnapshotTest(int $nodeID): object|array
    {
        ...
        $snapshot = $this->instance->dao->select('*')->from(TABLE_IMAGE)->where('id')->eq($result)->fetch();
        if(!$snapshot)
        {
    ...
    public function editSnapshotTest(int $snapshotID, object $data): object|array
    {
        ...
        $result = $this->instance->dao->select('*')->from(TABLE_IMAGE)->where('id')->eq($snapshotID)->fetch();
    ...
    public function updateImageStatusTest(int $imageID, object $data): object|bool
    {
        ...
        $result = $this->instance->dao->select('status,path')->from(TABLE_IMAGE)->where('id')->eq($imageID)->fetch();
        if(dao::isError()) return dao::getError();
        return $result ?: false;
    }
    ...
    public function createImageTest(int $zanodeID, object $data): mixed
    {
        ...
        // 检查节点是否存在
        $node = $this->instance->getNodeByID($zanodeID);
        if(!$node) return false;

        $result = $this->instance->createImage($zanodeID, $data);
        if(dao::isError()) return dao::getError();
    ...
    {
        $node = $this->instance->dao->select("t1.*, t2.name as hostName, if(t1.hostType='', t2.extranet, t1.extranet) ip,t2.zap as hzap,if(t1.hostType='', t3.osName, t1.osName) osName, if(t1.hostType='', t2.tokenSN, t1.tokenSN) tokenSN, if(t1.hostType='', t2.secret, t1.secret) secret")
        ...
        return $method->invoke($this->instance, $node);
    }
    ...
    public function setMenuTest(bool $createHost): object
    {
        ...
        if(!$createHost)
        {
            ...
            $this->instance->dao->delete()->from(TABLE_ZAHOST)->where('type')->eq('zahost')->exec();
        }
        else
        {
            ...
            $this->instance->dao->insert(TABLE_ZAHOST)->data($hostData)->exec();
        }
        ...
        if($createHost)
        {
            $this->instance->dao->delete()->from(TABLE_ZAHOST)->where('name')->eq('test_host_for_menu')->exec();
        }
