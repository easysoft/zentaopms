...
foreach($components as $component)
{
    if(isset($component->sourceID) && $component->sourceID)
    {
        ...
        {
            if(!isset($componentList['waterpolo']) && $chart->type == 'waterpolo' && $chart->id == 10209)
            {
...
$waterpolo = isset($componentList['waterpolo']) ? $screen->buildWaterPoloTest($componentList['waterpolo'], $chart) : null;
...
$expectedRecord  = $tester->dao->query($chart->sql)->fetch();
$expectedDataset = round((float)zget($expectedRecord, 'status', 0), 3);
r($waterpolo && round((float)$waterpolo->option->dataset, 3) === $expectedDataset) && p('') && e('1');  //判断生成的水球图数据是否正确。
