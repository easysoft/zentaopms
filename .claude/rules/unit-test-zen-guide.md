...
// 4. 创建测试实例（变量名与模块名一致）
${moduleName}Test = new {moduleName}ZenTest();
...
class {moduleName}ZenTest extends baseTest
{
...
php module/{moduleName}/test/zen/{methodName}.php

# 使用 ztf 运行测试脚本
test/runtime/ztf module/{moduleName}/test/zen/{methodName}.php
```
...
1. **添加测试文件到git（仅添加测试相关文件）**：
   ...
   ```bash
   git add module/{moduleName}/test/lib/zen.class.php
   git add module/{moduleName}/test/zen/{methodName}.php
   git add module/{moduleName}/test/zen/yaml/{methodName}/{tableName}.yaml
   ```
