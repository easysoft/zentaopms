...
// 4. 创建测试实例（变量名与模块名一致）
${moduleName}Test = new {moduleName}TaoTest();
...
class {moduleName}TaoTest extends baseTest
{
...
php module/{moduleName}/test/tao/{methodName}.php

# 使用 ztf 运行测试脚本
test/runtime/ztf module/{moduleName}/test/tao/{methodName}.php
```
...
1. **添加测试文件到git（仅添加测试相关文件）**：
   ...
   ```bash
   git add module/{moduleName}/test/lib/tao.class.php
   git add module/{moduleName}/test/tao/{methodName}.php
   git add module/{moduleName}/test/tao/yaml/{methodName}/{tableName}.yaml
   ```
