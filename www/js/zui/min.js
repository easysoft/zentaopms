/*!
 * ZUI: Zentao template - v1.8.1 - 2018-04-25
 * http://zui.sexy
        ...
        if ($template.length) {
            ...
            var createRow = function(id, $after) {
                ...
                if ($.fn.chosen && options.chosen) {
                    $row.find('.chosen').chosen($.isPlainObject(options.chosen) ? options.chosen : null);
                }

                if ($.fn.datetimepicker && options.datetimepicker) {
                    $row.datepickerAll($.isPlainObject(options.datetimepicker) ? options.datetimepicker : null);
                }

                if ($.fn.colorPicker && options.colorPicker) {
                    $row.find('input.colorpicker').colorPicker($.isPlainObject(options.colorPicker) ? options.colorPicker : null);
                }
                var ctrlIndex = 0;
                $row.find('input[type!="hidden"],textarea,select').each(function() {
                    var $ctrl = $(this);
...
(function($, undefined) {
    ...
    var Table = function(element, options) {
        ...
        // Initialize here
        var $table = that.$table = $element.is('table') ? $element : $element.find('table');
        that.$header = $table.find('thead');
        that.$footer = $element.find('.table-footer');
        ...
        if (options.checkable) {
            ...
            }).on('click', 'tbody a,tbody .btn', function(e) {
                e.stopPropagation();
            });
        ...
        that.$statistic = $element.find('.table-statistic');
        that.defaultStatistic = that.$statistic.html();
    ...
    Table.prototype.updateStatistic = function() {
        ...
        if (!statisticCols && statisticCols !== false) {
            ...
            that.$header.find('th').each(function(index) {
                var $th = $(this);
    ...
    Table.prototype.fixHeader = function() {
        ...
        var $fixHeaderTable = that.$.find('table.fixed-header-copy');
        if (isFixed) {
            if (!$fixHeaderTable.length) {
                $fixHeaderTable = $('<table class="fixed-header-copy"></table>').addClass(that.$table.attr('class')).append($header.clone()).insertAfter(that.$table);
            }
            $fixHeaderTable.css({
                top: 0,
                left: bounding.left,
    ...
    Table.prototype.fixFooter = function() {
        ...
        var bounding = that.$.find('tbody')[0].getBoundingClientRect();
        var fixFooterOption = that.options.fixFooter;
    ...
    Table.prototype.updateCheckUI = function() {
        ...
        $rows.each(function(index) {
            var $row = $(this);
            isRowChecked = $row.hasClass('checked');
            $row.toggleClass('row-check-begin', isRowChecked && !isLastRowChecked);
            ...
            if (rowsCount === (index + 1)) $row.toggleClass('row-check-end', isRowChecked);
        });
        this.$.toggleClass('has-row-checked', checkCount > 0).find('.check-all').toggleClass('checked', checkCount === rowsCount);
...
(function ($, window, undefined) {
    ...
    var DEFAULT = {
        ...
        dataType: 'json',
        errControlStyle: {borderColor: '#D2322D'}
    };
    var ajaxFormUrl = '';

    $.fn.enableForm = function(enable) {
    ...
    $.ajaxForm = function(form, options) {
        ...
        var errControlStyle = options.errControlStyle;
        delete options.errControlStyle;
        options = $.extend({
                ...
                if (response.result === 'success') {
                    ...
                    if (typeof message === 'string') {
                        ...
                        $.each(message, function(key, value) {
                            ...
                            $control.css(errControlStyle).addClass('has-error');

                            var removeLabel = function() {
                                var $theLabel = $('#' + errMsgLabelId);
                                if ($theLabel.length) {
                                    $theLabel.remove();
                                    $control.removeAttr('style').removeClass('has-error');
                                    return true;
                                }
                            };
                            ...
                            if ($chosen.length) {
                                $chosen.find('.chosen-single').css(errControlStyle).addClass('has-error').on('mousedown', function() {
                                    if (removeLabel() === true) {
                                        $(this).removeAttr('style').removeClass('has-error');
                                    }
                                });
                            }
    ...
    $.fn.setInputRequired = function() {
        return this.each(function() {
            ...
            if ($parent.is('.input-control')) {
                $parent.addClass('required');
            } else if ($ctrl.is('.chosen')) {
                $ctrl.next('.chosen-container').addClass('required');
            }
        });
    };
    ...
    $(function() {
        ...
        // Set required
        $('input[required],select[required]').setInputRequired();
    });
}(jQuery, window, undefined));
...
(function($) {
    ...
    var SearchList = function(element, options) {
        ...
        if ($dropdownMenu.length) {
            ...
            $dropdownMenu.parent().on('shown.zui.dropdown', function() {
                setTimeout(function() {
    ...
    SearchList.prototype.loadRemote = function() {
        var that = this;
        var options = that.options;
    ...
    LabelSelector.prototype.update = function() {
        ...
        that.$.children('option').each(function() {
            ...
            if (options.labelCreator) {
                ...
                if ((option.value === '' || option.value === '0') && !option.label) {
...
(function($) {
    ...
    FileInput.prototype.getFile = function() {
        return this.$input.prop('files')[0];
    };
    ...
    $.fn.fileInputList = function(option) {
        return this.each(function() {
            ...
            if(!data) $this.data(NAME, (data = new FileInputList(this, options)));

            if(typeof option == 'string') data[option]();
        });
    };

    FileInputList.NAME = NAME;
...
(function($) {
    ...
    */
    ...
    $.fn.timeSpanControl = function(options) {
        return this.each(function() {
            ...
            var updateEndInput = function() {
                ...
                if (startVal) {
                    const end = convertNumToTime(convertTimeToNum(startVal) + 30);
    ...
    $.fn.chosen = function(options) {
        return this.each(function() {
            var $this = $(this);
            return $this._chosen($.extend({}, $this.hasClass('chosen-simple') ? $.chosenSimpleOptions : $.chosenDefaultOptions, options));
        });
    };
    ...
    $.extend($.fn.pager.Constructor.DEFAULTS, {
        ...
        elements: ['total_text', 'size_menu', 'prev_icon', 'nav', 'next_icon'],
        onPageChange: function(state, oldState) {
            console.log('onPageChange', state, oldState);
    ...
    */
    ...
    var initHistories = function(element, options) {
        ...
        $element.on('click', '.btn-comment', function() {
    ...
    var initSidebar = function() {
        var $sidebar = $('#sidebar');
        if (!$sidebar.length || $sidebar.data('init')) return;
        sidebarId = 'sidebar:' + ($sidebar.data('id') || (config.module + '/' + config.method));
        var $main = $('main');
    ...
    // Query box
    $.toggleQueryBox = function(toggle) {
    ...
    $('a.iframe').modalTrigger({type: 'iframe'});
     ...
     * @return void
     */
    $.fixedTableHead = window.fixedTableHead = function(boxObj)
    {
        $(boxObj).scroll(function()
        {
            var hasFixed  = $(this).find('.table-fixed-head').size() > 0;
            if(!hasFixed)
            {
                $(this).css('position', 'relative');
                if($(this).find('table').size() == 1)
                {
                    var fixed = "<table class='table-fixed-head' style='position:absolute;top:0px'><thead>" + $(this).find('table thead').html() + '</thead></table>';
                    $(this).prepend(fixed);
                    var $fixTable = $(this).find('table.table-fixed-head');
            ...
            $(this).find('table.table-fixed-head').css('top',$(this).scrollTop());
        });
    }
}(jQuery));
