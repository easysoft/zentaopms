/*!
 * ZUI: Zentao template - v1.8.1 - 2018-05-18
 * http://zui.sexy
...
(function($, window, undefined) {
        ...
        if(options.type === 'custom' && custom) {
            ...
            if(options.type === 'iframe') {
                ...
                frame.onload = frame.onreadystatechange = function() {
                    console.log('frame loaded', options);
    ...
    ModalTrigger.prototype.close = function(callback, redirect) {
        ...
        if(callback || redirect) {
            that.$modal.on('hidden' + ZUI_MODAL, function() {
                ...
                if(typeof redirect === STR_STRING && !that.$modal.data('cancel-reload')) {
    ...
    var closeModal = function(modal, callback, redirect) {
        ...
        console.trace('closeModal', modal, callback, redirect, $('body').hasClass('modal-open'), $('.modal.in'), document.title);
...
(function($, undefined) {
    ...
    var Table = function(element, options) {
        ...
        if (options.checkable) {
            $element.on('click', '.check-all', function() {
                ...
                if ($(e.target).closest('.btn,a,.not-check').length) return;
                that.checkRow($(this));
    ...
    Table.prototype.getTable = function() {
        var $element = this.$;
        return $element.is('table') ? $element : $element.find('table');
    };
    ...
    Table.prototype.fixHeader = function() {
        ...
        if (isFixed) {
            if (!$fixHeaderTable.length) {
                $fixHeaderTable = $('<table class="fixed-header-copy"></table>').addClass($table.attr('class')).append($header.clone()).insertAfter($table);
            }
            $fixHeaderTable.css({
                top: 0,
                left: bounding.left,
...
if($.zui.browser.ie && $.zui.browser.ie < 10)
{
    var $uploader = $('#uploader');
    $uploader.on('click', '.uploader-btn-browse', function()
    {
        $uploader.find('input[type="file"]:first').click();
    });
}
...
(function($) {
    ...
    */
    ...
    // Searchbox
    var setSearchType = $.setSearchType = function(type) {
    ...
    var gotoObject = $.gotoObject = function(type, id) {
    ...
    */
    ...
    var initHistories = function(element, options) {
        ...
        var $commentModal = $element.find('.modal-comment').modal({
            ...
            position: function(modal) {
                ...
                var modalWidth = Math.min(670, $dialog.outerWidth());
    ...
    $(function()
    {
        $('a.iframe').each(function() {
            var $a = $(this);
            if ($a.parents('[data-ride="table"]').length) return;
            $a.modalTrigger();
        });
    });
    ...
    $.fixedTableHead = window.fixedTableHead = function(boxObj)
    {
        ...
        {
            var $tableBox = $box;
            var hasFixed  = $box.find('.table-fixed-head').size() > 0;
            if(!hasFixed)
            {
                $box.css('position', 'relative');
                if($box.find('table').size() == 1)
                {
                    var fixed = "<table class='table-fixed-head' style='position:absolute;top:0px'><thead>" + $box.find('table thead').html() + '</thead></table>';
                    $box.prepend(fixed);
                    var $fixTable = $box.find('table.table-fixed-head');
                    $fixTable.addClass($box.find('table:last').attr('class'));
                    var $dataTable = $box.find('table:last thead th');
                    $fixTable.find('thead th').each(function(i){$fixTable.find('thead th').eq(i).width($dataTable.eq(i).width());})
                }
            }
            $box.find('table.table-fixed-head').css('top',$box.scrollTop());
        });
    };
    ...
    $(function() {
        var $main = $('#main,#mainContent');
        if ($main.hasClass('fade')) {
            setTimeout(function() {
                $main.addClass('in');
            }, 200);
        }
