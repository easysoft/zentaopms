...
class helper extends baseHelper
{
     ...
     * @return array[]
     */
    public static function parseSqlToSemantic(string $sql): array
    {
        $sql = trim($sql);
        if($sql === '') return [];
        ...
        if(preg_match('/^create\s+(or\s+replace\s+)?view\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))               return [['mode' => 'create', 'action' => 'createView',  'view'  => static::extractTableName($matches[2])]]; // CREATE VIEW / CREATE OR REPLACE VIEW
        if(preg_match('/^drop\s+view\s+(if\s+exists\s+)?((?:`[^`]*`|\S)+)/i', $sql, $matches))                  return [['mode' => 'delete', 'action' => 'dropView',    'view'  => static::extractTableName($matches[2])]]; // DROP VIEW
        if(preg_match('/^create\s+(unique\s+)?index\s+`?([\w]+)`?\s+on\s+((?:`[^`]*`|\S)+)/i', $sql, $matches)) return [['mode' => 'create', 'action' => 'addIndex',    'table' => static::extractTableName($matches[3]), 'index' => static::extractTableName($matches[2])]]; // CREATE INDEX idx ON table_name
        if(preg_match('/^drop\s+index\s+`?([\w]+)`?\s+on\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))               return [['mode' => 'delete', 'action' => 'dropIndex',   'table' => static::extractTableName($matches[2]), 'index' => static::extractTableName($matches[1])]]; // DROP INDEX idx ON table_name
        if(preg_match('/^create\s+table\s+(if\s+not\s+exists\s+)?((?:`[^`]*`|\S)+)/i', $sql, $matches))         return [['mode' => 'create', 'action' => 'createTable', 'table' => static::extractTableName($matches[2])]]; // CREATE TABLE
        if(preg_match('/^drop\s+table\s+(if\s+exists\s+)?((?:`[^`]*`|\S)+)/i', $sql, $matches))                 return [['mode' => 'delete', 'action' => 'dropTable',   'table' => static::extractTableName($matches[2])]]; // DROP TABLE
        if(preg_match('/^rename\s+table\s+((?:`[^`]*`|\S)+)\s+to\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))       return [['mode' => 'update', 'action' => 'renameTable', 'old'   => static::extractTableName($matches[1]), 'new' => static::extractTableName($matches[2])]]; // RENAME TABLE
        if(preg_match('/^(insert|replace)\s+into\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))                       return [['mode' => 'create', 'action' => 'insertValue', 'table' => static::extractTableName($matches[2])]]; // INSERT / REPLACE
        if(preg_match('/^update\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))                                        return [['mode' => 'update', 'action' => 'updateValue', 'table' => static::extractTableName($matches[1])]]; // UPDATE
        if(preg_match('/^delete\s+from\s+((?:`[^`]*`|\S)+)/i', $sql, $matches))                                 return [['mode' => 'delete', 'action' => 'deleteValue', 'table' => static::extractTableName($matches[1])]]; // DELETE

        /* ALTER TABLE */
        $sql = str_replace("\n", ' ', $sql); // 将换行替换为空格，方便后续处理
        if(preg_match('/^alter\s+table\s+((?:`[^`]*`|\S)+)\s+(.+)/i', $sql, $matches))
        {
            $tableName = static::extractTableName($matches[1]);
            ...
            $results = [];
            $clauses = static::splitAlterClauses($alterBody);
     ...
     */
    private static function extractTableName(string $full): string
    {
