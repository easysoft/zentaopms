...
测试创建重复的产品 >> 『产品代号』已经有『testcase1』这条记录了。如果您确定该记录已删除，请到后台-系统-数据-回收站还原。
...
r($createtest->createObject($t_noid))         && p('code:0')              && e('『产品代号』不能为空。'); // 测试不填产品代号的情况
r($createtest->createObject($t_repeat))       && p('code:0')              && e('『产品代号』已经有『testcase1』这条记录了。如果您确定该记录已删除，请到后台-系统-数据-回收站还原。'); // 测试创建重复的产品
r($createtest->createObject($t_noname))       && p('name:0')              && e('『产品名称』不能为空。'); // 测试不填产品名称的情况
...
r($createtest->createObject($t_noacl))        && p('acl:0')               && e('『访问控制』不符合格式，应当为:『/open|private|custom/』。'); // 测试不填权限控制的情况
system("./ztest init");