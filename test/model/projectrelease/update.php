...
测试releaseID正常存在，name在当前发布存在，date正常存在，可修改 >> date,2022-03-03,2022-03-31
...
测试releaseID正常存在，name在其他发布存在，date正常，不可修改 >> 『name』已经有『产品发布9』这条记录了。如果您确定该记录已删除，请到后台-系统-数据-回收站还原。

*/

$releaseID = array(1, 100, '');
$name      = array('产品正常的正常的发布1', '产品发布9', '产品正常的正常的发布2', '');
...
r($projectrelease->updateTest($releaseID[0], $name[0], $date[0])) && p('0:field,old,new') && e('date,2022-03-03,2022-03-31');            //测试releaseID正常存在，name在当前发布存在，date正常存在，可修改
