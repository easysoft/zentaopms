...
class ThinkMatrixOptions extends zui.Component
{
    ...
    afterInit()
    {
        ...
        $element.draggable(
        {
            ...
            onDragOver: (event, dragElement, dropElement) =>
            {
                ...
                const dropSide = event.clientY > (dropBounding.top + dropBounding.height / 2) ? 'right' : 'left';
            ...
            `<div class="think-multiple-item flex-1 py-1.5" data-id="${item.id}">`,
                ...
                '<div class="flex-1">',
                    '<div class="form-control">',
                        `<input class="think-multiple-item-text w-full h-full" placeholder="${options.enterPlaceholder}${item.index + 1}" value="${name}" />`,
                    '</div>',
    ...
    _renderRow(item, $preRow, $list, $rows)
    {
        ...
        $row.find('.think-multiple-item-text').attr('name', `${options.name}[${item.name}]`);
        $row.find('.think-multiple-col-delete .btn').toggleClass('disabled', hasSub || this._items.length == 1);
        $row.find('.think-multiple-col-delete .btn.disabled').attr('title', options.deleteTip);
        return $row;
    }
    ...
    render()
    {
        ...
        const $rows = $list.find('.think-multiple-item').addClass('is-expired');
        let $preRow = null;
        items.forEach(item =>
        {
        ...
        if(!this.$element.find('.think-multiple-body .think-multiple-buttons').length) this.$element.find('.think-multiple-body').append(`<div class="think-multiple-buttons"><button type="button" class="btn ghost text-primary rounded size-sm square btn-action" data-action="sib"><i class="icon icon-plus"></i>${this.options.addColText}</button></div>`);
    }
