...
class ThinkMatrixOptions extends zui.Component
{
    ...
    saveData()
    {
        ...
        $.cookie.set('thinkRequiredCols', JSON.stringify(values));
        const url = this.$element.closest('.form').attr('action');
        loadPartial(url, '.required-options');
    }
            ...
            `<div class="think-multiple-item flex-1 py-1.5" data-id="${item.id}">`,
                ...
                '<div class="flex-1">',
                    '<div class="form-control">',
                        `<input class="think-multiple-item-text w-full h-full" placeholder="${options.colPlaceholder}${item.index + 1}" value="${name}" />`,
                    '</div>',
    ...
    _renderCol(item, $preCol, $list, $cols)
    {
        ...
        $col.find('.think-multiple-item-text').attr('name', `${options.colName}[${item.name}]`);
