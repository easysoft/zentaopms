...
class ThinkMatrixOptions extends zui.Component
{
    ...
    afterInit()
    {
        ...
        const linkColumn      = $('.think-multiple').data().linkColumn;
        ...
        if((!quotedQuestions || quotedQuestions.length == 0) && linkColumn.length == 0)
        {
            $element.draggable(
            {
    ...
    _renderCol(item, $preCol, $list, $cols)
    {
        ...
        const linkCannotDeleteTip   = $('.think-multiple').data().linkCannotDeleteTip;
