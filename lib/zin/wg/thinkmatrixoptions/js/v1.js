...
class ThinkMatrixOptions extends zui.Component
{
    ...
    afterInit()
    {
        ...
        const multipleData    = $('.think-multiple').data();
        const quotedQuestions = multipleData.quotedQuestions;
        const linkColumn      = multipleData.linkColumn;
        ...
        if((!quotedQuestions || !quotedQuestions.length) && !linkColumn.length)
        {
            $element.draggable(
            {
    ...
    _renderCol(item, $preCol, $list, $cols)
    {
        ...
        const multipleData          = $('.think-multiple').data();
        const tipQuestion           = multipleData.tipQuestion;
        const cannotDeleteColumnTip = multipleData.cannotDeleteColumnTip;
        const linkCannotDeleteTip   = multipleData.linkCannotDeleteTip;
        ...
        if((quotedQuestions && quotedQuestions.length) || linkColumn.length)
        {
            ...
            const quotedCannotDeleteTip = cannotDeleteColumnTip.replace(/%s/g, quotedIndex);
            $col.find('.think-multiple-item-move').attr('disabled', true);
            $col.find('.think-multiple-col-delete .btn').attr('disabled', true);
            $col.find('.think-multiple-col-delete .btn').attr('title', quotedIndex ? quotedCannotDeleteTip : linkCannotDeleteTip);
            $col.find('.think-multiple-item-move').attr('title', quotedIndex ? quotedCannotDeleteTip : linkCannotDeleteTip);
        }
        else
        {
