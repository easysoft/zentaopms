...
class ThinkMatrixOptions extends zui.Component
{
    ...
    render()
    {
        ...
        this.$element.find('.think-multiple-buttons .action-add').prop('disabled', items?.length && items.length > 15);
        this.$element.find('.think-multiple-buttons .action-add').attr('title', items?.length && items.length > 15 ? this.options.addTip : '');
    }
