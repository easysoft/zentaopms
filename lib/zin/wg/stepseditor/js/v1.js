...
class StepsEditor extends zui.Component
{
    ...
    static DEFAULT =
    {
        name: 'steps',
        expectsName: 'expects',
        data: ['1', '2', '3', '1.1', '1.1.1', '1.1.2', '2.1'],
        sameLevelIcon: 'plus',
        subLevelIcon: 'split',
        moveIcon: 'move',
        deleteIcon: 'trash',
    ...
    {
        this.render();
        this.$element.on('focus', '.steps-editor-step-text', e => $(e.target).closest('.steps-editor-col-step').addClass('focus').parent().addClass('is-step-focus')).on('blur', '.steps-editor-step-text', e => $(e.target).closest('.steps-editor-col-step').removeClass('focus').parent().removeClass('is-step-focus')).on('click', '.btn-action', e =>
        {
    ...
    deleteStep(name)
    {
        ...
        if(!Object.keys(this._map).length) this.update(['1'], true, true);
        this.render();
    }
    ...
    addSib(fromName)
    {
        ...
        if(index < siblings.length - 1)
        {
            siblings.slice(index + 1).forEach((nextItem, idx) =>
            {
                ...
                nextItem.name = nextItem.parent ? `${nextItem.parent.name}.${newIndex}` : `${newIndex}`;
                updateItems.push(nextItem)
            });
        }
