...
window.executeWithFormContext = async function(promptID)
{
    ...
    try
    {
        ...
        const isBatch    = $form.is('[z-use*="BatchForm"]');
        ...
        if($targetEl)
        {
            ...
            const skillsAttr = $targetEl.attr('data-agent-skills');
            if(skillsAttr)
            {
                try
                {
                    agentSkills = JSON.parse(skillsAttr);
                }
                catch(e) {}
            }
        }
        ...
        if(window.parent && window.parent.executeUniversalPromptWithZentaoAPI)
        {
            ...
            window.parent.executeUniversalPromptWithZentaoAPI(formSchema, contextIDs, promptID, promptFields, allowedFields, agentRole, agentPurpose, isBatch, agentSkills);
        }
    }
    catch(error)
    {
