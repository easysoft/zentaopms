...
window.initBcgModel = function()
{
    $.getLib(config.webRoot + 'js/echarts/echarts.common.min.js', {root: false}, function()
    {
        ...
        const markLineX = calculateMarkLine(prerelease.splitType[1], prerelease.splitValue[1], data.basicXAxis);
        const markLineY = calculateMarkLine(prerelease.splitType[0], prerelease.splitValue[0], data.basicYAxis);
        ...
        const minXAxis  = markLineX <= minX ? markLineX : Math.max(minX * 0.3, 40);
        const maxXAxis  = markLineX >= maxX ? markLineX : Math.max(maxX * 0.3, 40);
        const minYAxis  = markLineY <= minY ? markLineY : Math.max(minY * 0.3, 40);
        const maxYAxis  = markLineY >= maxY ? markLineY : Math.max(maxY * 0.3, 40);
                ...
                data: [
                    {
                        ...
                        emphasis: {
                            label: {
                                ...markLineLabel,
                                position: configureDimension.yAxisOrder == 1 ? 'end' : 'start'
                            }
                        }
                    },
                    {
