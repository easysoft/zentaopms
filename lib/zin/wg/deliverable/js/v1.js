...
window.getDeliverableFileActions = function(file, deliverable)
{
    ...
    if(!onlyShow)
    {
        ...
        actions[3] = {icon: 'trash', key: 'remove'};
    }
...
window.getDocActions = function(doc, deliverable)
{
    ...
    if(isTemplate)
    {
        ...
        actions[2] = {icon: 'trash', key: 'remove'};
    }
    else
    {
        ...
        if(deliverable?.status == undefined || (deliverable?.status == 'draft' && deliverable?.review == 0)) actions[2] = {icon: 'trash', key: 'remove'};
    }
    return actions;
}
...
window.getDeliverableActions = function(deliverable, category)
{
    ...
    {
        if(canCreateDoc) actions[0] = {text: createDoc, icon: 'doc', key: 'createDoc', 'onClick' : (event) => showBtnModal(deliverable, category, $(event.target).closest('.deliverable-item').z('gid'))}; // 前台交付物有三个创建文档的操作。
    }
