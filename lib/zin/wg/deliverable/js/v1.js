...
window.getDeliverableActions = function(deliverable, category)
{
    ...
    {
        const hasBtn = createDocUrl || uploadDocUrl || (typeof category.template != 'undefined' && category.template.length > 0);
        if(hasBtn) actions[0] = {text: createDoc, icon: 'doc', key: 'createDoc', 'onClick' : () => showBtnModal(deliverable, category)}; // 前台交付物有三个创建文档的操作。
    }
...
window.showBtnModal = function(deliverable, category)
{
    ...
    actions.push(`<div class="flex flex-col gap-4 justify-center items-center h-20">`);
    if(createDocUrl) actions.push(`<a class="btn btn-sm btn-primary" target='_blank' href="${createDocUrl}"><i class="icon icon-doc"></i>${createDoc}</a>`);
    if(uploadDocUrl) actions.push(`<a class="btn btn-sm btn-primary" data-toggle="modal" href="${uploadDocUrl}"><i class="icon icon-file"></i>${uploadFile}</a>`);

    if(typeof category.template != 'undefined' && category.template.length > 0)
    {
        actions.push(`<button class="btn btn-sm btn-primary" data-toggle="dropdown"><i class="icon icon-plus"></i>${createByTemplate} <span class="caret"></span></button>`);
