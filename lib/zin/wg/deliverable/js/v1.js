...
window.showBtnModal = function(deliverable, category)
{
    ...
    if(createDocUrl) actions.push(`<a class="btn btn-sm btn-primary w-28" target='_blank' href="${createDocUrl}"><i class="icon icon-doc"></i>${createDoc}</a>`);
    if(uploadDocUrl) actions.push(`<a class="btn btn-sm btn-primary w-28" data-toggle="modal" href="${uploadDocUrl}"><i class="icon icon-file"></i>${uploadFile}</a>`);
