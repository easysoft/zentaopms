...
window.addStep = function(id, addType)
{
    ...
    const {wizard, order, grade, path} = $element.data();
    ...
    inputTpl     += `<input id="stepName" class="form-control" data-id="${id}" data-parent="${parentID}" data-add-type="${addType}" data-wizard="${wizard}" data-order="${order}" data-grade="${grade}" data-path="${path}">`;
    inputTpl     += '</div></li>';
...
window.saveStep = function()
{
    const name = $(this).val();
    if(!name) return $(this).closest('.tree-item').remove();
    const {id, addType, parent, wizard, order, grade, path} = $(this).data();
