...
{
    const modeClass = $('.think-form-batch').data('modeClass');
    if(modeClass != undefined)
    {
        $(modeClass + ' .think-form-batch td[data-name="ACTIONS"] .btn[data-type="add"]').prop({
        ...
        $(modeClass + ' .think-form-batch td[data-name="ACTIONS"] .btn[data-type="delete"]').prop({
            disabled: disabled,
        })
    }
}

$(document).on('click', 'td[data-name="ACTIONS"] .btn', function()
{
    renderRowData();
...
/**
 * 做题时有错误提示的情况下有输入事件先清空错误提示。
 * Clear the error prompt when there is an input event in case of an error prompt during problem solving..
 *
 * @access public
 * @return void
 */
$(document).on('input', '.m-thinkrun-answer .form-batch-input', function()
{
    $('.form .form-batch-input.has-error').each(function()
    {
        $(this).removeClass('has-error');
        $(this).siblings('.form-tip.ajax-form-tip').remove();
        $(this).parent().siblings('.form-tip.ajax-form-tip').remove();
    })
});
