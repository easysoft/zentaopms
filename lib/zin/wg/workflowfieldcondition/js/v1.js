...
window.processParamBox = function($row, index, data)
{
    ...
    switch(paramType)
    {
        ...
        case 'custom':
            ...
            {
                processFieldControl($row, index, name, data, response);
            });
            break;
        default:
            $.getJSON($.createLink('workflowfield', 'ajaxGetParamOptions', 'paramType=' + paramType), function(response)
            {
                processFieldControl($row, index, name, data, {options: response});
            });
            break;
    }
...
window.processFieldControl = function($row, index, name, data, response)
{
