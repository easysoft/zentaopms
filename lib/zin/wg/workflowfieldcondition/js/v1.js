...
window.processParamBox = function($row, index)
{
    const name      = $($row).closest('.form-batch').attr('name');
    const table     = $('input[name=table]').val();
    ...
    switch(paramType)
    {
        ...
        case 'currentDept':
        case 'deptManager':
            $row.find("[data-name='" + name + "[param]']").append("<input class='form-control form-batch-input' disabled='disabled' type='text' autocomplete='off' value='" + datasources[paramType] + "'/><i    nput class='hidden' type='text' name='" + name + "[param][" + index + "]' id='" + name + "[param]' data-name='" + name + "[param]' value='" + paramType + "'/>");
            break;
        case 'form':
        case 'record':
            ...
            break;
        case 'formula':
            $row.find("[data-name='" + name + "[param]']").append("<input class='hidden' type='text' name='" + name + "[param][" + index + "]' id='" + name + "[param]' data-name='" + name + "[param]'/><a d    ata-on='click' data-call='setFormula' data-params='event' data-name='" + name + "[param][" + index + "]' class='btn ghost'>" + setFormula + "</a>");
            break;
        case 'custom':
            $.getJSON($.createLink('workflowfield', 'ajaxGetFieldOptions', 'module=' + moduleName + '&field=' + field), function(response)
            {
                processFieldControl($row, index, name, response);
            });
            break;
        default:
            $.getJSON($.createLink('workflowfield', 'ajaxGetParamOptions', 'paramType=' + paramType), function(response)
            {
                processFieldControl($row, index, name, {options: response});
            });
            break;
    }
...
window.processFieldControl = function($row, index, name, response)
{
    if(response.control == 'input')
    {
        $row.find("[data-name='" + name + "[param]']").append("<input class='form-control form-batch-input' type='text' autocomplete='off' name='" + name + "[param][" + index + "]' id='" + name + "[param]'     data-name='" + name + "[param]'/>");
    }
    else if(response.control == 'datePicker')
    {
        ...
        $(`#${name}[param]_${index}`).val(data.values);
    }
