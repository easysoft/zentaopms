window.addNewLine = function(e)
{
    ...
    newLine.find('.linkBranch, .addRoadmap').addClass('hidden');
    newLine.find("[name^='addRoadmap']").prop('checked', false);
...
window.loadProductBranches = function(obj)
{
    ...
    if(!productID)
    {
        ...
        $product.closest('.productBox').find('.addRoadmap').addClass('hidden');
...
window.loadStoryGrade = function(obj)
{
    const $product     = $(obj);
    const productID    = $product.val();
    const $gradePicker = $product.closest('.productBox').find('[name^=storyGrade]').zui('picker');

    $.getJSON($.createLink('demand', 'ajaxGetStoryGrade', "productID=" + productID), function(data)
    {
