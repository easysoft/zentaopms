setTimeout(function()
{
    createMonaco(id, action, options, diffContent, vsPath, clientLang);
    resize(id);
}, 200);

function createMonaco(id, action, options, diffContent, vsPath, clientLang)
{
    require.config({
        paths: {vs: vsPath},
        'vs/nls': {
    ...
    require(['vs/editor/editor.main'], function ()
    {
        if(action == 'diff')
        {
            ...
            if(window.onMouseDown) getOriginalEditor.onMouseDown(window.onMouseDown);
            if(window.onMouseMove) getOriginalEditor.onMouseMove(window.onMouseMove);
        }
        else
        {
            editor = monaco.editor.create(document.getElementById(id), options);

            if(window.onMouseDown) editor.onMouseDown(window.onMouseDown);
            if(window.onMouseMove) editor.onMouseMove(window.onMouseMove);
        }
        if(selectedLines)
        {
            ...
            editor.setSelection(new monaco.Range(startLine, 1, endLine, 1))
        }
    });
}
