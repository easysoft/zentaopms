        ...
        props: {
            ...
            actions: canDiffVersion ? [
                {text: versionLangData.settings, className: this.state.showCheckbox ? 'invisible pointer-events-none' : 'text-primary', onClick: () => {
                    ...
                    showVersions = [];
                    hiddenVersions = [];
                    this.setState({showCheckbox: true, checked: {}});
...
window.setVersionDropdownFooter = function()
{
    ...
    return {
        ...
        props: {
            ...
            className: 'p-1 pt-0',
            items: [
                {text: versionLangData.confirm, size: 'sm', disabled: diffMode ? this.getChecks().length < 2 : this.getChecks().length < 1, type: 'primary', onClick: () =>
                {
                    if(settingsMode)
                    {
                        settingsMode = false;
...
window.getVersionItem = function(item)
{
    ...
    if(settingsMode) return $.extend({checked: !!this.state.checked[item.key]}, item);
...
window.setClickVersionItem = function(info)
{
    if(this.state.showCheckbox)
    {
        if(settingsMode)
        {
            if(showVersions.includes(info.item.value))
            {
            ...
            {
                showVersions.push(info.item.value);
...
{
    if(!diffMode && !settingsMode) return;
