...
window.getVersionItems = function(menu, all = false)
{
    return all ? menu?.props?.allVersionItems || [] : menu?.props?.visibleVersionItems || [];
};
        ...
        props: {
            ...
            actions: canDiffVersion ? [
                {text: versionLangData.settings, className: this.state.showCheckbox ? 'invisible pointer-events-none' : 'text-primary', onClick: () => {
                    ...
                    this.setState({showCheckbox: true, items: getVersionItems(this, true).filter((item) => item.value !== 'nowait' && item.value !== 0)});
            ...
            items: [
                {text: versionLangData.confirm, size: 'sm', disabled: diffMode ? this.getChecks().length < 2 : false, type: 'primary', onClick: () =>
                {
                    if(settingsMode)
                    {
                        ...
                        this.setState({showCheckbox: false, items: getVersionItems(this)});
...
window.exchangeVersion = function(e)
{
    ...
    if($menu.state.showCheckbox) $menu.setState({showCheckbox: false, items: getVersionItems($menu)})
...
window.showMenu = function()
{
    ...
    {
        /* 设置时，items重置为所有版本。 */
        this.menu.setState({showCheckbox: true, items: getVersionItems(this.menu, true).filter((item) => item.value !== 'nowait' && item.value !== 0)});
    ...
    if(diffMode)
    {
        ...
        this.menu.setState({showCheckbox: true, items: getVersionItems(this.menu)});
        $(this.menu.base).find('li.menu-item .item-actions').addClass('hidden');
        return;
    }

    this.menu.setState({items: getVersionItems(this.menu)});
};
