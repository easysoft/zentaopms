...
/* 获取显示在版本菜单中的版本。*/
window.getVisibleVersionItems = function()
{
    const items = [];
    ...
    allVersionItems.forEach((item) => {
        ...
        {
            if(!isVersionVisible(item)) return;

            /* 先添加版本的分组标题，再将版本添加到组后面。*/
            if(pendingHeading)
            {
    ...
    return {
        ...
        props: {
            ...
            actions: canDiffVersion ? [
                {text: versionLangData.settings, className: this.state.showCheckbox ? 'invisible pointer-events-none' : 'text-primary', onClick: () => {
                    diffMode = false;
                    settingsMode = true;
                    showVersions = [];
                    hiddenVersions = [];

                    /* 点击设置按钮时，items要重置为所有版本。*/
                    this.setState({showCheckbox: true, checked: {}, items: allVersionItems.filter((item) => item.value !== 'nowait' && item.value !== 0)});
                ...
                {icon: 'exchange', text: versionLangData.compare, className: this.state.showCheckbox ? 'invisible pointer-events-none' : 'text-primary', onClick: () => {
                    ...
                    /* 点击对比按钮时，items要重置为版本菜单中可见的版本。*/
                    this.setState({showCheckbox: true, checked: {}, items: getVisibleVersionItems()});
                    $(this.base).find('li.menu-item .item-actions').addClass('hidden');
            ...
            items: [
                {text: versionLangData.confirm, size: 'sm', disabled: diffMode ? this.getChecks().length < 2 : false, type: 'primary', onClick: () =>
                {
                    if(settingsMode)
                    {
                        ...
                        /* 在设置中点击确认时，items重置为版本菜单中可见的版本。 */
                        this.setState({showCheckbox: false, checked: {}, items: getVisibleVersionItems()});
                        $.ajaxSubmit({url: $.createLink('programplan', 'ajaxSetShowVersion'), data: 'showVersions=' + showVersions + '&hiddenVersions=' + hiddenVersions});
                        return;
                    }
                ...
                {text: versionLangData.cancel, size: 'sm', className: 'not-hide-menu', type: 'default', onClick: (e) =>
                {
                    ...
                    diffMode = false;
                    this.setState({showCheckbox: false, checked: {}})
                    $(this.base).find('li.menu-item .item-actions').removeClass('hidden');
...
window.exchangeVersion = function(e)
{
    ...
    if($menu.state.showCheckbox) $menu.setState({showCheckbox: false, items: getVisibleVersionItems()})
...
window.showMenu = function()
{
    ...
    {
        /* 设置时，items重置为所有版本。 */
        this.menu.setState({showCheckbox: true, items: allVersionItems.filter((item) => item.value !== 'nowait' && item.value !== 0)});
    ...
    if(diffMode)
    {
        /* 对比时，items重置为版本菜单中可见的版本。 */
        this.menu.setState({showCheckbox: true, items: getVisibleVersionItems()});
        $(this.menu.base).find('li.menu-item .item-actions').addClass('hidden');
        return;
    }

    this.menu.setState({items: getVisibleVersionItems()});
};
