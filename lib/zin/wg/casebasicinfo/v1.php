    ...
    protected function getStatus($case): array
    {
        ...
        if(isset($case->fromCaseVersion) && $case->fromCaseVersion > $case->version && $from != 'testtask' && !empty($case->product))
        {
            ...
            (
                ...
                ' (',
                $lang->testcase->changed,
                hasPriv('testcase', 'confirmLibcaseChange') ? a(setClass('btn size-xs primary-pale mx-1 ajax-submit'), set::href(createLink('testcase', 'confirmLibcaseChange', "caseID={$case->id}&libcaseID={$case->fromCaseID}")), $lang->testcase->sync, setData(array('confirm' => sprintf($lang->testcase->confirmLibcaseChangeTip, $case->fromCaseVersion, $case->fromCaseVersion)))) : '',
