...
window.updateCriticalPath = function()
{
    ...
    $('#criticalPath').attr('title', showCriticalPath ? ganttLang.hideCriticalPath : ganttLang.showCriticalPath);
...
{
    if($('.gantt .gantt_marker.today').length == 0) return zui.Modal.alert(ganttLang.warningNoToday);
...
function taskManualSchedule(task, taskID, estStarted, deadline)
{
    ...
    if(response.result == 'success')
    {
        ...
        if(typeof response.message !== 'undefined')
        {
            ...
            zui.Modal.confirm(response.message).then(result =>
            {
                if(result)
                {
                    ...
                    $.ajaxSubmit({url: $.createLink('project', 'ajaxSaveTaskSchedule', 'type=manual&isFromConfirm=1'), data: form});
                }
            });
        }
        ...
        else
        {
            const form = new FormData();
            form.append('data', JSON.stringify(response.data));
            $.ajaxSubmit({url: $.createLink('project', 'ajaxSaveTaskSchedule', 'type=manual'), data: form});
        }
    }
    else
    {
...
function setScalesByZoom(zooming)
{
    ...
    const weekLang    = ganttLang.zooming['week'];
    const quarterLang = ganttLang.zooming['quarter'];
...
window.toggleAllTasks = function()
{
    ...
    gantt.eachTask(function(task)
    {
        ...
        if(gantt.hasChild(task.id)) parent[root][task.id] = isCollapsed;
    });
    gantt.render();
...
function initGantt()
{
    window.jsonedGanttData = JSON.stringify(options);
    ...
    if(showBaselines) gantt.config.row_height = 50;
    ...
    if(showFields.includes('owner_id')) gantt.config.columns.push({name: 'owner_id', align: 'left', resize: true, width: 80, template: function(task){return getByIdForGantt(gantt.serverList('userList'), task.owner_id)}})
    if(showFields.includes('id')) gantt.config.columns.push({name: 'id', align: 'left', resize: true, width: 80, template: function(task){return !isNaN(task.id) && task.id > 0 ? task.id : ''}})
    ...
    if(showFields.indexOf('progress') != -1)
    {
    ...
    {
        if(['text', 'id', 'owner_id', 'status', 'begin', 'deadline', 'duration', 'progress', 'delay', 'delayDays', 'taskProgress'].includes(field)) return;
    ...
    if(gridWidth)
    {
        ...
        gridWidth = gridWidth[root] ? gridWidth[root] : 0;
    }
    gantt.config.layout =
    {
        css: "gantt_container",
        cols:[{
                width: gridWidth ? gridWidth : colsWidth,
                rows:[
    ...
    if(!gantt.bindedEvents)
    {
        ...
        gantt.attachEvent('onTaskClick', function(id, e)
        {
            ...
            if(editBtn.hasClass('editDeadline'))
            {
                ...
                options.weekStart    = 1;
                options.minDate      = new Date(deadlineDate);
                ...
                $('#changeDeadlineModal [name="projectID"]').val(projectID);
            ...
            if(editBtn.hasClass('icon-confirm') || editBtn.hasClass('submitBtn') || editBtn.find('.icon-confirm').length || editBtn.closest('.submitBtn').length || $(e.srcElement || e.target).closest('.submitBtn').length)
            {
                ...
                zui.Modal.open({
                    url: $.createLink('review', 'create', 'projectID=' + projectID + '&deliverable=0&reviewID=' + reviewID + '&type=decision&objectID=' + category),
                    modal: true
                });
...
window.getSplitState = function(root)
{
    ...
    if(root && !ganttSplitState[root])
    {
        ganttSplitState = {};
        ganttSplitState[root] = {};
    }
...
window.initGanttSplitState = function()
{
    ...
    if(!ganttSplitState[root]) return;
    $.each(ganttSplitState[root], function(id, state)
    {
        if(!state) return;
        gantt.open(id);
    });
