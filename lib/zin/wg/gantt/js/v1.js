...
window.updateCriticalPath = function()
{
    ...
    $('#criticalPath').attr('title', showCriticalPath ? window.loadedGanttLang.hideCriticalPath : window.loadedGanttLang.showCriticalPath);
...
function validateResources(id)
{
    ...
    let type   = task.type;
    let statusLang = window.loadedGanttLang.taskStatusList['wait'];
    if(typeof type == 'undefined') return false;

    /* Check status. */
    if(status !== statusLang && type != 'point')
    {
        let tipMsg = window.loadedGanttLang.errorPlanDrag;
        if(type == 'task') tipMsg = window.loadedGanttLang.errorTaskDrag;
...
window.toggleAllTasks = function()
{
    ...
    gantt.eachTask(function(task)
    {
        ...
        if(gantt.hasChild(task.id)) parent[root][ganttType][task.id] = isCollapsed;
    });
    gantt.render();
...
window.getSplitState = function(root)
{
    ...
    {
        const objectKeys = Object.keys(ganttSplitState);
        if(objectKeys.length > 20) delete ganttSplitState[objectKeys[0]]; // 保留最后20个。 Keep the last 20.

        ganttSplitState[root] = {};
    }
    if(!ganttSplitState[root][ganttType]) ganttSplitState[root][ganttType] = {};
...
window.saveSplitState = function(id, state)
{
    ...
    ganttSplitState[root][ganttType][id] = state;
...
window.initGanttSplitState = function()
{
    ...
    if(!ganttSplitState[root] || !ganttSplitState[root][ganttType]) return;
    $.each(ganttSplitState[root][ganttType], function(id, state)
    {
        if(!state) return;
        gantt.open(id);
    });
