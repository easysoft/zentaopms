...
function setScalesByZoom(zooming)
{
    ...
    if(zooming == 'day')     gantt.config.scales = [{unit: "year", step: 1, format: "%Y"}, {unit: 'month',   step: 1, format: '%M'}, {unit: 'day', step: 1, format: '%d'}];
    ...
    if(zooming == "month")   gantt.config.scales = [{unit: "year", step: 1, format: "%Y"}, {unit: 'month', step: 1, format: '%M'}];
...
function initGantt()
{
    ...
    if(showFields.indexOf('deadline') != -1) gantt.config.columns.push({name: 'deadline', align: 'center', resize: true, width: 80, template: getDeadlineBtn});
    gantt.config.columns.push({name: 'duration', align: 'center', resize: true, width: 120});
    ...
    gantt.templates.timeline_cell_class = function(item, date)
    {
        ...
        if(weekend.weekend == 2 && (day == 0 || day == 6)) return 'weekend';
        if(weekend.weekend == 1 && weekend.restDay == day) return 'weekend';

        if(holidays.includes(gridDateToStr(date))) return 'weekend';
    };
