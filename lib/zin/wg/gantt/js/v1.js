...
function initGantt()
{
    ...
    if(showFields.length != 0)                gantt.config.columns.push({name:"custom", label:"", align: "left", width: 40, template: getSubmitBtn});
    if(showFields.includes('owner_id') != -1) gantt.config.columns.push({name: 'owner_id', align: 'left', resize: true, width: 80, template: function(task){return getByIdForGantt(gantt.serverList('userList'), task.owner_id)}})
    if(showFields.includes('id') != -1)       gantt.config.columns.push({name: 'id', align: 'left', resize: true, width: 80, template: function(task){return !isNaN(task.id) && task.id > 0 ? task.id : ''}})
    if(showFields.indexOf('status') != -1)    gantt.config.columns.push({name: 'status', align: 'center', resize: true, width: 80});
    if(showFields.indexOf('begin') != -1)     gantt.config.columns.push({name: 'begin', align: 'center', resize: true, width: 80});
    ...
    if(showFields.indexOf('duration') != -1)  gantt.config.columns.push({name: 'duration', align: 'center', resize: true, width: 120});
    ...
    if(!gantt.bindedEvents)
    {
        ...
        gantt.attachEvent('onTaskClick', function(id, e)
        {
            ...
            if(editBtn.hasClass('icon-confirm') || editBtn.hasClass('submitBtn') || editBtn.find('.icon-confirm').length || editBtn.closest('.submitBtn').length || $(e.srcElement || e.target).closest('.submitBtn').length)
            {
                ...
                zui.Modal.open({
                    url: $.createLink('review', 'create', 'projectID=' + window.globalProject + '&deliverable=0&reviewID=' + reviewID + '&type=decision&objectID=' + category),
                    modal: true
                });
