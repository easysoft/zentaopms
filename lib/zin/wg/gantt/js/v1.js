...
function initGantt()
{
    ...
    gantt.attachEvent('onTaskClick', function(id, e)
    {
        ...
        let task = gantt.getTask(id);
        if(task.type == 'point' && task.rawStatus) loadPage($.createLink('review', 'view', 'reviewID=' + task.reviewID));
        if(task.type == 'plan' && !task.isParent)  $.apps.open($.createLink('execution', 'task', 'id=' + task.id), 'execution');
