 ...
 * @return void
 */
window.drawRegionInput = function(canvas, region, colorIndex, clientLang)
{
    region.minX += 30;
    region.maxY -= 10;
    ...
    {
        if([0, 4, 5].includes(colorIndex)) top -= 30;
        if(colorIndex == 4) left += 50;
        if(colorIndex == 5) left -= 60;
    }
    ...
    input.name        = `blockTitle[${colorIndex}]`;
...
function renderRegionInput(regions)
{
    ...
    regions.forEach((region, index) => {
        if(!$(`#input_${index}`).length) drawInputFunc(canvas, region, index, clientLang);
    });
}
