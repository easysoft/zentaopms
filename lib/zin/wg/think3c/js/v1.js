...
window.initThinkCanvas = function()
{
    ...
    if(mode == 'view')
    {
        ...
        $mainContent.addClass('load-indicator').addClass('loading').css('height', Math.max(200, Math.floor($(window).height() - $('#header').outerHeight() - $('#mainMenu').outerHeight()))).attr('data-loading', loadGraphTip);
    ...
    image.onload = function()
    {
        ...
        colorRegions(binarizeImageUrl).then((data) => {
            ...
            {
                for(var i = 0; i < coloredRegions.length - 1; i++)
                {
                    if(coloredRegions[i + 1].minY - coloredRegions[i].minY < 8)
                    {
                        if(coloredRegions[i + 1].minX < coloredRegions[i].minX)
                        {
                            const item = coloredRegions[i];
                            coloredRegions[i]     = coloredRegions[i + 1];
                            coloredRegions[i + 1] = item;
                        }
                    }
                }
                afterFinish();
...
function afterFinish()
{
    setTimeout(function()
    {
        $('#mainContent').attr('data-loading', '').css('height', '').removeClass('loading').removeClass('load-indicator');
...
function calculateCardPosition(region, index)
{
    const h         = 180;
    const minW      = 140;
    const positions = calculateCardPositionsOfScale(region);
    ...
    {
        const scale        = 1 / Math.ceil(cards.length / positions.length);
        const newPositions = calculateCardPositionsOfScale(region, scale);
        cards.each((i, card) => {
            $(card).css({
                left: `${newPositions[i][0]}px`,
                top: `${newPositions[i][1]}px`,
                width: `${minW}px`,
                height: `${h}px`,
...
/**
 * 计算缩放的卡片位置。
 * Calculate the position of the scaled card.
 *
 * @param  object region
 * @param  number scale
 * @access public
 * @return void
 */
function calculateCardPositionsOfScale(region, scale = 1)
{
    let h           = 180; // 原始卡片高度
    let minW        = 140; // 原始卡片宽度
    let maxW        = 280; // 最大宽度
    ...
    const groupedData = groupPositions(positions, h);
    return calcPositionsWithOffset(groupedData);
}
