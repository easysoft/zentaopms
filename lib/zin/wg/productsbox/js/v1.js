...
window.loadPlans = function(product, branch)
{
    ...
    const filterPlans = $('.productsBox').data('filterPlans');
    let planParams = 'multiple';
    if(filterPlans) planParams += ',undone';

    $.get($.createLink('product', 'ajaxGetPlans', "productID=" + productID + '&branch=' + branchID + '&params=' + planParams + '&skipParent=true'), function(data)
    {
        if(data)
        {
