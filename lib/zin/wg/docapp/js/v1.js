...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        if(docAIPrompts && docAIPrompts.length)
        {
            ...
            const aiActionItems = docAIPrompts.map(prompt => ({icon: 'magic', text: prompt.status !== 'active' ? zui.html`${prompt.name}<span class="label size-sm gray-500-pale ring-gray-500 whitespace-nowrap ml-1">${aiLang.statuses[prompt.status]}</span>` : prompt.name, command: `executeAIPrompt/${prompt.id}/${doc.id}`, hint: prompt.desc, trailing: 'haha'}));
