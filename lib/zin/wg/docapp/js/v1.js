...
const commands =
{
    ...
    createLib: function(_, args)
    {
        ...
        let url;
        if(isApiLib(docApp.lib, docApp))
        {
            url = $.createLink('api', 'createLib', spaceID ? `type=${docApp.spaceType}&objectID=${spaceID}&createMode=${createMode}` : `type=${docApp.spaceType}&createMode=${createMode}`);
        }
        else
        {
            url = $.createLink('doc', 'createLib', spaceID ? `type=${docApp.spaceType}&objectID=${spaceID}` : `type=${docApp.spaceType}`);
        }
        zui.Modal.open({size: 'sm', url: url});
    },
