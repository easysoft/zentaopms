...
{
    if(!menus) return;
    if(!menus.length) return zui.Editor.setSlashMenu([], {reset: true});
...
function getZentaoListProps(type, blockID, props)
{
    ...
    const fetcher        = `${$.createLink('doc', 'zentaoList', `type=${type}&blockID=${blockID}`)}`;
    return $.extend({}, props,
    {
        title: title,
        fetcher: fetcher,
...
function showDocBasicModal(parentID, docID, isDraft, modalType = 'doc', docTypeOrDoc = 'text')
{
    const docApp = getDocApp();
    if(!docApp) return;
    const spaceType = docApp.signals?.spaceType?.value ?? docApp.spaceType ?? 'custom';
    const spaceID   = docApp.signals?.spaceID?.value ?? docApp.spaceID ?? 0;
    const libID     = docApp.signals?.libID?.value ?? docApp.libID ?? 0;
    const moduleID  = docApp.signals?.moduleID?.value ?? docApp.moduleID ?? 0;
    const docInfo   = typeof docTypeOrDoc === 'object' && docTypeOrDoc ? docTypeOrDoc : null;
    const docType   = typeof docTypeOrDoc === 'string' ? docTypeOrDoc : 'text';
    const url       = $.createLink('doc', 'setDocBasic', `objectType=${spaceType}&objectID=${spaceID}&libID=${libID}&moduleID=${moduleID}&parentID=${parentID || 0}&docID=${docID || 0}&isDraft=${isDraft ? 'yes' : 'no'}&modalType=${modalType}` + (docType ? `&docType=${docType}` : ''));

    /* 新建文档发布时把编辑器中已填写的标题传入基本信息弹窗，避免重复填写。 */
    const postData = {};
    if(!docID && docInfo && docInfo.title) postData.title = docInfo.title;
...
window.beforeSetDocBasicInfo = function(event, form)
{
    ...
    const docType = formData.get('contentType');

    if(docType === 'url' && docID)
    {
        event.preventDefault();

        try
        {
            const url = $.createLink('doc', 'edit', `docID=${docID}`);
            $.post(url, formData, function(res)
            {
                ...
                {
                    const docApp = getDocApp();
                    if(docApp && docApp.update && data.doc)
                    {
                        docApp.update('doc', data.doc);
                    }
...
{
    const app = getDocApp();
    return !String(doc.id).startsWith('api.') && app && app.props.privs.moveDoc;
}

function canCopyDoc(doc)
{
    const app = getDocApp();
    return !String(doc.id).startsWith('api.') && app && app.props.privs.copyDoc;
}
...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        if(info.ui === 'sidebar')
        {
            ...
            const moreItems =
            [
                hasPriv('addChapter') && !isApi ? {icon: 'plus', text: lang.addSubChapter, command: `addChapter/${doc.id}`} : null,
                hasPriv('addChapter') && !isApi && doc.parent != 0 ? {icon: 'plus', text: lang.addSameChapter, command: `addChapter/${doc.parent}`} : null,
                !isTemplate && hasPriv('create') ? {icon: 'plus', text: lang.addSubDoc, command: `startCreateDoc/text/${doc.id}`} : null,
        ...
        if(canCopyDoc(doc)) moreItems.push({icon: 'copy', text: lang.copyDoc, command: `copyDoc/${doc.id}`});
        if(!isApi && !isTemplate)
        {
        ...
        if (isApi) {
            ...
            const canExportOpenAPI = hasPriv('exportOpenApi');
            const exportApiItems = [
                canExportApi ? {icon: 'export', text: lang.exportWord, command: 'exportApi'} : null,
                canExportOpenAPI ? {icon: 'export', text: lang.exportOpenAPI, command: 'exportOpenAPI'} : null,
            ].filter(Boolean);
            moreItems.push(...exportApiItems);
        }
        const docApp = this;
        ...
        let aiAction         = null;
        let aiTeammateAction = null;
        const docAIPrompts = window.docAIPrompts;
        const isDocContent = ['text', 'article', 'template'].includes(doc.type)
            && ['doc', 'html', 'markdown'].includes(doc.contentType);
        if(docAIPrompts && docAIPrompts.length && isDocContent)
        {
            ...
            else aiAction = {icon: 'lightning', text: aiLang.dropdownTitle || 'AI', type: 'dropdown', btnType: 'ai-styled', size: 'sm', caret: 'down', placement: 'bottom-end', items: aiActionItems};

            const aiTeammateLang = window.docAITeammateLang;
            const aiTeammates    = window.docAITeammates;
            if(aiTeammates && aiTeammates.length)
            {
                const aiTeammateItems = aiTeammates.map(teammate => {
                    const name = aiTeammateLang.nameLabel.replace('%s', teammate.name);
                    return {text: zui.jsx`<img src="${teammate.avatar}" class="avatar rounded-full mr-2" style="width: 20px; height: 20px; line-height: 20px;" /> ${name}`, hint: name, command: `assignAITeammate/${teammate.id}/${doc.id}`,};
                });
                aiTeammateAction = {icon: 'hand-right', text: aiTeammateLang.dropdownTitle, type: 'dropdown', btnType: 'ai-styled', size: 'sm', caret: 'down', placement: 'bottom-end', items: aiTeammateItems};
            }
        }

        let editCommand = '';
        if (doc.editable) {
            editCommand = doc.contentType === 'url'
                ? `showDocSettingModal/${doc.id}/url/1`
                : 'startEditDoc';
        }
        ...
        const actions = isInModal ? [] : [
            aiAction,
            aiTeammateAction,
            !isApi && canViewAITask ? {hint: lang.aiTask, icon: 'task-list', command: `viewAITasks/${doc.id}`} : null,
            ...
            canEditDoc ? {icon: 'edit', class: doc.editable ? null : 'disabled', type: doc.editable ? 'ghost text-primary' : 'ghost text-gray', hint: doc.editable ? lang.edit : lang.needEditable, rounded: 'lg', command: editCommand} : null,
    ...
    'doc-list': function(info)
    {
        ...
        if(isApiLib(docApp.lib))
        {
            ...
            const canImportOpenAPI = hasPriv('importOpenApi') && libID && config.edition !== 'open';

            const items =
            [
                canCreateApi ? {icon: 'plus', text: lang.createApi, command} : null,
                canImportOpenAPI ? {icon: 'import', text: lang.importOpenAPI, command: 'importOpenAPI'} : null,
                ((canCreateApi || canImportOpenAPI) && canCreateLib) ? {type: 'divider'} : null,
                canCreateLib ? {icon: 'wiki-lib', text: lang.createLib, command: 'createLib'} : null,
            ].filter(Boolean);
            const canExportApi = hasPriv('exportApi') && libID;
            const canExportOpenAPI = hasPriv('exportOpenApi') && libID && config.edition !== 'open';
            const exportItems = [
                canExportApi ? {text: lang.exportWord, command: 'exportApi'} : null,
                canExportOpenAPI ? {text: lang.exportOpenAPI, command: 'exportOpenAPI'} : null,
            ].filter(Boolean);
            const hideSelectTemplate = item => item && item.command !== 'selectTemplate' && item.key !== 'selectTemplate';
            return [
                exportItems.length ? {type: 'divider', style: {margin: '6px 0'}} : null,
                exportItems.length ? {icon: 'export', text: lang.export, type: 'dropdown', items: exportItems.filter(hideSelectTemplate)} : null,
                items.length ? {icon: 'plus', type: 'dropdown', btnType: 'primary',  size: 'md', text: lang.create, items: items.filter(hideSelectTemplate)} : null,
            ];
        }
        ...
        [
            ...
            canCreateDoc ? {icon: 'text-link', text: lang.createList.url, command: 'startCreateUrl'} : null,
    ...
    file: function(info)
    {
        ...
        const canDownload  = privs.downloadFile;

        const previewAction = {
            icon: 'eye',
            hint: lang.filePreview,
            onClick: () => {
                const index  = doc.files.findIndex(f => f.id === file.id);
                const files  = parseAttachments(doc.files);
                const isOpen = isOpenVersion(config.version);
                ...
                showPreviewModal(
                    ...
                    canDownload,
                    {
                        cantPreview: lang.cantPreview,
                        officeNotSupported: lang.officeNotSupported,
                        officeNotInstalled: lang.officeNotInstalled,
                    },
                );
            },
        };
        ...
        return [
            privs.previewFile ? previewAction : null,
            canDownload ? {target: '_blank', url: zui.formatString(docApp.props.fileUrl, file), hint: lang.fileDownload, icon: 'download'} : null,
...
const commands =
{
    ...
    startCreateDoc: function(_, args)
    {
        ...
        const docType = (args && args[0]) || 'text';
        const parentID = (args && args[1]) || 0;
        const isCreateFromTemplate = args && args.length >= 2 && !args[0] && /^[1-9]\d*$/.test(String(args[1]));
        const isSingleParent = !isCreateFromTemplate && args && args.length === 1 && /^[1-9]\d*$/.test(String(args[0]));
        const finalDocType  = isCreateFromTemplate ? 'text' : (isSingleParent ? 'text' : docType);
        const finalParentID = isCreateFromTemplate ? 0 : (isSingleParent ? parseInt(args[0], 10) : parentID);
        const templateID    = isCreateFromTemplate ? parseInt(args[1], 10) : (args?.[2] ?? 0);

        const {spaceType, spaceID, libID, moduleID} = docApp;
        if(spaceType === 'api' || parentID === 'api') return docApp.startCreateDoc();

        const isDraft = (finalDocType === 'url') ? false : true;
        return showDocBasicModal(finalParentID, 0, isDraft, 'doc', finalDocType).then((formData) => {
            if(finalDocType === 'url')
            {
                const docUrl = formData.get('content');
                const doc = {type: 'url', contentType: 'url', content: docUrl, status: 'normal'};
                return submitNewDoc(doc, spaceID, libID, moduleID, formData, (newDoc) => {
                    docApp.selectDoc(newDoc.id);
                    docApp.startViewDoc(newDoc.id);
                });
            }
            else
            {
    ...
    uploadDoc: function(_, args)
    {
        ...
        let docID    = args[0] || 0;
        const type   = docApp.lib ? docApp.lib.data.type : docApp.spaceType;
        if(!docID && docApp.docID) docID = docApp.docID;
    ...
    /** 创建 URL 文档。Create URL doc. */
    startCreateUrl: function()
    {
        const docApp = getDocApp();
        if(!docApp.libList.length) return zui.Modal.alert(getLang('createLibFirst'));
        ...
        const isDraft = false;
        return showDocBasicModal(parentID, 0, isDraft, 'doc', docType).then((formData) => {
            const docUrl = formData.get('content');
            const doc    = {type: 'url', contentType: 'url', content: docUrl, status: 'normal'};
            return submitNewDoc(doc, spaceID, libID, moduleID, formData, (newDoc) => {
                docApp.selectDoc(newDoc.id);
    ...
    createLib: function(_, args)
    {
        const docApp     = getDocApp();
        const spaceID    = args[0] || docApp.spaceID;
        const createMode = args[1] || 'create';

        let url;
        if(isApiLib(docApp.lib, docApp)) url = $.createLink('api', 'createLib', spaceID ? `type=${docApp.spaceType}&objectID=${spaceID}&createMode=${createMode}` : `type=${docApp.spaceType}&createMode=${createMode}`);
        else url = $.createLink('doc', 'createLib', spaceID ? `type=${docApp.spaceType}&objectID=${spaceID}` : `type=${docApp.spaceType}`);
    ...
    deleteLib: function(_, args)
    {
        ...
        $.ajaxSubmit(
        {
            ...
            onSuccess: function()
            {
                ...
                if(docApp.mode === 'home' && docApp._homeRef) docApp.loadHomeLibs();
                else docApp.forceUpdate();
                $(docApp.element).find('.lazy-content.doc-api-home').trigger('loadContent');
            }
        });
    },
    ...
    addChapter: function(_, args)
    {
        ...
        return showDocBasicModal(args[0], 0, false, 'chapter', 'chapter').then((formData) => {
    ...
    editChapter: function(_, args)
    {
        const docApp = getDocApp();
        const {spaceID, libID, moduleID} = docApp;
        return showDocBasicModal(0, args[0], false, 'chapter', 'chapter').then((formData) => {
    ...
    copyDoc: function(_, args)
    {
        ...
        const spaceType = docApp.spaceType;
        const spaceID   = docApp.spaceID;
        const url       = $.createLink('doc', 'copyDoc', `docID=${docID}&libID=${libID}&spaceType=${spaceType}&space=${spaceID}`);
        zui.Modal.open({size: 'sm', url: url});
    },
    handleMovedDoc(_, args)
    {
        const docID      = +args[0];
        const spaceStr   = String(args[1]);
        const sepIdx     = spaceStr.indexOf('.');
        const spaceType  = sepIdx > -1 ? spaceStr.substring(0, sepIdx) : '';
        const spaceID    = sepIdx > -1 ? +spaceStr.substring(sepIdx + 1) : +spaceStr;
        const docType    = args[3];
        const actionType = args[4];
        const docApp     = getDocApp();
        const openEditor = actionType === 'copy' && docType === 'text';

        if(spaceType === docApp.spaceType && spaceID === docApp.spaceID)
        {
            docApp.load(null, null, null, {noLoading: true, picks: 'doc'}).then(() => {
                docApp.selectDoc(docID);
                if(openEditor) docApp.startEditDoc(docID);
            });
        }
        else loadPage($.createLink('doc', openEditor ? 'edit' : 'view', `docID=${docID}`));
    },
    ...
    exportApi: function(_, args)
    {
        ...
        let docID      = args[3] || docApp.docID;

        if(typeof docID === 'string') docID = getApiID(docID);
    ...
    exportOpenAPI: function(_, args)
    {
        ...
        const url = $.createLink('api', 'exportOpenApi', `libID=${libID}&version=0&release=${release}&moduleID=${moduleID}${docID ? `&apiID=${docID}` : ''}`);
        zui.Modal.open({url: url, size: 'sm'});
    },
    importOpenAPI: function(_, args)
    {
        const docApp   = getDocApp();
        const libID    = args[0] || docApp.libID;
        const moduleID = args[1] || docApp.moduleID || 0;
        const url      = $.createLink('api', 'importopenapi', `libID=${libID || 0}&moduleID=${moduleID}&mode=existingLib`);
        zui.Modal.open({size: 'sm', url: url});
    },
    ...
    showDocSettingModal: function(_, args)
    {
        ...
        const doc        = docID ? docApp.getDoc(docID) : docApp.doc;
        const saveEdited = args[2] || 0;
        showDocBasicModal(0, docID, false, 'doc', docType).then(formData => {
            savingDocData[docID] = formData;
            if(saveEdited == 1 && doc && doc.data)
            {
                const docData = {
    ...
    /*
     * 委派数字员工任务。
     * Assign the ai task to the ai teammate.
     */
    assignAITeammate: function(_, args)
    {
        const teammateID = args[0];
        const docID      = args[1];
        const pageInfo   = 'doc,view';
        loadModal($.createLink('aiteammate', 'assignagent', `teammateID=${teammateID}&objectType=doc&objectID=${docID}&pageInfo=${pageInfo}`), null, {size: 'sm'});
    },
     ...
     * View the ai tasks.
     */
    viewAITasks: function(_, args)
    {
        const docID = args[0];
        loadModal($.createLink('aitask', 'browse', `teammate=0&browseType=&involved=0&orderBy=id_desc&param=0&recTotal=0&recPerPage=20&pageID=1&objectType=doc&objectID=${docID}`));
    }
};
...
const customRenders =
{
    ...
    toolbar: function()
    {
        ...
        if(this.mode === 'list')
        {
            ...
            const items = this.getActions('doc-list', {ui: 'toolbar'});
            if(!items) return null;

            const filtered = items.reduce((acc, item) =>
            {
                if(!item) return acc;
                if(item.command === 'selectTemplate' || item.key === 'selectTemplate') return acc;
                if(Array.isArray(item.items))
                {
                    const subItems = item.items.filter(i => i && i.command !== 'selectTemplate' && i.key !== 'selectTemplate');
                    if(subItems.length) acc.push({...item, items: subItems});
                }
                else
                {
                    acc.push(item);
            ...
            return {component: 'toolbar', props: {items: filtered}};
        }
    },
...
function getTableOptions(options, info)
{
    ...
    if(info.type === 'doc-list')
    {
        ...
        options.cols.forEach(col =>
        {
            ...
            if(!config.clientLang.includes('zh'))
            {
                if(col.name === 'addedDate' || col.name == 'editedDate') col.width = '120';
            }
            if(col.name === 'actions' && col.actionsMap)
            {
                ...
                col.actionItemCreator = function(item, info)
                {
                    ...
                    if(item.key === 'edit' && rowData.type === 'url')
                    {
                        item.command = `showDocSettingModal/${rowData.id}/url/1`;
                    }
    ...
    if(!hasPriv('batchMoveDoc') && !hasPriv('exportDoc'))
    {
        ...
        options.footer    = options.footer.filter(f => f !== 'checkbox' && f !== 'checkedInfo');
    }
    return options;
}
...
function flatBlocks(blocks) {
    ...
    for (const block of blocks) {
        ...
        const subList = children && children.length
            ? flatBlocks(children)
            : [];

        list.push({ ...rest }, ...subList);
    }
...
function initOfficePreview(options)
{
    ...
    {
        canPreviewFile: (file) => {
            if(getDocApp().mode === 'edit') return false;

            return options.privs.previewFile;
        },
        onPreviewFile: (file) => {
            const doc    = getDocApp().doc || {data: {id: 0}};
            const data   = JSON.parse(doc.data.content || '');
            const blocks = flatBlocks([data.blocks]);
            const list   = blocks.filter(item => item.flavour === 'affine:attachment');
            const index  = list.findIndex(f => f.props.sourceId === file.key);
            const files  = parsePreviewFiles(list);
            const lang   = getLang();
            const isOpen = isOpenVersion(config.version);
    ...
    if(options.privs.downloadFile === false) zui.Editor.setGlobalConfig({canDownloadFile: (file) => false});
}
...
function handleImportDocSnapshot(snapshot)
{
    ...
    const fixUrl = (url, method, paramList) =>
    {
        ...
        if(link.requestType.toUpperCase() === requestType || method.toLowerCase() !== link.methodName.toLowerCase()) return link.url || url
...
async function mentionsFetcher(query)
{
    const mentions = await zui.fetchData($.createLink('user', 'ajaxGetItems', `params=noclosed,nodeleted`), [], {method: 'POST', data: {search: query, maxCount: 10}});
    return mentions;
}
...
{
    if(options.docFetcher) options.docFetcher = {url: options.docFetcher, dataFilter: data => data.doc};
    ...
    initOfficePreview(options);

    return $.extend(
    {
        editorProps          : {onImportSnapshot: handleImportDocSnapshot, mentionsFetcher: mentionsFetcher, onSelectMention: handleSelectMention,},
...
window.getFileActions = function(file)
{
    ...
    let canPreview       = canPreviewFile;
    let officeTypes      = 'doc|docx|xls|xlsx|ppt|pptx|pdf'.split('|');
    let isOfficeFile     = officeTypes.includes(file.extension);
    let previewExtension = 'txt|jpg|jpeg|gif|png|bmp|mp4'.split('|');
    if(previewExtension.includes(file.extension)) canPreview = true;
    if(libreOfficeTurnon && isOfficeFile)         canPreview = true;
    if(canPreview)
    {
        let previewAction = {icon: 'eye', title: previewLang, url: $.createLink('file', 'preview', `fileID=${flle.id}&mouse=left`), className: 'text-primary', target: '_blank'};
        if(!isOfficeFile)
        {
    ...
    if(canDownloadFile) fileActions.push({icon: 'download', title: downloadLang, url: downloadLink.replace('{id}', file.id).replace('\\', ''), className: 'text-primary', target: '_blank'});
    return fileActions;
};
...
window.getPageFormHelper = function()
{
    ...
    if(docApp.mode === 'edit' && !isApiDoc(docApp.doc))
    {
        const fields =
        {
            ...
            content: (_, $scope) => $scope.find('[z-use-editor],zen-editor'),
        };
