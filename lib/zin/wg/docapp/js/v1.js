...
window.setZentaoSlashMenu = function(menus, title, vision, position, projectReviewTemplates)
{
    ...
    {
        menu.showWhen = function(context) {
            const doc = getDocApp().doc;
            const docID = doc.data.id;
            const templateType = doc.data.templateType;
            if(reviewObjects.includes(templateType) && projectReviewTemplates?.[templateType]?.[docID]) return menu.reviewObject !== undefined && menu.reviewObject.indexOf(templateType) !== -1;
            return true;
        }
        ...
        if(menu.subMenu)
        {
            menu.subMenu = menu.subMenu.map(subMenu => {
                subMenu.showWhen = function(context) {
                    const doc = getDocApp().doc;
                    const docID = doc.data.id;
                    const templateType = doc.data.templateType;
                    if(reviewObjects.includes(templateType) && projectReviewTemplates?.[templateType]?.[docID]) return subMenu.reviewObject !== undefined && subMenu.reviewObject.indexOf(templateType) !== -1;
                    return true;
                }
...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        const actions = [
            ...
            canEditDoc ? {icon: 'edit', type: doc.editable ? 'ghost text-primary' : 'ghost text-gray', hint: doc.editable ? lang.edit : lang.needEditable, rounded: 'lg', command: doc.editable ? 'startEditDoc' : null} : null,
