...
function handleSwitchView(mode, location, info)
{
    ...
    if(!formatUrl || (rawModule === 'doc' && rawMethod === 'view' && mode === 'view' && !lastAppUrl))
    {
    ...
    const url = (typeof formatUrl === 'function' ? formatUrl.call(this, formatOptions) : zui.formatString(formatUrl, formatOptions).replace('&libID=0&moduleID=0&docID=0&mode=home&orderBy=id_desc&recTotal=0&recPerPage=0&pageID=1&filterType=&search=&noSpace=false', '').replace('&libID=0&moduleID=0&browseType=all&param=&orderBy=&recTotal=0&recPerPage=20&pageID=1&mode=home&docID=0&search=', ''));
