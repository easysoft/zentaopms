...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        if(!isApi)
        {
            ...
            if(doc.contentType === 'doc' && config.debug > 3)
            {
                const exportItems = [
                    canExportWord ? {text: lang.exportWord, command: 'exportWord'} : null,
                    {text: lang.exportPdf, command: 'exportDoc/pdf'},
                    {text: lang.exportImage, command: 'exportDoc/png'},
                    {text: lang.exportMarkdown, command: 'exportDoc/markdown'},
                    {text: lang.exportHtml, command: 'exportDoc/html'},
                    config.debug ? {text: lang.export + ' JSON', command: 'exportDoc/json'} : null,
                ].filter(Boolean);
        ...
        if(!isOpen && docApp.mode === 'view' && doc.status != 'draft' && !doc.api)
        {
    ...
    'doc-edit': function(info)
    {
        ...
        const moreItems = [];
        if(doc.contentType === 'doc' && config.debug > 3 && !isApiDoc(doc))
        {
            moreItems.push(
