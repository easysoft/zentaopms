...
const commands =
{
    ...
    startCreateDoc: function(_, args)
    {
        ...
        const isDraft = (docType === 'url') ? false : true;
...
{
    const isEmptyQuery = !zui.isNotEmptyString(query);
    const lastMentions = zui.store.get('recentUserMentions', []);
    if(isEmptyQuery && lastMentions.length >= 10) return lastMentions;
    ...
    if(isEmptyQuery && lastMentions.length)
    {
        for(const mention of mentions)
        {
            if(lastMentions.find(m => m.value === mention.value)) continue;
            lastMentions.push(mention);
            if(lastMentions.length >= 10) break;
        }
