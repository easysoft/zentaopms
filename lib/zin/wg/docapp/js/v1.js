...
window.setZentaoSlashMenu = function(menus, title, vision)
{
...
function showDocBasicModal(docID, isDraft, modalType = 'doc')
{
    const docApp    = getDocApp();
    ...
    const url       = $.createLink('doc', 'setDocBasic', `objectType=${spaceType}&objectID=${spaceID}&libID=${libID}&moduleID=${moduleID}&docID=${docID || 0}&isDraft=${isDraft ? 'yes' : 'no'}&modalType=${modalType}`);
...
const commands =
{
    ...
    startCreateDoc: function(_, args)
    {
        ...
        return showDocBasicModal(args?.[0] ?? 0, true, args?.[0] ? 'subDoc' : 'doc').then((formData) => {
    ...
    editChapter: function(_, args)
    {
        const docApp = getDocApp();
        const {spaceID, libID, moduleID} = docApp;
        return showDocBasicModal(args[0], false, 'chapter').then((formData) => {
    ...
    deleteDoc: function(_, args)
    {
        ...
        $.ajaxSubmit(
        {
            confirm: getLang(isChapter ? 'confirmDeleteChapter' : (doc.isParent ? 'confirmDeleteWithSub' : 'confirmDelete')),
