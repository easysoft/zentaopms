...
const actionsMap =
{
    ...
    file: function(info)
    {
        ...
        let canPreview    = privs.previewFile && 'txt|jpg|jpeg|gif|png|bmp|mp4'.includes(file.extension);
        if(privs.previewFile && typeof libreOfficeTurnon !== 'undefined' && libreOfficeTurnon && isOfficeFile) canPreview = true;
        if(canPreview)
        {
...
{
    if(options.canPreviewOffice && options.fileInfoUrl)
    {
        zui.Editor.setGlobalConfig(
        {
            ...
            onPreviewFile: (file) => {
                ...
                showPreviewModal(lang.filePreview, files, index, lang.cantPreview);
            }
        });
    }
