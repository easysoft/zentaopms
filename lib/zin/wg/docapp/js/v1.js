    ...
    doc: function(info)
    {
        ...
        if(info.ui === 'sidebar')
        {
            ...
            [
                ...
                isTemplate && hasPriv('create') ? {icon: 'plus', text: lang.addSubDocTemplate, command: `startCreateTemplate/${doc.id}`} : null,
        ...
        return showDocBasicModal(args?.[0] ?? 0, 0, true, 'doc').then((formData) => {
            zui.Editor.loadModule().then(() => {
                ...
                return submitNewDoc(doc, spaceID, libID, moduleID, formData, (newDoc) => {
                    ...
                    if(doc.template)
                    {
                        const url = $.createLink('doc', 'ajaxGetTemplateContent', `id=${doc.template}&docID=${doc.id}`);
                        $.get(url, function(data)
                        {
                            data = JSON.parse(data);
                            docApp.importDoc(data.content, 'json', data.title);
                        });
                    }
