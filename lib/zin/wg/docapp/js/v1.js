...
function setZentaoSlashMenu()
{
    const menus = $('#docApp #zentaoMenus').data('menus');
    const title = $('#docApp #zentaoMenus').data('title');
    const slashMenus = menus.map(menu =>
    {
        if(menu.subMenu)
        {
            menu.subMenu = menu.subMenu.map(subMenu => {
                subMenu.action = function (context) {
                    this.insertIframe({
                        src: $.createLink('doc', 'zentaolist', `type=${subMenu.key}`),
        ...
        else
        {
            menu.action = function (context) {
                this.insertIframe({
                    src: $.createLink('doc', 'zentaolist', `type=${menu.key}`),
                    class: 'h-96 overflow-auto',
                }, context);
            }
        }
 ...
 * Migrate doc.
 */
function handleMigrateDoc(doc)
{
    const url       = $.createLink('doc', 'ajaxMigrateDoc', `docID=${doc.id}`);
    const docData   = {html : doc.html};
    return new Promise((resolve) => {
        $.post(url, docData, (res) => resolve(!!res));
    });
}
...
window.setDocAppOptions = function(_, options)
{
    ...
    return $.extend(
    {
        ...
        onMigrateDoc         : userPrivs.edit ? handleMigrateDoc : null,
