...
const commands =
{
    ...
    handleMovedDoc(_, args)
    {
        ...
        const docApp  = getDocApp();
        const loadAndEdit = () => {
            docApp.loadDoc(docID).then(() => {
                const doc = docApp._treeMap.value.docs.get(docID).data;
                docApp.selectDoc(docID);
                if(doc && doc.type === 'text') docApp.startEditDoc(docID);
            });
        };
        if(docApp.mode === 'view')
        {
            if(spaceID === docApp.spaceID) loadAndEdit();
