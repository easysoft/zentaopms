...
window.setZentaoSlashMenu = function(menus, title)
{
    ...
    zui.Editor.setSlashMenu(slashMenus, {before: '$first', reset: true});
 ...
 * 插入禅道列表。
 */
window.insertZentaoList = function(type, blockID, props, tryUpdateFirst)
{
    if(tryUpdateFirst)
    {
        const $block = $('#docApp').find(`.zentao-list[data-id="${blockID}"]`);
    ...
    getDocApp().editor.$.insertCustom($.extend(
    {
...
function showDocBasicModal(docID, isDraft, formData)
{
    const docApp    = getDocApp();
    ...
    zui.Modal.open(
    {
        ...
        request: {
            error: error => {
                ...
                showSaveFailedAlert(error, formData);
            }
        }
    });
...
function showSaveFailedAlert(error, docData)
{
    if(docData) $.db.setCacheData(docData.id || docData.uid || `new-${Date.now()}`, docData, 0, 'unsavedDoc');
    zui.Modal.alert(
    {
        title: getLang('saveDocFailed'),
        content: error instanceof Error ? error.message : String(error),
        actions: [
            {key: 'backup', text: getLang('exportJSON'), type: 'primary', onClick: () => getDocApp().exportDoc('json')},
            {key: 'close'}
        ]
    });
...
function submitNewDoc(doc, spaceID, libID, moduleID, formData)
{
    ...
    return new Promise((resolve, reject) =>
    {
        $.post(url, docData, (res) =>
        {
            ...
            {
                showSaveFailedAlert(error, docData);
                reject(error);
            }
        }).fail(error => {
            resolve(false);
...
{
    return showDocBasicModal(0, doc.status === 'draft', doc).then((formData) => {
...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        if(!isApi)
        {
            ...
            if(doc.contentType === 'doc' || doc.contentType === 'markdown')
            {
                const exportItems = [
                    ...
                    config.debug > 3 ? {text: lang.exportPdf, command: 'exportDoc/pdf'} : null,
                    config.debug > 3 ? {text: lang.exportImage, command: 'exportDoc/png'} : null,
        ...
        const docApp = getDocApp();
        const [_, version] = $('#zuiCSS').attr('href').split('?v=');
        const isOpen = /^\d/.test(version);
