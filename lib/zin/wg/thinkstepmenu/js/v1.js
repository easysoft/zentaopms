...
window.addQuestion = function(stepID, parentID, type, questionType = '')
{
    ...
    setTimeout(function()
    {
        ...
        loadPartial($.createLink('thinkstep', 'create', `marketID=${marketID}&wizardID=${wizard}&parent=${parentID}&type=${type}&questionType=${questionType}`), '.think-step');
        /* 去掉节点收起状态。*/
...
window.saveStep = function()
{
    ...
    $.ajaxSubmit({
        url:  $.createLink('thinkstep', 'ajaxCreate', `marketID=${marketID}`),
        data: {
            wizard,
            title: name,
