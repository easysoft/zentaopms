...
window.addQuestion = function(stepID, parentID, type, questionType = '')
{
    ...
    const style     = `style="margin-left: -18px; padding-left: 16px;"`;
    ...
    {
        if($element.next('.tree').length == 0) $element.after(`<menu class="tree" level="${level}" data-level="${level}"></menu>`);
        $(`div[data-id='${stepID}']`).next('.tree').prepend($title);
        loadPage({url: $.createLink('thinkstep', 'create', `wizardID=${wizard}&parent=${parentID}&type=${type}&questionType=${questionType}`), selector: '.think-step'});
    }, 1);
}
