...
(11, '绘制需求原型图智能体', '根据输入的需求信息, 生成一个HTML格式的原型图示例。', 0, 'story', ',story.title,story.spec,story.verify,story.product,story.module,story.pri,story.category,story.estimate,', 'empty.empty', '根据输入的需求信息，生成一个HTML格式的原型图示例。原型图应包含以下部分： 1.标题 2.整体结构框架（如导航栏、侧边栏、主要内容区） 3.各功能模块的简要设计（如按钮、表单、图表等） 4.交互设计提示（如如何进行用户操作）5.生成一个html代码的代码块。', '请你扮演一名资深的产品经理。', '负责绘制需求原型图的过程。您将需求转化为可视化的原型图，同时确保其符合需求描述和需求验收标准。', 'system', '2025-11-30 12:00:00', 'active');
