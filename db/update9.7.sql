ALTER TABLE `zt_story` CHANGE `stage` `stage` ENUM('','wait','planned','projected','developing','developed','testing','tested','verified','released','closed') NOT NULL DEFAULT 'wait';
...
ALTER TABLE `zt_todo` CHANGE `status` `status` ENUM('wait','doing','done','closed') NOT NULL DEFAULT 'wait';
